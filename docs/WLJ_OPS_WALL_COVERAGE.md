...
| Conversation image retention | ✅ Yes | `media_persistence`: `expired_unpurged` count. Cleaner now exists — daily `apps.ai.tasks.purge_expired_images` drains expired 72h `image_data` (OPS-11 follow-up); the count is the steady-state stall signal + OPS-1 tracks the cleaner's liveness |
...
| ~~**Technical Debt**~~ | ~~**OPS-11 — Retire legacy `_run_autonomous_remediation`**~~ → **DONE (2026-07-12, ADR-26).** Confirmed inert (P3 filter never matches P1/P2-only MISSED_RUN/SUPPRESSION_STORM; escalation only promotes upward) and **removed entirely** — function + `_auto_rerun_engine`/`_auto_clear_suppression` + constants + `run_same()` call site + `auto_remediated` return key + `AutonomousRemediationTest`. Enum choices left intact (immutable historical audit labels). Sole engine re-run path is now the R1 `EngineStarvationRetriggerHandler`. **Also DONE:** the OPS-8b-surfaced expired-image cleaner (`apps/ai/image_retention.py`, daily Beat, `test_image_retention.py` ×8). | ✅ Done |
