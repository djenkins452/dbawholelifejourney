...
_SHORTFALL_STATUSES = {D_PARTIAL, D_SHORTAGE, D_UNSUPPORTED, D_NO_PANTRY, D_NO_QUANTITY}
...
def _deduct_one(household, ri: RecipeIngredient, scale: Decimal, prep: PreparationEvent) -> dict:
    ...
    amount = convert_between(required_scaled, req_unit, pantry.unit)
    if amount is None:
        return {"ingredient": name, "status": D_UNSUPPORTED, "required": float(required_scaled),
                "required_unit": req_unit, "pantry_unit": pantry.unit, "deducted": 0.0,
    ...
    deducted = deduct_pantry_item(
        ...
        notes=f"Prepared {prep.recipe_title or 'recipe'}", preparation=prep)
