...
SLICE_SPECS = [
    ...
    # ==== FINAL TRUTH-LAYER COMPLETION: full-record health entities, events, dietary ====
    QuestionSpec("health.steps_entity", "health", LIST,
                 "Show my full step records (activity ring).", "entity",
                 {"entity_type": "steps"}, {"kind": "entities_min", "n": 3},
                 "health_records", provider="health"),
    QuestionSpec("health.weight_entity", "health", LIST,
                 "Show my weigh-ins with body composition.", "entity",
                 {"entity_type": "weight"}, {"kind": "entities_min", "n": 2},
                 "health_records", provider="health"),
    QuestionSpec("health.glucose_entity", "health", LIST,
                 "Show my glucose readings with context.", "entity",
                 {"entity_type": "glucose"}, {"kind": "entities_min", "n": 2},
                 "health_records", provider="health"),
    QuestionSpec("health.bp_entity", "health", LIST,
                 "Show my blood-pressure readings with context.", "entity",
                 {"entity_type": "blood_pressure"}, {"kind": "entities_min", "n": 1},
                 "health_records", provider="health"),
    QuestionSpec("health.glucose_unit_correctness", "health", TIMELINE,
                 "Glucose trend normalizes mmol/L to mg/dL (correctness).", "history",
                 {"metric": "glucose", "period": "custom",
                  "start": "@glucose_start", "end": "@glucose_end"},
                 {"kind": "series_point_value", "value": "@glucose_mmol_mgdl"},
                 "health_records", provider="health"),

    # ---- SIGNIFICANT EVENTS (all types — new authority) --------------------------
    QuestionSpec("events.upcoming", "events", CURRENT_FACT,
                 "What events are coming up?", "current",
                 {"metric": "upcoming_events"}, {"kind": "present"},
                 "events", provider="events"),
    QuestionSpec("events.list", "events", LIST,
                 "Show all my significant events.", "entity",
                 {"entity_type": "event"}, {"kind": "entities_min", "n": 3},
                 "events", provider="events"),
    QuestionSpec("events.lookup", "events", EXISTENCE,
                 "Tell me about my anniversary.", "entity_one",
                 {"name": "@event"}, {"kind": "entity_found"},
                 "events", provider="events"),

    # ---- MEALS dietary profile + GOALS annual direction --------------------------
    QuestionSpec("meals.dietary_current", "meals", CURRENT_FACT,
                 "What are my dietary targets?", "current",
                 {"metric": "dietary_profile"}, {"kind": "present"},
                 "dietary", provider="meals"),
    QuestionSpec("meals.dietary_entity", "meals", LIST,
                 "Show my dietary profile.", "entity",
                 {"entity_type": "dietary_profile"}, {"kind": "entities_min", "n": 1},
                 "dietary", provider="meals"),
    QuestionSpec("goals.annual_direction", "goals", LIST,
                 "What's my word/theme for the year?", "entity",
                 {"entity_type": "annual_direction"}, {"kind": "entities_min", "n": 1},
                 "goals", provider="goals"),
]
