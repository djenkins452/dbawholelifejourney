...
| ├ P1.2c Framework consumption foundation (autoUpload predicate + server attachment_ids) | ✅ DONE | _(this change)_ | Framework `autoUpload` may be a predicate `(item)→bool` (config, not a chat branch — lets a consumer keep some kinds inline). Server: `multimodal.attachments_from_ids(user, ids)` (owner-scoped resolver) + both chat views parse `attachment_ids` → `CoSGateway.respond` → arrival path merges pre-uploaded artifacts as attachments-as-data (dedup by id; merged before the streaming dispatch too). Additive; existing chat unchanged. Tests: resolver owner-scoping + suite (42) + request-path contract green. NEXT: chat client adoption (thin consumer). |
