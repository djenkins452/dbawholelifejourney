...
class WeightTrendUpRule(BaseInsightRule):
    ...
    def evaluate(self, user, event):
        ...
        if not requires_min_points(entries_list, 3):
            return []

        values_with_dates = [(e[1], float(e[2])) for e in entries_list]
        trend = compute_simple_trend(values_with_dates)

        if not trend or trend["direction"] != "up" or trend["net_change"] < 5:
            return []
        ...
        return [
            {
                ...
                "explain_why": (
                    ...
                    f"of {trend['net_change']:+.1f} {unit} (threshold: +5)."
                ),
                "evidence": {
...
class WeightTrendDownRule(BaseInsightRule):
    ...
    def evaluate(self, user, event):
        ...
        if not trend or trend["direction"] != "down" or trend["net_change"] > -5:
            return []
        ...
        return [
            {
                ...
                "explain_why": (
                    ...
                    f"Gap: {gap_days} days (threshold: 14)."
                ),
                "evidence": {
                    "rule_name": self.rule_name,
