...
- **Ingredient** owns canonical name, aliases, the deterministic `normalized_name` identity key, category, storage type, shelf life, substitution group, density, default unit, and its link to a nutrition source. It does not own quantity-on-hand or nutrient values. **Ingredient identity is a single deterministic authority — Ingredient Intelligence** (`ingredient_intelligence.py`): every acquisition path resolves a name to one canonical ingredient through `resolve_ingredient` (exact → alias → normalized key → create), so the same real ingredient never forks into duplicate rows. No fuzzy matching, no AI. Identity (same thing: case/plural/synonym) is distinct from substitution (interchangeable things), which are never merged. Governing doc: `docs/WLJ_INGREDIENT_INTELLIGENCE.md`.
