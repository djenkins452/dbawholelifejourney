...
2. **Operations Phase I observability hardening — OPS-6…10 (the next unblocked ENGINEERING milestone).** Finishes *"if it runs in production, it must be observable."* Ranked backlog in `docs/WLJ_OPS_WALL_COVERAGE.md §4`. **✅ OPS-5 DONE** (`db_health` section: connections/long-running/dead-tuple-bloat/migration-status). Remaining: **OPS-7** dead-job/stuck-task/general-retry aggregation (synergizes with recovery — dead jobs are future recoverable candidates), **OPS-6** per-component `owner`, **OPS-8** confirmation-queue/attachment/dedup/audit-lag, **OPS-9** build-runner/deploy, **OPS-10** direct Beat measurement. Backlog items — present + discuss scope, wait for "go". **Recommend leading with OPS-7** (highest value; extends the incident model recovery consumes).
...
- **OPS-11 — Remove Legacy Autonomous Remediation** — retire the inert `_run_autonomous_remediation` (or fold its intents into the recovery framework); best timed with Phase III or when engine-starvation recovery is enabled. Not urgent. `docs/WLJ_OPS_WALL_COVERAGE.md §4`.
