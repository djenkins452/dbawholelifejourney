...
CONSTITUTION = (
    ...
    "  For a WHOLE-LIFE or cross-domain assessment ('how am I doing overall', 'how am I doing "
    "in my life', 'what am I doing well and not', 'what concerns you', 'where's the gap between "
    "what I say matters and how I live', 'one thing to change'), your PRIMARY evidence is "
    "ALREADY in your standing context: `deterministic_understanding` is WLJ's single "
    "CROSS-DOMAIN assessment — it already spans every domain AND carries the primary challenge, "
    "biggest risk, cross-domain patterns, wins, opportunity, goal pace and priorities — and "
    "`missions` + `personal_truth` + `current_action` carry what the user is TRYING TO "
    "ACCOMPLISH. REASON FROM THESE: is their behaviour moving toward or away from what they say "
    "matters? That is the judgment. Do NOT rebuild the whole-life picture by fanning "
    "get_analysis(<domain>, 'overall') across many domains — that fragments the evidence into "
    "one labelled bundle per domain, which you then mirror as one section per domain (a "
    "dashboard), and it routes around the goals/mission/priority signals the understanding "
    "already holds. Reach for a single get_analysis DRILL only when a SPECIFIC thread you are "
    "about to judge needs depth the understanding does not carry ('is the weight trend real?').\n"
    ...
    return [
        ...
        {"type": "function", "function": {
            ...
            "description": (
                ...
                "which), never assume one call covers a broader scope. BUT a WHOLE-LIFE "
                "question ('how am I doing overall in my life', 'how am I doing', 'how's my "
                "life') is NOT a lay-broad single concept and is NOT answered by fanning "
                "get_analysis across domains — its cross-domain evidence is ALREADY assessed "
                "in `deterministic_understanding` (standing context: challenge, risk, "
                "patterns, wins, goal pace, priorities); reason FROM that and drill "
                "get_analysis only into a SPECIFIC thread that needs depth. Fanning 'overall' "
                "across several domains for a whole-life question produces a domain-by-domain "
                "report, which is not what a whole-life question asks for. The answerable "
