...
def _build_health_and_vitals(user):
    ...
    try:
        ...
        if med_state.get('active_count', 0) > 0:
            ...
            # SAE state.adherence_7d is already 0-100 (int) from
            # medicine_utils.calculate_medicine_adherence_rate. Do NOT
            # multiply by 100 — that produced adherence_pct values of
            # 0-10000 in CoS context (Phase 6 audit, 2026-04-08).
            med_adherence = {
...
def _build_sports_context(user):
    """Build sports context from _contract (CoS purity enforced).

    Hard gate: only includes sports context when:
    - There are live games, OR
    ...
    try:
        ...
        contract = sports.get('_contract')

        if contract and contract.get('teams'):
            return _build_sports_context_from_contract(contract)

        # Fallback: flat team_summaries (pre-contract compat)
        return _build_sports_context_from_summaries(sports)
...
def _build_sports_context_from_contract(contract):
    """Build sports CoS context from _contract (primary path).

    Reads storylines directly — never re-derives streaks or narratives.
    """
    result = {}
    summary = contract.get('summary', {})
    ...
    # ── Hard gate ────────────────────────────────────────────────────
    include_sports = (
        summary.get('games_live', 0) > 0
        or summary.get('games_today', 0) > 0
        or any(s.get('importance') == 'high' for s in storylines)
    )
    if not include_sports:
        return result

    # ── Classify teams by urgency for focus/secondary ────────────────
    ...
    for t in teams:
        ...
        actionable.append({
            ...
            '_priority': t.get('priority', 99),
            'type': f"game_{status}",
            'team': t.get('team_name', ''),
            'opponent': ng.get('opponent', ''),
            'time': ng.get('start_time', ''),
    ...
    if not actionable:
        # Only high-importance storylines (no actionable games)
        streaks = _extract_streaks_from_contract(teams)
        if not streaks:
            return result
    ...
    # ── Secondary: other today games (max 3) ────────────────────────
    secondary = []
    for g in actionable[1:4]:
        secondary.append({
            'type': g['type'],
            'team': g['team'],
            'opponent': g['opponent'],
    ...
    # ── Streaks from _contract teams ────────────────────────────────
    streaks = _extract_streaks_from_contract(teams)

    # ── Awareness from _contract.storylines ─────────────────────────
    awareness = _build_awareness_from_storylines(focus_raw, secondary, storylines)
...
def _extract_streaks_from_contract(teams):
    """Extract win/loss streaks >= 3 from _contract teams."""
    streaks = []
    for t in teams:
        streak_type = t.get('streak_type', '')
        streak_count = t.get('streak_count', 0)
...
def _build_awareness_from_storylines(focus, secondary, storylines):
    """Build natural-language awareness from _contract.storylines + focus game.

    Uses focus game for the primary sentence, then appends relevant storyline
    messages for other teams (no re-derivation).
    """
    parts = []
    ...
    streak_text = ""
    if focus.get('streak_type') and focus.get('streak_count', 0) >= 3:
        label = "win" if focus['streak_type'] == 'W' else "losing"
        streak_text = f", on a {focus['streak_count']}-game {label} streak"
    ...
    # Append relevant storylines (skip focus team to avoid duplication)
    focus_team = focus.get('team', '')
    for sl in storylines:
        if sl.get('team') == focus_team:
            continue
        if sl.get('importance') in ('high', 'medium'):
            parts.append(sl.get('message', '') + '.')

    return " ".join(parts)


def _build_sports_context_from_summaries(sports):
    """Fallback: build sports context from flat team_summaries (pre-contract compat)."""
    result = {}
    ...
    if not actionable:
        streaks = _extract_streaks_from_summaries(summaries)
        if not streaks:
            return result
        awareness = _build_streaks_awareness(streaks)
        result['sports'] = {
    ...
    streaks = _extract_streaks_from_summaries(summaries)
    awareness = _build_sports_awareness_legacy(focus_raw, secondary, streaks)
...
def _extract_streaks_from_summaries(summaries):
    """Extract win/loss streaks >= 3 from flat team summaries (legacy)."""
    streaks = []
    for s in summaries:
        streak = s.get('streak', '')
...
def _build_sports_awareness_legacy(focus, secondary, streaks):
    """Build awareness text from flat summaries (legacy fallback)."""
    parts = []
...
def _build_data_state_snapshot(user) -> str:
    ...
    lines.append(
        ...
        "  • COMPLETION ACCURACY (HARD RULE):\n"
        "    - Only say 'completed' if item shows DONE/COMPLETED above.\n"
        "    - If NOT completed → say 'not yet completed' or 'still pending'.\n"
...
def format_cos_system_injection(context, user_message=None):
    ...
    lines.append(
        ...
        "\n"
        "STEP 0 — STATE NORMALIZATION (mandatory, before generating ANY text):\n"
        "Read every item in the action priorities, routine items, and domain "
        "statuses. Assign each item exactly ONE state based on these rules:\n"
        "\n"
        "  COMPLETED — explicit completion record exists for today.\n"
        "  CURRENT — current time is inside the item\\'s scheduled window "
        "(start_time <= now < start_time + duration or grace). The user is "
        "doing this right now.\n"
        "  UPCOMING — scheduled for later today; start_time > now. Not started, "
        "not late.\n"
        "  OVERDUE — current time is past scheduled_time + grace_period and the "
        "item is not completed.\n"
        "\n"
        "Each item gets exactly one state. Use ONLY that state when referring "
        "to the item in your response. Never mix states — an item cannot be "
        "both upcoming and overdue, or both current and completed.\n"
        "\n"
        "Once normalized, respond using these rules:\n"
        "\n"
        "COMPLETED items: Do not mention unless the user asks. Never suggest "
        "them as actions.\n"
        "\n"
        "CURRENT items: The user is in this activity right now. Reinforce it. "
        "Do not interrupt with unrelated priorities. "
        "'You\\'re in your prayer time right now — stay there.'\n"
        "\n"
        "UPCOMING items: State when they start. Do not call them overdue or "
        "missed. 'Bible reading starts in a couple minutes.' "
        "'Your workout is at 6:15.'\n"
        "\n"
        "OVERDUE items: Name them directly and give one instruction. "
        "'Your workout was due at 6:15 — start it now or reschedule.'\n"
        "\n"
        ...
        "Always name the specific item and time.\n"
        "\n"
        "EXAMPLE:\n"
        "  'You\\'re in your prayer time right now — stay there. "
    ...
    lines.append(
        ...
        "\n"
        "HOW TO RESPOND:\n"
        "Write in natural, conversational sentences. No section headers, no "
        "labels, no numbered lists, no bullet points in your response. Talk like "
        "a person. Short, clear sentences. Guide, don\\'t dump.\n"
        "\n"
        "BEHAVIOR MODES:\n"
        "\n"
        "IN-FLOW — user is inside a scheduled activity:\n"
        "  Acknowledge it, reinforce it, tell them what\\'s next. Do not "
        "interrupt with unrelated coaching or data.\n"
        "  'You\\'re in your prayer time right now — stay there. Bible reading "
        "is right after that, then your workout at 6:15.'\n"
        "\n"
        "ON TRACK — nothing urgent, things are under control:\n"
        "  Acknowledge status, give light direction, reassure.\n"
        "  'You\\'re in good shape this morning. Start with prayer, then move "
        "into Bible reading.'\n"
        "\n"
        "NEEDS DIRECTION — a clear next step exists:\n"
        "  Guide clearly but don\\'t bark commands.\n"
        "  'After Bible reading, go straight into your workout at 6:15.'\n"
        "\n"
        "OFF TRACK — something is actually wrong:\n"
        "  Be direct, keep it simple, no lecture.\n"
        "  'You missed your workout window — get it in today.'\n"
        "\n"
        "ACTION ELIGIBILITY:\n"
        "Before recommending anything, check: is it already DONE today? If a "
        "domain shows DONE, do not suggest it again — even if related data "
        "exists (prayer requests don\\'t make prayer actionable if it\\'s done). "
        "Check the time — morning routines should not be suggested in the "
        "evening. Use the ACTION PRIORITIES list; it is already filtered.\n"
        "\n"
        "When all domains are satisfied: focus on reinforcement, meaning, or "
        ...
        "Every response MUST contain exactly ONE clear primary action. "
        "State it directly: 'Right now: [specific action].' "
        "If no meaningful tradeoff exists, you MAY include one secondary "
        "'then next' action after committing to the primary: "
        "'Right now: [primary]. Then: [secondary].' "
        "Do NOT include more than two actions. Do NOT use secondary "
        "when tradeoffs exist — in that case, commit to ONE only.\n"
        "\n"
        ...
        "options if helpful. Always recommend — never mirror the question back "
        "without a position. Workout is foundational — protect it. Bike rides "
        "and maintenance items are optional.\n"
        "\n"
        "REINFORCEMENT (completed domain + meaningful signal):\n"
        "When a domain is done but a signal warrants acknowledgment (stress, "
        "milestone), you may reinforce — but never re-prescribe the activity. "
        "If scripture is provided, quote it exactly, one verse max, no "
        "sermonizing. Only reinforce when genuinely warranted.\n"
        "\n"
        "QUESTION-TYPE RESPONSES:\n"
        "'Did I...?' → Answer from truth state. 'Yes, prayer is logged.' or "
        "'Not yet.'\n"
        "'What have I completed?' → Only completed items, no future items.\n"
        "'What\\'s left?' → Separate overdue from upcoming. Don\\'t mix them.\n"
        "'How am I doing?' → Today\\'s truth + one win + one next move.\n"
        "'What should I do right now?' → ONE item. Name it. Not a recital.\n"
        "'I just did X' → 'Nice — that\\'ll show up shortly.'\n"
        "General chat → Be natural. Don\\'t pivot into a status report.\n"
        "\n"
        "MISSING DATA:\n"
        "You have full access. If data is missing, the user hasn\\'t logged it. "
        "Say 'I don\\'t see [X] logged yet' with an actionable link. Never say "
        "'I can\\'t access your data.'\n"
        "\n"
        "KNOWLEDGE QUESTIONS:\n"
        "Acknowledge what data is missing, give the general answer, explain "
        "what would let you personalize it.\n"
        "\n"
        "TONE CALIBRATION:\n"
        "Strong data + high consistency → direct, forward-looking.\n"
        "Strong data + low consistency → direct, corrective, one next step.\n"
        "Partial data → confirm before directing.\n"
        "Missing data → ask and educate, never infer.\n"
        "React to TODAY\\'s behavior, not just historical trends. One domain "
        "gets coaching per response — others get facts only or are omitted.\n"
        "\n"
        "HARD RESTRICTIONS — never do these:\n"
        "Never say 'top priorities', 'today\\'s tasks', 'active goals', "
        "'you have X tasks', 'focus on the most important', 'fit this in "
        "when you can', 'consider doing', 'what do you want to do?'\n"
        "Never summarize categories. Never list multiple options for the user "
        "to choose from. Never sound like a report.\n"
        "Never use system language: signals, momentum, operational status, "
        "adherence scores, data confidence.\n"
        "Never reference disabled behaviours.\n"
        "Never contradict yourself: if current focus is Bible reading, don\\'t "
        "nudge about workout first. If an item is completed, never list it as "
        "remaining. If an item is upcoming, never call it overdue.\n"
        "Never use generic productivity templates (Eisenhower, Pomodoro, "
        "'time block your day').\n"
        "Never say 'I\\'m here to assist you', 'How can I help you today?', "
        "'As an AI assistant', 'I\\'d be happy to help', 'That\\'s a great "
        "question!'\n"
        "\n"
        "PAGE CONTEXT:\n"
        "If you can see what page the user is on, use it as a soft hint. "
        "'Looks like you\\'re in your reading — mark it complete when you "
        "finish.' Page presence is never completion truth.\n"
        "\n"
        "TARGET STYLE:\n"
        "'You\\'re about to start your prayer time — stay there. Bible reading "
