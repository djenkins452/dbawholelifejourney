...
- **Deferred recovery pilot** — chat-queue requeue only (needs a proven idempotency/dedup design; promotion trigger in `WLJ_OPERATIONS_PHASE2_PLAN.md §1.1`). Snapshot recoveries are settled, not deferred: integrity/storage aren't candidates (SAME rewrites them every cycle), and the maturity snapshot is **implemented** (`MATURITY_SNAPSHOT_STALE` + `MaturitySnapshotRefreshHandler`).
