...
Hub view reads pre-computed view model from cache — zero raw GameEvent queries.
...
class SportsHubView(LoginRequiredMixin, SportsEnabledMixin, TemplateView):
    """
    My Teams hub page — signal-driven, cache-first.

    Reads a pre-computed view model from cache. On cache miss,
    builds it synchronously (lightweight, single user).
    Zero raw GameEvent queries on the request path.
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        # Quick check: does user have any followed teams?
        has_teams = UserTeamFollow.objects.filter(user=user, is_active=True).exists()
        context["has_teams"] = has_teams

        if not has_teams:
            return context

        # Read pre-computed view model from cache
        from apps.sports.services.cache_manager import get_user_view_model, warm_view_model_for_user

        vm = get_user_view_model(user)
        if vm is None:
            # Cache miss — warm synchronously (lightweight, lock-guarded)
            vm = warm_view_model_for_user(user)
        ...
        # Template convenience: extract metadata
        meta = vm.get("meta", {})
        context["last_updated"] = meta.get("last_updated")
        context["data_source"] = meta.get("data_source", "")
