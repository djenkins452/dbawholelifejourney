...
**PRODUCT is the North Star (REQUIRED — read before the development model):** The only success metric is: *"If this were the only conversation a paying customer ever had with Beth, would they immediately want to use her again tomorrow?"* Elegant architecture, a passing Layer 2, a correctly-selected handler — none of that is the product; the customer experiences **trust**, not layers. EVERY production transcript begins with a **Product Review BEFORE the Architecture Review**, in this order: (1) Would a paying customer trust this conversation? (2) If not, why — in customer terms (she contradicted herself / forgot / answered the wrong question)? (3) Only THEN, which architectural layer caused it. Never the reverse. Trust-breakers are fixed one at a time, wherever they live (often Layer 1 truth or Layer 4 experience, NOT orchestration), ranked by trust impact. The Conductor is a tool, not the goal — do not advance its roadmap because it is "next"; advance it only when it is the highest-trust-impact fix. **As Chief Architect, call it out plainly when engineering is improving but the product experience is not.**

**Development model (REQUIRED):** For ANY production issue in a Beth / Chief-of-Staff surface, FIRST classify which architectural layer failed — **Truth (WLJ) → Conductor (orchestration) → Capability (reasoning) → Experience** — and fix the first layer that failed (diagnose top-down). Do NOT reflexively build a new capability: only a proven Layer 3b gap (no existing capability covers the situation) warrants one; a wrong owner is Layer 2 (fix orchestration), a correct owner reasoning badly is Layer 3a (improve the capability). The Conductor is the least-frequently-modified layer — repeated changes to it are an architectural smell. Governing doc: `docs/WLJ_CONDUCTOR_DEVELOPMENT_MODEL.md`.
...
| `docs/WLJ_CONDUCTOR_DEVELOPMENT_MODEL.md` | **GOVERNING development model — read at the START of ANY Beth/Chief-of-Staff production issue, and BEFORE proposing a new capability. Classify the failing layer (Truth→Conductor→Capability→Experience), fix the first that failed; only a proven Layer 3b gap warrants a new capability. The Conductor is the least-modified layer.** |
