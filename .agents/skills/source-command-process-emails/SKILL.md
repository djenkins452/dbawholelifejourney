---
name: "source-command-process-emails"
description: "Migrated source command `process-emails`"
---

# source-command-process-emails

Use this skill when the user asks to run the migrated source command `process-emails`.
...
```bash
curl -s -X POST -H "X-Codex-API-Key: $WLJ_CLAUDE_API_KEY" "https://wholelifejourney.com/admin-console/api/Codex/process-emails/"
```
...
```bash
curl -s -X POST -H "X-Codex-API-Key: $WLJ_CLAUDE_API_KEY" "https://wholelifejourney.com/admin-console/api/Codex/process-emails/?max_emails=20"
```
...
```bash
curl -s -X POST -H "X-Codex-API-Key: $WLJ_CLAUDE_API_KEY" "https://wholelifejourney.com/admin-console/api/Codex/process-emails/?dry_run=true"
```

## Diagnostics (check settings, connection, folder access)

```bash
curl -s -X POST -H "X-Codex-API-Key: $WLJ_CLAUDE_API_KEY" "https://wholelifejourney.com/admin-console/api/Codex/process-emails/?diagnose=true"
```
