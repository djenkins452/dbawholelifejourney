...
class FinanceClosureAPIView(APIRateLimitMixin, View):
    """TEMPORARY operator surface for the Finance 2.0 accounting closure. Read-mostly.

    Three questions that can only be answered against real production data:
    where does the net-worth total come from, does the transfer total double count, and
    what does full-population pairing actually find.

    Read-only actions verify their own claim — a row count is taken before and after and
    a mismatch returns an error instead of a report. Write actions refuse without an
    explicit `reviewed=yes`.

    Delete this view, its URL and its test in ONE commit once the closure is verified.

    GET /admin-console/api/claude/finance-closure/?email=&action=
    Auth: X-Claude-API-Key.
    """

    rate_limit_requests_per_minute = 8
    rate_limit_requests_per_hour = 80
    rate_limit_key_prefix = 'admin_api_finance_closure'

    ACTIONS = ('net_worth_recon', 'transfer_audit', 'pairing_rehearse',
               'pairing_apply', 'measures', 'snapshot')
    WRITE_ACTIONS = ('pairing_apply', 'snapshot')
    ...
    def get(self, request):
        ...
        action = (request.GET.get('action') or 'measures').strip()
        if not email:
        ...
        if action not in self.ACTIONS:
            return JsonResponse({'error': f'action must be one of {self.ACTIONS}'},
                                status=400)
        if action in self.WRITE_ACTIONS and \
                (request.GET.get('reviewed') or '').lower() != 'yes':
            return JsonResponse(
                {'error': 'refusing to write: pass reviewed=yes only after the '
                          'rehearsal has been read'}, status=400)
        ...
        return getattr(self, f'_{action}')(request, user)

    def _net_worth_recon(self, request, user):
        from apps.finance.models import FinancialAccount, TangibleAsset
        from apps.finance.services.finance_calc import reconciliation as R

        before = (FinancialAccount.objects.filter(user=user).count(),
                  TangibleAsset.objects.filter(user=user).count())
        report = R.reconcile_net_worth(user)
        after = (FinancialAccount.objects.filter(user=user).count(),
                 TangibleAsset.objects.filter(user=user).count())
        if before != after:
            return JsonResponse({'error': 'RECONCILIATION WROTE TO THE DATABASE',
                                 'before': before, 'after': after}, status=500)
        report['read_only_proof'] = {'accounts_and_assets_before': before,
                                     'after': after}
        return JsonResponse(report, json_dumps_params={'indent': 1})

    def _transfer_audit(self, request, user):
        from apps.finance.services.finance_calc import transfer_audit as TA

        return JsonResponse(TA.audit(user), json_dumps_params={'indent': 1})

    def _pairing_rehearse(self, request, user):
        from apps.finance.models import Transaction
        from apps.finance.services import transfer_detection as TD

        before = Transaction.objects.filter(
            user=user, transfer_pair__isnull=False).count()
        report = TD.rehearse_pairing(user)
        after = Transaction.objects.filter(
            user=user, transfer_pair__isnull=False).count()
        if before != after:
            return JsonResponse({'error': 'REHEARSAL WROTE TO THE DATABASE',
                                 'paired_before': before, 'paired_after': after},
                                status=500)
        report['read_only_proof'] = {'paired_before': before, 'paired_after': after}
        return JsonResponse(report, json_dumps_params={'indent': 1})

    def _pairing_apply(self, request, user):
        from apps.finance.services import transfer_detection as TD
        from apps.finance.services.finance_calc import backfill

        result = TD.pair_all(user)
        result['reclassified'] = backfill.run(user, commit=True)
        return JsonResponse(result, json_dumps_params={'indent': 1})

    def _snapshot(self, request, user):
        from apps.finance.services.finance_calc import net_worth as NW

        return JsonResponse(NW.take_snapshot(user, commit=True),
                            json_dumps_params={'indent': 1})

    def _measures(self, request, user):
        from apps.finance.models import Transaction
        ...
        return JsonResponse({
            'coverage': {
                ...
                'held_for_review': rows.filter(
                    economic_role=Transaction.ROLE_UNCERTAIN).count(),
                'paired': rows.filter(transfer_pair__isnull=False).count(),
