...
Every architectural decision in WLJ traces back to one pipeline (codified in `@WLJ_SYSTEM_PROMPTS/03_CANON_REFERENCE/WLJ ARCHITECTURE LAWS.md`, Laws 1–2):
...
*Generated by read-only architecture discovery. No code, data, engines, signals, state, dashboards, or integrations were modified. Companion reference docs: `@WLJ_SYSTEM_PROMPTS/03_CANON_REFERENCE/` (Architecture Laws, Domain Registry, Signal Ontology).*
