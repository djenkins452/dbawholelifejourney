...
| ├ P1.2d **Chat panel = first framework consumer** (thin consumer) | ✅ DONE | _(this change)_ | `assistant_panel.html` DELETED its bespoke attachment logic (`attachedImages`/`handleImageFile`/`handleImageFiles`/`renderImagePreviews`/`removeImage`/`clearAttachment`) and now mounts `WLJAttachments.mount()` — images inline for perception (`autoUpload` predicate), non-images → artifact_ids; universal `accept`; chips+thumbs; send threads `getImagesPayload()`+`getArtifactIds()`. **Fixed a real framework bug** (`autoUpload` function was coerced to `true` by `!== false`). Bumped cache-bust to `?v=20260719c`. Browser-verified on the real panel: PDF chip + image thumb; send body = image inline + PDF as single `attachment_ids`; image NOT double-uploaded; preview clears; no console errors. NEXT: same adoption for `chat_widget.html` (mobile). |
