...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # One-time: Reload release_notes for Executive Voice + Goal Hierarchy
        # (release note PK 257)
        self._reset_executive_voice_release_note(DataLoadConfig, force, verbosity)
        ...
        # One-time: Reload release_notes for Conversational Discipline / Orientation (PK 261)
        self._reset_orientation_release_note(DataLoadConfig, force, verbosity)
        ...
        # One-time: Reload release_notes for "Your Body Story" executive briefing (PK 270)
        self._reset_body_story_release_note(DataLoadConfig, force, verbosity)

        # One-time: Reload release_notes for Body Shape + Limb Development visuals (PK 271)
        self._reset_body_visual_stories_release_note(DataLoadConfig, force, verbosity)
...
Tasks are sorted by priority (ascending) then creation date.""",
    ...
    def _reset_legacy_places_fixtures(self, DataLoadConfig, force=False, verbosity=1):
        """
        One-time reset to reload fixtures for the Legacy Places domain.
        Reloads release_notes (PK 243), teaching_destinations (PKs 188-189),
        and help_topics (PK 158) so the Places What's New, teaching destinations,
        and help topic appear.
        """
        reset_tracker_name = 'reset_legacy_places_fixtures_2026_07_06'
        ...
        try:
            for loader_name in ('release_notes', 'teaching_destinations', 'help_topics'):
                try:
                    ...
                    config.reset()
                    if verbosity >= 1:
                        self.stdout.write(f'  Reset {loader_name} loader for Legacy Places')
                except DataLoadConfig.DoesNotExist:
                    pass

            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset fixtures for Legacy Places (Jul 2026)',
                'command', 'One-time reset to reload release notes, teaching destinations, and help topics for Legacy Places'
            )
    ...
    def _reset_daypart_stance_release_note(self, DataLoadConfig, force=False, verbosity=1):
        """
        One-time reset to reload release_notes (PK 253) so the What's New entry for
        Daypart Reasoning & Conversation Awareness appears in production.
        """
        reset_tracker_name = 'reset_daypart_stance_release_note_2026_07_07'
        ...
        try:
            try:
                ...
                config.reset()
                if verbosity >= 1:
                    self.stdout.write('  Reset release_notes loader for Daypart Stance')
            except DataLoadConfig.DoesNotExist:
                pass

            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for Daypart Stance (Jul 2026)',
                'command', 'One-time reset to reload the release note for Daypart Reasoning & Conversation Awareness'
            )
    ...
    def _reset_executive_voice_release_note(self, DataLoadConfig, force=False, verbosity=1):
        """
        One-time reset to reload release_notes (PK 257) so the What's New entry for
        Executive Voice + Goal Hierarchy (natural language, one mission thesis) appears.
        """
        reset_tracker_name = 'reset_executive_voice_release_note_2026_07_07'
        ...
        try:
            try:
                ...
                config.reset()
                if verbosity >= 1:
                    self.stdout.write('  Reset release_notes loader for Executive Voice')
            except DataLoadConfig.DoesNotExist:
                pass

            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for Executive Voice (Jul 2026)',
                'command', 'One-time reset to reload the release note for Executive Voice + Goal Hierarchy'
            )

        except Exception as e:
            if verbosity >= 1:
                self.stdout.write(self.style.ERROR(f'Reset Executive Voice release note FAILED: {e}'))

    def _reset_executive_filtering_release_note(self, DataLoadConfig, force=False, verbosity=1):
        """
        One-time reset to reload release_notes (PK 258) so the What's New entry for
        Executive Filtering (filter routine, end with judgment, concrete actions) appears.
        """
        reset_tracker_name = 'reset_executive_filtering_release_note_2026_07_07'
        ...
        try:
            try:
                ...
                config.reset()
                if verbosity >= 1:
                    self.stdout.write('  Reset release_notes loader for Executive Filtering')
            except DataLoadConfig.DoesNotExist:
                pass

            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for Executive Filtering (Jul 2026)',
                'command', 'One-time reset to reload the release note for Executive Filtering'
            )
    ...
    def _reset_orientation_release_note(self, DataLoadConfig, force=False, verbosity=1):
        """
        One-time reset to reload release_notes (PK 261) so the What's New entry for
        Executive Conversational Discipline (orient, then hand back) appears in production.
        """
        reset_tracker_name = 'reset_orientation_release_note_2026_07_07'
        ...
        try:
            try:
                ...
                config.reset()
                if verbosity >= 1:
                    self.stdout.write('  Reset release_notes loader for Orientation')
            except DataLoadConfig.DoesNotExist:
                pass

            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for Orientation (Jul 2026)',
                'command', 'One-time reset to reload the release note for Executive Conversational Discipline'
            )

        except Exception as e:
            if verbosity >= 1:
                self.stdout.write(self.style.ERROR(f'Reset Orientation release note FAILED: {e}'))

    def _reset_goal_scope_release_note(self, DataLoadConfig, force=False, verbosity=1):
        """
        One-time reset to reload release_notes (PK 262) so the What's New entry for
        Goal Question-Scope discipline (one named goal → that goal only) appears.
        """
        reset_tracker_name = 'reset_goal_scope_release_note_2026_07_07'
        ...
        try:
            try:
                ...
                config.reset()
                if verbosity >= 1:
                    self.stdout.write('  Reset release_notes loader for Goal Scope')
            except DataLoadConfig.DoesNotExist:
                pass

            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for Goal Scope (Jul 2026)',
                'command', 'One-time reset to reload the release note for Goal Question-Scope'
            )
    ...
    def _reset_body_story_release_note(self, DataLoadConfig, force=False, verbosity=1):
        """
        One-time reset to reload release_notes (PK 270) so the What's New entry for
        "Your Body Story" — the executive briefing that now leads Body Intelligence —
        appears.
        """
        reset_tracker_name = 'reset_body_story_release_note_2026_07_13'
        ...
        try:
            try:
                ...
                config.reset()
                if verbosity >= 1:
                    self.stdout.write('  Reset release_notes loader for Your Body Story')
            except DataLoadConfig.DoesNotExist:
                pass

            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for Your Body Story (Jul 2026)',
                'command', 'One-time reset to reload the release note for the Body Story briefing'
            )

        except Exception as e:
            if verbosity >= 1:
                self.stdout.write(self.style.ERROR(f'Reset body story release note FAILED: {e}'))

    def _reset_body_visual_stories_release_note(self, DataLoadConfig, force=False, verbosity=1):
        """
        One-time reset to reload release_notes (PK 271) so the What's New entry for the
        Executive Visual Story — Body Shape + Limb Development — appears.
        """
        reset_tracker_name = 'reset_body_visual_stories_release_note_2026_07_13'
        ...
        try:
            try:
                ...
                config.reset()
                if verbosity >= 1:
                    self.stdout.write('  Reset release_notes loader for Body Shape + Limb Development')
            except DataLoadConfig.DoesNotExist:
                pass

            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for Body Shape + Limb Development (Jul 2026)',
                'command', 'One-time reset to reload the release note for the Executive Visual Story'
            )

        except Exception as e:
            if verbosity >= 1:
                self.stdout.write(self.style.ERROR(f'Reset body visual stories release note FAILED: {e}'))

    def _reset_change_map_release_note(self, DataLoadConfig, force=False, verbosity=1):
        """
        One-time reset to reload release_notes (PK 272) so the What's New entry for
        "How Your Body Changed" — the animate-the-difference Change Map — appears.
        """
        reset_tracker_name = 'reset_change_map_release_note_2026_07_13'
        ...
        try:
            try:
                ...
                config.reset()
                if verbosity >= 1:
                    self.stdout.write('  Reset release_notes loader for How Your Body Changed')
            except DataLoadConfig.DoesNotExist:
                pass

            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for How Your Body Changed (Jul 2026)',
                'command', 'One-time reset to reload the release note for the Change Map'
            )
    ...
    def _reset_health_certification_fixtures(self, DataLoadConfig, force=False, verbosity=1):
        """
        One-time reset to reload release_notes (PK 288) for Health Knowledge
        Certification: the CoS can now answer target-adherence ("are my carbs in line?")
        and period comparison ("this week vs last week") questions across health metrics.
        """
        reset_tracker_name = 'reset_health_certification_2026_07_29'
        ...
        try:
            try:
                ...
                config.reset()
                if verbosity >= 1:
                    self.stdout.write('  Reset release_notes loader for health certification')
            except DataLoadConfig.DoesNotExist:
                pass

            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for Health Knowledge Certification (Jul 2026)',
                'command', 'One-time reset to reload PK 288 release note'
            )
    ...
    def _reset_event_frequency_fixtures(self, DataLoadConfig, force=False, verbosity=1):
        """
        One-time reset to reload release_notes (PK 290) for Event Frequency Analysis:
        the CoS can now answer "are my overnight lows getting more frequent", "are severe
        lows increasing", and "what time of night do my lows happen" from deterministic
        event counts across recurring windows, with the reused frequency trend.
        """
        reset_tracker_name = 'reset_event_frequency_2026_07_30'
        ...
        try:
            try:
                ...
                config.reset()
                if verbosity >= 1:
                    self.stdout.write('  Reset release_notes loader for event frequency')
            except DataLoadConfig.DoesNotExist:
                pass

            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for Event Frequency Analysis (Jul 2026)',
                'command', 'One-time reset to reload PK 290 release note'
            )
    ...
    def _reset_executive_assessment_platform_fixtures(self, DataLoadConfig, force=False,
                                                      verbosity=1):
        """
        One-time reset to reload release_notes (PK 294) for the Executive Assessment platform:
        whole-domain executive assessment is now a reusable platform behavior whose coverage
        tracks composed truth (state + trends), so every area of life — finances,
        relationships, medication, and more — answers a broad question, not just health.
        """
        reset_tracker_name = 'reset_executive_assessment_platform_2026_08_02'
        ...
        try:
            try:
                ...
                config.reset()
                if verbosity >= 1:
                    self.stdout.write('  Reset release_notes loader for exec-assessment platform')
            except DataLoadConfig.DoesNotExist:
                pass

            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for Executive Assessment platform (Aug 2026)',
                'command', 'One-time reset to reload PK 294 release note'
            )
    ...
    def _reset_intent_evolution_release_note(self, DataLoadConfig, force=False, verbosity=1):
        """
        One-time reset to reload release_notes (PK 265) so the What's New entry for
        Conversational Intent Evolution (Status -> Diagnosis: the Chief of Staff
        recognizes when the user shifts from checking status to wanting to understand
        why something changed, and pivots into grounded diagnostic reasoning) appears.
        """
        reset_tracker_name = 'reset_intent_evolution_release_note_2026_07_07'
        ...
        try:
            try:
                ...
                config.reset()
                if verbosity >= 1:
                    self.stdout.write('  Reset release_notes loader for Intent Evolution')
            except DataLoadConfig.DoesNotExist:
                pass

            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for Intent Evolution (Jul 2026)',
                'command', 'One-time reset to reload the release note for Intent Evolution'
            )

        except Exception as e:
            if verbosity >= 1:
                self.stdout.write(self.style.ERROR(f'Reset Intent Evolution release note FAILED: {e}'))
    ...
    def _reset_journal_mentions_release_notes(self, DataLoadConfig, force=False, verbosity=1):
        """One-time reset to reload release_notes after adding PK 277 (Journal @mention
        person recognition — type @ to tag canonical people in a journal entry)."""
        reset_tracker_name = 'reset_journal_mentions_2026_07_18'
        try:
            if self._is_loader_complete(DataLoadConfig, reset_tracker_name):
                return
            try:
                ...
                if config.is_loaded:
                    ...
                    config.save()
                    if verbosity >= 1:
                        self.stdout.write('  Reset release_notes for Journal @mentions (PK 277)')
            except DataLoadConfig.DoesNotExist:
                pass
            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for Journal @mentions', 'command',
                'One-time reset: added PK 277 for Journal @mention person recognition')
        except Exception as e:
            if verbosity >= 1:
    ...
    def _reset_journal_editor_layout_release_notes(self, DataLoadConfig, force=False, verbosity=1):
        """One-time reset to reload release_notes after adding PK 285 (Journal edit screen —
        editor fills the space, mood/tags expanded, sticky actions)."""
        reset_tracker_name = 'reset_journal_editor_layout_2026_07_20'
        try:
            if self._is_loader_complete(DataLoadConfig, reset_tracker_name):
                return
            try:
                ...
                if config.is_loaded:
                    ...
                    config.save()
                    if verbosity >= 1:
                        self.stdout.write('  Reset release_notes for journal editor layout (PK 285)')
            except DataLoadConfig.DoesNotExist:
                pass
            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset release_notes for journal editor layout', 'command',
                'One-time reset: added PK 285 for the Journal editor layout')
        except Exception as e:
            if verbosity >= 1:
                self.stdout.write(self.style.ERROR(
                    f'Reset journal editor layout release notes FAILED: {e}'))
    ...
    def _reset_finance_2_0_completion(self, DataLoadConfig, force=False, verbosity=1):
        """One-time reset for the Finance 2.0 completion workspaces."""
        reset_tracker_name = 'reset_finance_2_0_completion_2026_08_31'
        try:
            if self._is_loader_complete(DataLoadConfig, reset_tracker_name):
                return
            for loader in ('release_notes', 'help_topics', 'teaching_destinations'):
                try:
                    ...
                    if config.is_loaded:
                        ...
                        if verbosity >= 1:
                            self.stdout.write(
                                f'  Reset {loader} for Finance 2.0 completion')
                except DataLoadConfig.DoesNotExist:
                    pass
            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset fixtures for the Finance 2.0 completion', 'command',
                'One-time reset: budgets/reserves, net worth, data health')
        except Exception as e:
            if verbosity >= 1:
                self.stdout.write(self.style.ERROR(
    ...
    def _reset_recurring_intelligence(self, DataLoadConfig, force=False, verbosity=1):
        ...
        reset_tracker_name = 'reset_recurring_intelligence_2026_09_01'
        try:
            if self._is_loader_complete(DataLoadConfig, reset_tracker_name):
                return
            for loader in ('release_notes', 'help_topics', 'teaching_destinations'):
                try:
                    ...
                    if config.is_loaded:
                        ...
                        if verbosity >= 1:
                            self.stdout.write(
                                f'  Reset {loader} for recurring intelligence')
                except DataLoadConfig.DoesNotExist:
                    pass
            self._mark_loader_complete(
                DataLoadConfig, reset_tracker_name,
                'Reset fixtures for recurring intelligence', 'command',
                'One-time reset: detected recurring series, review and CRUD')
        except Exception as e:
            if verbosity >= 1:
                self.stdout.write(self.style.ERROR(
                    f'Reset recurring intelligence fixtures FAILED: {e}'))
