...
1. **Next engineering milestone → OPS-9 (Build-runner / deploy-pipeline observability — re-scoped small)** — *Infrastructure.* External Railway build; the high-value slice is already proxied by OPS-5 (migrations) + OPS-7 (post-deploy task fallout), so scope tightly (e.g., deploy-marker + startup-task success). **✅ OPS-8a + OPS-8b DONE** (`confirmation_audit`, `media_persistence`). Also worth a quick win: **expired-image cleanup task** (a Layer-1 hygiene task OPS-8b discovered — 72h `image_data` is never purged; this is *action*, not observability). Follow the OPS-2/5/7 pattern for any new monitor. Backlog task — present + discuss scope, wait for "go". `docs/WLJ_OPS_WALL_COVERAGE.md §4.1`.
2. **Then the rest of the post-Phase-I backlog** (`§4.1`): OPS-11 (retire inert legacy remediation — opportunistic tech-debt). **OPS-6** (`owner`) deferred (near-zero value for a single-owner system); **OPS-10** (direct Beat) closed as accept-inference (subsumed by scheduler_health + OPS-1 + OPS-7).
...
- **WLJ Operations is the primary initiative.** Engineering (Claude, next chat): start **OPS-9 — Build-runner/deploy observability (re-scoped small)** OR the **expired-image cleanup task** (a hygiene gap OPS-8b discovered). OPS-8a + OPS-8b are done. Present the scope, discuss, wait for "go". Follow the OPS-2/5/7 monitor pattern.
