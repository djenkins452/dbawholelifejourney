...
class FinanceOpsAPIView(APIRateLimitMixin, View):
    """TEMPORARY operator surface for the Finance 2.0 completion work. Read-mostly.

    WLJ has no production shell, and the remaining work needs to be REHEARSED against
    real history before it is applied — pairing in particular, where a wrong match
    silently merges two unrelated movements.

    Actions:
      * `pairing_rehearse` — read-only. Why is each held credit unpaired?
      * `pairing_apply`    — runs the improved pairing pass. Requires `reviewed=yes`.
      * `snapshot`         — takes today's net-worth snapshot. Idempotent.
      * `verify`           — coverage, measures, reconciliation, pairing state.

    Delete this view, its URL and its test in ONE commit once the work is verified.

    GET /admin-console/api/claude/finance-ops/?email=&action=
    Auth: X-Claude-API-Key.
    """

    rate_limit_requests_per_minute = 6
    rate_limit_requests_per_hour = 60
    rate_limit_key_prefix = 'admin_api_finance_ops'

    ACTIONS = ('pairing_rehearse', 'pairing_apply', 'snapshot', 'verify')
    #: Actions that change data. Each needs an explicit acknowledgement, so none can be
    #: reached by a guessed URL, a retry, or a link someone pasted into a browser.
    ...
    def get(self, request):
        ...
        action = (request.GET.get('action') or 'verify').strip()
        if not email:
    ...
    def _pairing_rehearse(self, request, user):
        ...
        report['existing_authority'] = PR.existing_authority_limits(user)
    ...
    def _pairing_apply(self, request, user):
        ...
        result = TD.pair_liability_credits(user)
        # Roles depend on pairing, so re-derive them. Idempotent; user decisions safe.
    ...
    def _verify(self, request, user):
        ...
        return JsonResponse({
            ...
            'spending_bridge': {
                ...
                for k, v in M.spending_bridge(measures['net_spending']).items()
                if k != 'steps'},
