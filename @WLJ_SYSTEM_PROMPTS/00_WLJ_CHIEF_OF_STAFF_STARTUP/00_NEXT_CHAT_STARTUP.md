...
## Recently completed
- **Phase II-A recovery foundation — verified as-built + hardened** (2026-07-12) — the recovery foundation (engine/policy/registry/3 R1 handlers incl. the snapshot-refresh `MaturitySnapshotRefreshHandler`/`RecoveryAttempt`+migration 0130/kill switches/gated enqueue/Ops Wall card/contracts) was **already shipped dark** (`b3e6c40a`…). Did NOT rebuild. Added the one genuine gap: a **DB-level per-incident concurrency lock** (`SELECT … FOR UPDATE SKIP LOCKED`, ADR-22) closing the TOCTOU window. Still ship-dark; O1 until a pilot runs in prod. **Next to enable: the operator O1→O2 rollout** (`PHASE2_PLAN §11.1`).
