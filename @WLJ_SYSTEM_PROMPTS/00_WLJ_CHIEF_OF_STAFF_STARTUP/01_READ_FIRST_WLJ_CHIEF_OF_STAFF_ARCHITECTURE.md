...
- **Observability (Ops Wall):** now its own **Layer 1 truth domain — "WLJ Operations"** (`docs/WLJ_OPERATIONS_VISION.md`, architecture frozen 2026-07-11). Phase I visibility **shipped** (OPS-1…4 monitors). **Phase II Deterministic Recovery framework + Phase II-B expanded R1 recoveries shipped dark** — three R1 handlers across two shapes (Beat-task re-enqueue, engine-starvation re-trigger, maturity-snapshot recompute; `OPS_RECOVERY_ENABLED=False` → zero behavior change). **Phase III (recovery-as-config) is NOT yet justified — the missing evidence is controlled production experience, not more abstraction.** The O1→O2 gate is a pilot recovering a real condition in production (operator-gated; not yet done). Governing principle: *"if it runs in production, it must be observable."*
