...
| ├ P1.1a Shared attachment module + image normalization + drag-drop | ✅ DONE | _(this change)_ | New `static/js/wlj-attachments.js` (build once) consumed by BOTH chat surfaces: EXIF orientation, downscale to 2048px, HEIC→JPEG (WebKit/iOS), compress under cap → kills "file too large" + sideways photos; widened `accept` (images incl HEIC → Photos/Camera/Browse via OS sheet); drag-and-drop. Browser-verified (4000px PNG→2048; passthrough small; reject non-image; no console errors). Non-image types (docs/audio/video) = next milestone. |
