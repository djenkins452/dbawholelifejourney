...
- **Confirmation presentation** — when policy requires confirmation, the domain handler returns ONLY structured truth (`confirmation_detail`: what was recognized, what will/won't import + reasons, blank-field count, derived facts). Turning that into the RESULTS-not-intentions text the user sees is owned by ONE generic framework (`apps/ai/import_confirmation.py` — a renderer registry keyed by `renderer`), NOT by any domain handler. Facts only, never a verdict. Body measurements is the first registered renderer; every future import registers a lead+noun and reuses it. A future structured *card* consumes the same `confirmation_detail`; the text presenter is then its graceful-degradation fallback.
