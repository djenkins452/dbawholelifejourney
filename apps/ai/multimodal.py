...
# ── Multimodal import confirmation SUMMARY (RESULTS, not intentions) ──────────
# The generic confirmation pattern for EVERY multimodal import. It reports the RESULT of
# perception and the exact deterministic import plan — what was perceived, exactly what will
# and will NOT import, and WHY — so a perceived value is NEVER silently dropped. Facts only;
# WLJ never renders a verdict, it states what it found and what it will do.
_SKIP_REASON_TEXT = {
    "unrecognized_metric": (
        "WLJ doesn't have a place to store this one yet, so I left it out."),
    "implausible": (
        "the reading looked out of range, so I left it out rather than save a likely misread."),
}


def format_measurement_detail(value, unit):
    """Human-readable value+unit for a single measurement (e.g. 16.29\" / 24.5% / 150 lb)."""
    try:
        v = float(value)
    except (TypeError, ValueError):
        return str(value)
...
def compose_import_confirmation_summary(*, lead, validated, skipped,
                                        derived=None, absent_count=0, noun="measurements"):
    """Deterministic RESULTS-not-intentions confirmation for a multimodal import.

    ``lead``        one-line statement of what was analyzed (e.g. "I analyzed your body
                    measurement screenshot.").
    ``validated``   rows WLJ will import — dicts with ``label``, ``value``, ``unit`` and an
                    optional ``uncertain`` flag (low perception confidence).
    ``skipped``     perceived rows WLJ will NOT import — dicts with ``label``/``metric``,
                    ``value``, ``unit`` and a ``reason`` key (see ``_SKIP_REASON_TEXT``).
    ``derived``     display-only derived facts (e.g. ``{"waist_hip_ratio": 1.16}``).
    ``absent_count``how many perceived fields were blank / not-measured (reported, not hidden).

    Returns the summary text WLJ hands to the model as the confirmation tool-result, so the
    user sees exactly what was recognised and precisely what will and won't be saved.
    """
    validated = list(validated or [])
    skipped = list(skipped or [])
    recognized = len(validated) + len(skipped)
    lines = [lead, "", "Measurements recognized:"]
    for v in validated:
        detail = format_measurement_detail(v.get("value"), v.get("unit"))
        note = "  — please double-check (low confidence)" if v.get("uncertain") else ""
        lines.append(f"✓ {v.get('label')} — {detail}{note}")
    for s in skipped:
        label = s.get("label") or s.get("metric") or "Unrecognized reading"
        detail = format_measurement_detail(s.get("value"), s.get("unit"))
        lines.append(f"⚠ {label} — {detail} (can't import)")

    lines += ["", "Import summary:",
              f"• {recognized} recognized",
              f"• {len(validated)} can be imported",
    ...
    if skipped:
        ...
        for s in skipped:
            label = s.get("label") or s.get("metric") or "Unrecognized reading"
            reason = _SKIP_REASON_TEXT.get(s.get("reason"), "I left it out.")
            lines.append(f"• {label} — {reason}")

    if derived:
        whr = derived.get("waist_hip_ratio")
        if whr is not None:
            lines += ["", f"I'll also show your waist-hip ratio ({whr}), "
                          "derived from waist and hips (never stored separately)."]
