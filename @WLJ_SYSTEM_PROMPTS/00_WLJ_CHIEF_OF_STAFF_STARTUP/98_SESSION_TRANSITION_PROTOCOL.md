...
## 4b. Investigation discipline — hypothesis → runtime proof → decision (before any implementation)
*(Learned 2026-07-21, the "protein yesterday" Truth Retrieval Selection investigation.)* Any investigation of **CoS runtime behavior** (why the assistant retrieved/selected/answered as it did) runs in **three separated phases, in order, and does not skip to code:** **(1) Architectural hypothesis** — read the code, form a candidate cause. **(2) Runtime proof** — reproduce through the *production path* (`CoSGateway.respond(surface=chat)` + the real model) and read the actual `ToolCallLog` before concluding; a code-inference hypothesis is **not** proof and is routinely wrong (Investigation 1's inferred cause was disproved by the trace). **(3) Architectural decision** — only after the runtime evidence agrees. Separating these keeps a plausible-but-wrong architectural story from becoming a fix. Implementation that emerges belongs to the owning track, not the investigation session (transfer it into the bootloader; do not create overlapping work).
