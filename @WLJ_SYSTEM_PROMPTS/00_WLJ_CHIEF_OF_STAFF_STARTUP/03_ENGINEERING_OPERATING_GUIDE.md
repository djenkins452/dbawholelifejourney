...
### 10a. Real-Model Testing Cost Discipline (REQUIRED — this spends OpenAI money)
Real-model runs (`cos-run`, acceptance/natural certification, live `generate()`) cost real credit. A **broad** CoS turn is **3–9 billable provider requests** (initial call + up to 7 tool continuations + Executive Synthesis Phase 2); a multi-turn script multiplies that. The 2026-08 cost surge was a certification program that silently became a load-test. The permanent tiers:

- **Tier 1 — Deterministic/local (DEFAULT, every change):** unit / service / fixture tests, catalog certification (`certify_questions`, ratchet suites), mocked provider boundary, structural evidence inspection. **No real OpenAI call.**
- **Tier 2 — Real-model smoke (after a model-affecting deploy):** exactly **ONE** representative real-model run to prove the production path works.
- **Tier 3 — Repeated real-model investigation:** allowed **only** when the failure is plausibly *stochastic* / grounding-related. State why repetition is needed, start with the minimum, inspect evidence *between* batches, and **never** auto-launch another batch because the previous one failed.
- **Tier 4 — Large natural-cert suite:** reserved for major milestones, domain-program closure, or explicit Danny request — **not** per commit.

**Hard rule (never default to load-testing):** before every real-model batch, answer internally — (1) what deterministic/local evidence is already exhausted? (2) why is a real model necessary? (3) why are *multiple* runs necessary? (4) what decision will the runs resolve? **If those lack concrete answers, do not run the batch.** Cost control is **not** an excuse for fake testing — model reasoning, tool selection, grounding, synthesis, and proactive quality genuinely need real validation; use it *deliberately*, never reflexively. One meaningful real test beats ten repetitive ones.

**Observability (so a surge is never invisible again):** every billable provider request flows through the ONE accounting seam `apps/ai/llm_accounting.py :: record_llm_event` → `owner_finance.LLMUsageEvent` (tokens + cost + `source` + `traffic_class`). Certification traffic is tagged `traffic_class=certification` (acceptance tasks), proactive as `proactive`. Read usage at `/owner/finance/` (dashboard) or `GET /admin-console/api/claude/cost-summary/?days=7` (headless). Full incident + architecture: `docs/WLJ_OPENAI_COST_AUDIT.md`.
...
- **Railway deploys can be slow under load — PENDING ≠ FAILED (learned 2026-08-18).** Deploys usually cut over in ~5 min but have taken **~35 min** on busy days; the `wlj-worker` can also lag the web service and turns can queue behind a backlog (concurrency 2). Before diagnosing a "failed" deploy, **verify by the commit stamp** (web `web_commit` + the worker's `worker_build.commit` via the truth-probe) and confirm the code actually imports; a still-old stamp means *not yet deployed*, not broken. Do not re-push or re-enqueue repeatedly — that only deepens the queue (and, for `cos-run`, wastes model budget).
