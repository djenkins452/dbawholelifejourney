web: python manage.py migrate --noinput && python manage.py load_initial_data && python manage.py reload_help_content && python manage.py load_danny_workout_templates && python manage.py load_reading_plans && python manage.py collectstatic --noinput && gunicorn config.wsgi --log-file -
# Force rebuild: 2026-01-01-v9-remove-pm
