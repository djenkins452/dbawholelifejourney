# Whole Life Journey — Codex Instructions
...
**Real-Provider Calls — HARD RULE, ENFORCED IN CODE (REQUIRED):** **Codex does NOT have standing permission to spend Danny's OpenAI credits.**
...
## Current Context — every page is Chief-of-Staff-aware (REQUIRED)
...
| Item | Value |
|------|-------|
| **Task API** | `GET /admin-console/api/Codex/ready-tasks/?auto_start=true` |
| **Task Status** | `POST /admin-console/api/Codex/tasks/<id>/status/` |
| **API Key Header** | `X-Codex-API-Key: $WLJ_CLAUDE_API_KEY` |
...
| `docs/WLJ_FINANCE_2_0_ARCHITECTURE.md` | **GOVERNING architecture for ALL Finance work — read FIRST for any Finance / money / debt / budget / asset / net-worth / spending / CoS-financial change. Finance 2.0 = a personal financial operating system with the Chief of Staff as its decision layer. Four layers (Record → Derived Truth → Calculation → Decision); every figure is computed by a NAMED deterministic service and NEVER by the model; every answer carries assumptions, coverage, calculation version and freshness; missing is UNKNOWN, never $0.00. Defines the debt-payoff engine, the spending-opportunity engine, the governed CoS asset summary, and the distortion controls that keep transfers/refunds/card-payments out of spending. Records Danny's standing decision that NO paid valuation provider is authorised. Companion backlog: `WLJ_FINANCE_2_0_BACKLOG.md`.** |
| `docs/WLJ_FINANCE_2_0_BACKLOG.md` | **The phased, dependency-aware Finance 2.0 backlog + status ledger. Read before starting ANY Finance feature — it says which phase the work belongs to and what must exist first. Records the verified Asset Registry contract, the residual gaps, and the exact first implementation slice.** |
...
| `docs/WLJ_EXECUTIVE_REFLECTION_DESIGN_ASSUMPTIONS.md` | **Engineering memory for Phase 4 — read BEFORE changing anything in `apps/ai/reflection/`, the learning gate, EIO routing, or read-back gating. Explains WHY it was built conservatively and what was intentionally NOT built (deferred reinforcement, gated learning, human-reviewed EIOs). Protects against architectural drift — if you're loosening a gate to make the Chief of Staff "smarter," read §4 and §8 first.** |
...
| `docs/WLJ_MULTIMODAL_INTAKE_ARCHITECTURE.md` | **GOVERNING architecture for ALL multimodal intake — read FIRST for ANY upload / attachment / image / video / audio / document / PDF / OCR / transcription / receipt / artifact / "send a file to the CoS" work. ONE reusable platform every domain leverages (never a per-domain upload path): the model perceives; WLJ never interprets bytes; perceived content becomes a normal named intent tagged `source_artifact_id`+`confidence`; one arrival path → one artifact seam → one truth spine (validate→dedup→confirm→execute→audit→link) → artifacts are first-class retrievable truth (DomainTruth entity + Current Context + gallery). Aligns with the Constitution (no Review). Conform or amend deliberately. Execution companion: `docs/WLJ_MULTIMODAL_INTAKE_ROADMAP.md` (scorecard + phased milestones + status ledger; progress lives there + changelog, never in the governing doc).** |
...
| `docs/WLJ_SECURITY_AUTHORIZATION_FRAMEWORK.md` | **GOVERNING architecture (RATIFIED 2026-07-18; architecture only — NOT yet implemented) — read FIRST for ANY authorization / identity / permissions / roles / admin-access / sharing / delegation / impersonation / AI-authority work. The complete long-term authorization model: primitives **Identity · Space · Capability · Ownership · Delegation · Trust**; one deterministic Policy Decision Point (PDP); roles are capability *bundles* (never `is_staff`/role-string branches); **Space is the canonical container** — every resource belongs to exactly one Space, ownership/capabilities/authorization are evaluated *within* a Space, and the **Platform is NOT a Space**; the CoS is a *derived* principal that never self-authorizes and never exceeds the human it acts for; platform authority is fully separate from data ownership. Today's `user_id` ownership is the *physical stand-in* for a person's Personal Space — Space is the architectural truth, never a permanent `user_id` shortcut. Fits within the Constitution (no Review; Article VI NOT created). Distinct from `WLJ_SECURITY_PRIVACY_RETENTION.md` (privacy/retention). Conform to this doc or amend it deliberately.** |
