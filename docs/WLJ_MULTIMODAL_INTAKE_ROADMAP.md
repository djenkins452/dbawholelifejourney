...
| ├ P1.2a Universal intake — server foundation (validator + upload endpoint) | ✅ DONE | _(this change)_ | Generalized `upload_validation.py`: byte-sniffing for ALL classes (image incl TIFF/HEIC, PDF, Office [PK+ext], text/CSV, MP3/WAV/M4A/AAC, MP4/MOV) + per-class size caps + graceful unsupported rejection. New endpoint `POST /assistant/api/attachments/` (`AssistantAttachmentUploadView`) → shared `store_and_persist_artifact` primitive → durable `MultimodalArtifact` (background). Multi-file. Tests `apps/ai/tests/test_attachment_validation.py`; browser-verified end-to-end (PDF→document, PNG+WAV multi-file sniffed to image+audio, unsupported→400). Client UX (preview chips/icons/progress) + chat wiring = next sub-milestones. |
