...
# ============================================================================
# TEMP DIAGNOSTIC — Person Recognition glass-box (Stage 1).
# Read-only, login-required, self-scoped to request.user. Answers the entry-206
# "Heather" reproduction against prod truth. REMOVE COMPLETELY once evidence is
# captured (this view + its URL, one commit; verify `manage.py check`).
# ============================================================================
from django.contrib.auth.decorators import login_required


@login_required
def recognition_diag(request):
    """Read-only evidence for the Person-recognition reproduction. Never mutates."""
    from django.contrib.contenttypes.models import ContentType
    from django.db.models import Q

    u = request.user
    entry_pk = int(request.GET.get('entry', 206))
    name = request.GET.get('name', 'Heather')
    out = {'user_id': u.id, 'entry_pk': entry_pk, 'name': name}

    # ── Gates the journal mention signals check ──
    prefs = getattr(u, 'preferences', None)
    out['gates'] = {
        'ai_enabled': getattr(prefs, 'ai_enabled', None),
        'personal_assistant_enabled': getattr(prefs, 'personal_assistant_enabled', None),
    }

    # ── The entry itself (owned by the caller only) ──
    entry = JournalEntry.objects.filter(pk=entry_pk, user=u).first()
    if entry:
        body = getattr(entry, 'body', '') or ''
        out['entry'] = {
            'found': True,
            'created_at': str(getattr(entry, 'created_at', None)),
            'body_len': len(body),
            'contains_name': name in body,
            'contains_at': '@' in body,
            'body_snippet': body[:200],
        }
        # Mention rows on this entry (the legacy relationships.Mention sidecar)
        from apps.relationships.models import Mention
        ct = ContentType.objects.get_for_model(JournalEntry)
        ms = Mention.objects.filter(content_type=ct, object_id=entry_pk)
        out['mentions_on_entry'] = [
            {'person_id': m.person_id,
             'person': getattr(m.person, 'get_display_name', lambda: str(m.person))()}
            for m in ms.select_related('person')[:25]
        ]
    else:
        out['entry'] = {'found': False, 'note': 'no entry with that pk owned by this user'}

    # ── "Heather" across the three Person authorities (caller-scoped) ──
    def dump(qs, fields, limit=25):
        rows = []
        for p in qs[:limit]:
            rows.append({'pk': p.pk, **{f: str(getattr(p, f, '')) for f in fields}})
        return rows

    from apps.relationships.models import Person as RP
    from apps.core.ai_relationships.models import Person as AP
    from apps.people.models import Person as PP

    out['relationships_person'] = {
        'total_for_user': RP.objects.filter(owner=u).count(),
        'name_matches': dump(
            RP.objects.filter(owner=u).filter(
                Q(display_name__icontains=name) | Q(first_name__icontains=name)),
            ['display_name', 'first_name', 'last_name']),
    }
    out['ai_relationships_person'] = {
        'total_for_user': AP.objects.filter(user=u).count(),
        'name_matches': dump(
            AP.objects.filter(user=u).filter(display_name__icontains=name),
            ['display_name']),
    }
    out['people_person'] = {
        'total_for_user': PP.objects.filter(user=u).count(),
        'name_matches': dump(
            PP.objects.filter(user=u).filter(
                Q(display_name__icontains=name) | Q(first_name__icontains=name)),
            ['display_name', 'first_name', 'full_name']),
    }

    # ── Canonical resolver behaviour for the name ──
    try:
        from apps.people.services.resolution import resolve
        r = resolve(u, name)
        out['canonical_resolve'] = {
            'status': r.status,
            'person': getattr(r.person, 'display_name', None),
            'candidates': [getattr(c, 'display_name', None) for c in r.candidates],
        }
    except Exception as ex:  # pragma: no cover - diagnostic only
        out['canonical_resolve'] = {'error': str(ex)}
