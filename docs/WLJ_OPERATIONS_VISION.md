...
## 5a. Operational Disposition Model (Governing)

> **Measure the truth. Improve the truth. Never game the score.**

§5 is what the system does **autonomously** (detect → recover → verify → close). This is the
**engineering** counterpart: how a human resolves any operational issue that reaches investigation.
Every detected operational issue MUST end in **exactly one** deterministic disposition. This turns
ad-hoc firefighting into a consistent, auditable lifecycle.

```
Detect → Investigate → Disposition {FIX · RETIRE · RECONFIGURE · ACCEPT} → Verify → Close
```

**The four dispositions (exactly one per issue):**

| Disposition | When | Discipline |
|---|---|---|
| **FIX** | Genuine defect in an **active** subsystem. | Smallest safe fix; preserve architecture + request-path safety; prove root cause first. |
| **RETIRE** | Obsolete/dormant subsystem or dead path. | **Reduce operational surface** (remove the scheduled job / dead code) — prove blast radius first. NEVER "retire" a monitor of an active subsystem (that is suppression). |
| **RECONFIGURE** | Subsystem is **healthy** but the monitor/config does not reflect its true, intended behavior. | **Encode the real model** (e.g. the actual dispatch cadence). NEVER weaken a health rule to hide a real problem — RECONFIGURE corrects a *false positive*, it does not relax a *true* one. |
| **ACCEPT** | Understood, low-impact, correct-by-design. | Record the residual **and** the promotion trigger that reopens it (ties to §3.2 "eliminate the class" — accept only what cannot be eliminated at proportionate cost). |

**Non-negotiable rules:**
- **Evidence-first.** No FIX / RETIRE / RECONFIGURE without production evidence proving the root cause. A passing unit test is not proof (`WLJ_RUNTIME_TRACE_DEBUGGING.md`). If evidence contradicts the investigation, **stop and report** — do not proceed on the hypothesis.
- **The score follows the truth, never the reverse.** Improving the Operations score is only ever a *consequence* of FIX/RETIRE/RECONFIGURE/ACCEPT applied to reality — never the goal, and never achieved by changing thresholds, suppressing monitors, or hiding alerts.
- **Record it.** Every disposition + its evidence is logged in the §15 Disposition Ledger; a disposition that touches architecture or a Constitution Article also gets an ADR (§16).
- **Close only after Verify** confirms the condition cleared (FIX/RECONFIGURE) or the surface is gone with no user-facing regression (RETIRE) or the residual + trigger are recorded (ACCEPT).

**On "EVOLVE" — why there is NO fifth disposition (a deliberate decision).** A tempting
fifth disposition, EVOLVE, was considered for conditions that are *operationally correct
today but strategically not where the product is headed* (e.g. Relationships: the
pipeline works, but relationship intelligence shouldn't depend on explicit logging
forever). **We rejected it, and ACCEPT absorbs the case via an optional evolution
pointer.** Reasons:
- The four dispositions are a **closed, terminal set** — each answers "what do we do
  about this *condition/monitor* now?" and lets the incident **Close**. EVOLVE is not
  terminal (it stays open as a roadmap item) and is not about the condition — it is a
  **product-roadmap** decision. Mixing a roadmap direction into the operational
  disposition set breaks the "Close" step and blurs Operations truth with product
  planning.
- **Almost every ACCEPT has a future.** If "there's a better long-term design" promoted
  ACCEPT to EVOLVE, ACCEPT would nearly vanish and the distinction would carry no
  information.
- **The strategy still gets recorded** — but where it belongs. An ACCEPT may carry a
  **product-evolution pointer**: a one-line link from the ledger entry to the product /
  architecture artifact that tracks the direction (e.g. `→ WLJ_PASSIVE_SIGNAL_ARCHITECTURE.md`).
  The operational incident **Closes as ACCEPT** (the monitor is correct, nothing to
  change in Operations); the *product ambition* lives in the roadmap it belongs to, not
  in the operational ledger.

So the disposition set stays **FIX / RETIRE / RECONFIGURE / ACCEPT** (four, terminal),
and "evolution" is an annotation on ACCEPT, not a state of its own.

*Placement rationale:* this lives in the Operations Vision (not the general engineering guide) because it is Operations-subsystem governance — a direct peer of the automated Recovery Lifecycle (§5) and the Maturity Model (§6), and it feeds the same ADR log and status ledger.
...
### 15.1 Operational Disposition Ledger (§5a)

The evidence-backed disposition of each investigated operational condition. `PENDING-EVIDENCE` = investigation
complete, disposition blocked on a production telemetry read (never acted on a hypothesis).

| Condition | Investigation finding | Disposition | Evidence | Status |
|---|---|---|---|---|
| **`sports.sync_games_from_provider` MISSED_RUN** | Default-OFF, read-only CONTEXT feature, no followers in default product; dormant 15-min ESPN sync; ~zero customer value; blast radius negligible (consumers gated; on-demand self-heal remains). | **RETIRE** | Code: `context_processors.py:163` (default OFF), `apps/sports/tasks.py:54-56` (self-heal), gated consumers (`cos_context.py`, `state_builder.py`). No prod evidence needed — dormancy is structural. | ✅ Retired 2026-07-13 (Beat entry removed; task kept registered) |
| **`core.check_system_health` (COAS) MISSED_RUN** | Name matches; task swallows all exceptions (can't self-report error). COAS *scorers* are reused **live** by the maturity engine (`maturity_engine.py:92-96`, bypasses the snapshot); the 5-min snapshot task's only unique consumers are the `coas_health` Ops Wall panel (`ops_telemetry.py:641,2736`) + `OperationalAlert` rows (admin-surfaced). Marginal/redundant, not cleanly obsolete. | **RECONFIGURE** (recommended) | Architectural review (§P4). Confirming prod read: `coas_health.computed_at` vs `scheduled_tasks[core.check_system_health].last_run_at` (fresh-vs-stale ⇒ mis-recording vs genuine miss). | Recommended — evolve the panel to the live scorers + fold COAS alerts into SAME; retire the standalone 5-min writer. **Not yet actioned** (per directive: review only). |
| **`capture.process_pending_captures` MISSED_RUN** | Active feature; idempotent 5-min sweep; name + queue + recorder all correct in code; no defect explains a stale `ran_at`. Likely transient dispatch gap. Highest potential customer impact of the set (stuck captures). | **PENDING-EVIDENCE** (FIX or ACCEPT) | Requires prod `scheduled_tasks` row: `OK`/recent ⇒ ACCEPT (transient); `MISSED`+`success`+old ⇒ FIX (dispatch); `error` ⇒ FIX (task-path). | Blocked on prod read — **not acted on**. |
| **DNE engine MISSED_RUN** ("Engine Execution") | Healthy engine flagged: writes an `EngineRun` unconditionally every cycle; 600s cadence + 60s fallback jitter = 660s deadline, but dispatched on the 300s ISE grid (real spacing 600–900s) ⇒ flaps. Monitor false-positive. | **RECONFIGURE** | Prod evidence (Engine Health card, made discoverable): DNE reporting on cadence — Engine Execution HEALTHY, 10/10 engines on cadence (score 96). Effectively closed. | ✅ Closed 2026-07-13 — production evidence shows DNE healthy; jitter change unnecessary. |
| **`finance` SIGNAL_DROUGHT (~1008h)** | Coming-soon domain: sole expected type `financial_health` is stubbed (`signal_aggregation.py:83`); the health seed excludes it (`ops_telemetry.py:1576-1597`), but `_merge_domain` re-adds it via a stray ~42-day-old signal → flagged. Not meant to produce signals. | **RECONFIGURE** | Code-proven (registry + stub list + seed logic). The drought detector now runs only on seed-eligible domains (`signal_eligible`), so coming-soon domains are not held to a signal expectation. No threshold change, no finance special-case. | ✅ Done 2026-07-13 (ADR-29) — `signal_eligible` gate; `tests_signal_health.py` (+4). |
| **`cross_domain` SIGNAL_LOW_DIVERSITY (2 types / 1475·7d)** | Engine-output module (correlation rules, `rules_cross_domain.py:37`), noise-budgeted; NOT a registered behavioral domain. Few insight types at volume is normal for a correlation engine; flagged only because it leaked via `_merge_domain`. | **RECONFIGURE** | Code-proven. The diversity detector now runs only on seed-eligible behavioral domains, excluding engine modules. "More diversity" is explicitly NOT a target. | ✅ Done 2026-07-13 (ADR-29) — same `signal_eligible` gate. |
| **`relationships` SIGNAL_DROUGHT** | Real, enabled domain; producer `_compute_relational_engagement` correct — emits a signal ONLY when a `RelationshipInteraction` is logged that day (`signal_aggregation.py:929-930`). Silence = genuine user inactivity, not a fault. | **ACCEPT** → *evolution pointer* | Code-proven (producer returns `None` with no data). Pipeline functioning; monitor correct; low-severity P3 by design. | ✅ Closed as ACCEPT 2026-07-13. **Product-evolution pointer → `docs/WLJ_PASSIVE_SIGNAL_ARCHITECTURE.md`** (derive relationship signals passively from journal/capture/calendar rather than explicit logging). Not an Operations change. |

**Note:** the first four surfaced as "Recovery Skipped" only because the beat-retry recovery pilot ships dark (observe-only R0) — a presentation artifact (ADR-27), not a per-subsystem signal. The last three (signal-health) are the platform shifting from exposing *infrastructure* failure to exposing *product* truth. **`relationships` is the first ledger entry to carry an evolution pointer** (see §5a: ACCEPT + pointer, not a fifth disposition).
...
| **Operational Hardening** | **OPS-8b** Media & capture persistence (`media_persistence` section) | [x] **DONE 2026-07-12** — `tests_media_persistence.py` (9). Roadmap corrected: NO S3 (prod = Cloudinary); dedup-rate/orphan/verification-failure signals NOT built (would be fabricated). Discovered: no expired-image cleanup task exists (now closed → the Expired-image cleanup row below, ADR-26). |
| **Technical Debt** | **Expired-image cleanup task** (discovered by OPS-8b) — 72h `AssistantMessage.image_data` / `MessageImage.image_data` were never purged; a Layer-1 hygiene task should add the cleaner. | [x] **DONE 2026-07-12** (ADR-26) — `apps/ai/image_retention.py::purge_expired_images` + daily Beat task `apps.ai.tasks.purge_expired_images`; `test_image_retention.py` (8). Idempotent bulk UPDATE+DELETE; failure caught by OPS-1 (MISSED_RUN) **and** OPS-8b `expired_unpurged` growth. |
...
| **Technical Debt** | **OPS-11** Retire inert legacy `_run_autonomous_remediation` | [x] **DONE 2026-07-12** (ADR-26) — proven inert (P3 filter never matched P1/P2-only MISSED_RUN/SUPPRESSION_STORM) and removed entirely from `same_engine.py` (function + `_auto_rerun_engine`/`_auto_clear_suppression` + constants + call site + return key + `AutonomousRemediationTest`). Sole engine re-run path is now the R1 `EngineStarvationRetriggerHandler`. |
...
| [x] | **Recovery Engine — first-class Ops Wall component** (mode + source, configured/enabled handlers, per-handler enabled + allowlists, beat/engine/maturity flags, 24h verified/failed/escalated/simulations/pending, last activity). Telemetry now publishes EVERY SAME cycle **regardless of mode** so config/status is visible when DISABLED; read-only, no new recovery logic — ADR-24 | 2026-07-12 | *(this commit)* | ship-dark | §4a / ADR-24 | `RecoveryConfigTelemetryTests` |
...
| ADR-24 | 2026-07-12 | **Recovery visibility must not depend on recovery being enabled.** The Ops Wall "Recovery Activity" card became a first-class **Recovery Engine** component exposing existing deterministic truth: mode + source (`describe_mode_source()`), configured vs enabled handlers, per-handler enabled state + allowlist counts (`recovery_config_snapshot()` — the single source; each handler's `is_enabled()`/`allowlist_size()` is also what `diagnose()` reuses), beat/engine/maturity flags, 24h verified/failed/escalated/simulations/pending, and last activity. Because telemetry previously published ONLY inside the mode-gated action task, a DISABLED subsystem showed nothing; a separate read-only `publish_recovery_telemetry_task` now runs EVERY SAME cycle regardless of mode (name-based enqueue, §11 preserved; the mode-gated ACTION enqueue and its resolver mirror are unchanged). No new recovery logic, no invented metrics. | *"Expose existing truth before inventing new capability."* An operator configuring recovery via Railway env vars (`OPS_RECOVERY_MODE`, `OPS_RECOVERY_BEAT_RETRY`, allowlists) could not read back mode/handlers/allowlists/status from the Ops Wall — visibility that must exist BEFORE Shadow-Mode testing. Publishing config/status independent of the enable gate makes the DISABLED state itself observable, which is exactly when an operator most needs it. Facts only (Constitution I.4); readers stay cache-only (request-path safe). |

| ADR-29 | 2026-07-13 | **Signal-health detectors measure only the signal-ELIGIBLE population (RECONFIGURE — finance drought + cross_domain diversity).** `compute_signal_health` seeds a deterministic eligible set (BEHAVIORAL domains with ≥1 non-stubbed expected signal type), but `_merge_domain` re-adds ANY module present in the data — coming-soon domains (finance, via a stray old signal) and engine-output modules (cross_domain, the correlation-rules engine). The drought and diversity detectors previously ran on that leaked population, so finance was flagged SIGNAL_DROUGHT and cross_domain SIGNAL_LOW_DIVERSITY though neither is a domain those monitors are meant to measure. Fix: `compute_signal_health` now stamps each domain `signal_eligible` (captured from the seed, before `_merge_domain`) and excludes non-eligible domains from the aggregate silent/active/stalest counts; `_detect_signal_drought` and `_detect_signal_low_diversity` skip `signal_eligible == False`. No threshold changed, no finance/cross_domain special-case, no alert suppressed — the detectors simply operate on the deterministic set the seed already defines. `relationships` stays eligible and correctly monitored (disposition ACCEPT, not suppression). Proven by `tests_signal_health.py::SignalEligibilityFilterTests` (+4). | The Operations platform has matured from exposing infrastructure failure to exposing product truth; at that maturity, a monitor firing on a population it was never meant to measure (a coming-soon domain, an internal engine) is itself a defect in the truth. Correcting the measured population is RECONFIGURE in its purest form — it makes the score reflect reality without touching a single threshold or hiding a real domain. Deliberately NOT done: weakening the 48h/diversity thresholds, or special-casing finance by name. |
| ADR-28 | 2026-07-13 | **Operational Disposition Model — every operational issue ends in exactly one of FIX / RETIRE / RECONFIGURE / ACCEPT (§5a).** Formalized the ad-hoc "measure the truth, improve the truth, never game the score" practice into a governing lifecycle (Detect → Investigate → Disposition → Verify → Close), placed as the engineering counterpart to the automated Recovery Lifecycle (§5), with an evidence-first rule (no FIX/RETIRE/RECONFIGURE without production root-cause proof; contradicting evidence ⇒ stop) and a §15.1 Disposition Ledger. First applications recorded: **sports sync → RETIRE** (dormant default-OFF job; Beat entry removed, task kept registered for on-demand self-heal — reduces operational surface, not monitor suppression); **DNE → RECONFIGURE** and **capture → FIX/ACCEPT** held **PENDING-EVIDENCE**; **COAS → RECONFIGURE** recommended after review. | The Operations investigation exposed that most "degradation" was monitor false-positives + dormant jobs, not product defects. Without a disposition discipline, the tempting fix is to move thresholds — which hides real problems and destroys trust in the score. Encoding the four dispositions makes "improve the truth" the ONLY sanctioned path and makes RECONFIGURE (correct a false positive) categorically distinct from relaxing a true health rule. Belongs in the Vision (Operations governance), feeding the same ADR log + ledger. |
| ADR-27 | 2026-07-12 | **Recovery Events must distinguish observe-only (R0) from failed recovery.** The first real ACTIVE-pilot events read "Recovery Failed / Verification: Failed" for a `DNE` incident that was in fact an observe-only R0 skip (`phase=SKIPPED_UNSAFE` — target not allowlisted; no `recover()`, no `verify()` ever ran). The engine was correct; the operator EVENT was misleading. Fix is presentation-only in the telemetry producer + Ops Wall JS (no engine/lifecycle/model/migration change; every `RecoveryAttempt` audit row unchanged): (1) a `SKIPPED_UNSAFE` row maps to a distinct `kind=skipped` ("Recovery Skipped", verification "Not applicable", action "No recovery performed… observe-only (R0)"), excluded from `failed_24h` and counted as `skipped_24h`; (2) entity labels resolve the true monitored resource via the engine registry — an engine-heartbeat `MISSED_RUN` shows "Delivery Notification Engine (DNE)" + "Engine heartbeat missed its expected cadence." instead of "Beat task 'DNE'"; (3) expanded events `scrollIntoView` so details near the wall bottom stay readable. | Operator trust: "Recovery Failed" implies a handler ran, recovery was attempted, and verification failed — none true for an R0 skip. Conflating a deliberate no-action with a failure erodes trust in the pilot and would false-alarm for every non-allowlisted engine that misses cadence. Truthful communication (Constitution I.4 — facts, not verdicts) without hiding events. Note (out of scope): `BeatTaskRetryHandler` over-claiming engine-heartbeat `MISSED_RUN` is the deeper routing cause and remains a future consideration; the audit's reuse of `OUTCOME_FAILED` for R0 skips is intentional (complete audit) and distinguished at presentation via the `phase` field. |
| ADR-26 | 2026-07-12 | **OPS-11 technical-debt closure — removed the inert legacy `_run_autonomous_remediation`, and added the deterministic expired-image cleaner it was adjacent to.** (1) **Removal, not disable:** the in-SAME auto-remediation was PROVEN inert — it only acted on `severity="P3" AND anomaly_type ∈ {MISSED_RUN, SUPPRESSION_STORM}`, but the detectors create those two types only at P1/P2 (`same_engine.py` MISSED_RUN P1/P2, SUPPRESSION_STORM P2) and escalation only promotes upward (P3→P2→P1), so the action branches could never fire (the passing tests synthesised a P3+MISSED_RUN row the pipeline cannot produce — a unit test is not runtime proof). The entire path (the function, `_auto_rerun_engine`, `_auto_clear_suppression`, the `AUTONOMOUS_REMEDIATION_ENABLED`/`MAX_AUTO_ACTIONS_PER_CYCLE`/`AUTO_ACTION_COOLDOWN_MINUTES` constants, the `run_same()` call site + `auto_remediated` return key, and `AutonomousRemediationTest`) was deleted; the `AdminIntervention`/`EngineExecutionLog` enum choices are LEFT intact (immutable historical audit labels — removing them is migration churn with prod-display risk and near-zero benefit; logged residual). The sole engine re-run path is now the R1 `EngineStarvationRetriggerHandler` (ADR-20 comment updated). (2) **Cleaner:** `apps/ai/image_retention.py::purge_expired_images` (daily Beat `apps.ai.tasks.purge_expired_images`, 03:15 UTC) purges expired (>72h) chat-image bytes — bulk `UPDATE` clears `AssistantMessage` bytes (row kept: conversation history) and bulk `DELETE` removes expired `MessageImage` attachment rows. Idempotent (filters only match rows still holding expired bytes), request-path safe (background only), observable (returns+logs counts; OPS-1 records `ScheduledTaskRun` and fires MISSED_RUN if it stops; OPS-8b `expired_unpurged` independently catches a stall). | Removes a latent reactivation footgun (a second, boundary-crossing remediation path living inside the truth engine — §11 says truth never acts) rather than leaving disabled dead code; and closes the OPS-8b-surfaced DB-growth gap with a minimal, monitored cleaner that reuses existing Operations infrastructure (Celery Beat + OPS-1 scheduled-task monitor) instead of a new framework. Eliminate-the-class: the inert path is gone, not gated. |
| ADR-25 | 2026-07-12 | **The Ops Wall HTML shell must not be cached (`@never_cache` on `OperationsWallView`).** Stage-0 Shadow validation proved Recovery telemetry correct end-to-end (SHADOW rows written, `/ops/stream/` JSON carried the full `recovery` section) yet the operator saw no Recovery Engine panel. Root cause was purely presentation: the wall view sent no cache headers, so the HTML shell that carries each monitor card's DOM was served stale (browser/WKWebView/CDN), predating the newly-added card; the polled JSON was fresh but the missing DOM node never rendered. Fix adds `Cache-Control: no-cache, no-store, must-revalidate` — no telemetry/recovery change. | Upholds the Operations principle *"if it runs in production, it must be observable."* An always-changing authenticated operational surface whose card set can grow must never let a stale shell hide a running monitor; shell freshness (not just polled-data freshness) is a correctness requirement. Same class as the authenticated-dashboard never_cache rule. |
