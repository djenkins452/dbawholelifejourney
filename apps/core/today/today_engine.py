...
def get_today_context(user) -> dict:
    ...
    # ── Step 4: Next action ──
    # SINGLE canonical next-action source. The check-in, "what should I do
    # next?", and execution guidance MUST agree, so they all consume the same
    # block-gated execution selector via build_locked_next_action (already
    # computed into facts['next_action'] = get_next_action(build_execution_state)).
    # No separate prioritizer here: the selector handles overdue+now with
    # active-block gating, so a long-stale item is never chosen, and there is
    # one truth for "what's next" (trust bug 2026-06-15: the check-in and
    # "what should I do next?" disagreed because they used different engines).
