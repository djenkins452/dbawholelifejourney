...
**Files:** `apps/relationships/models.py`, migration `0003_alter_person_relationship_type.py`
...
- **Feature: UAL v2 — Executive Stability & Adaptation Layer** — Enhances the Universal Arbitration Layer with four new capabilities: (1) **Confidence Dampening** — classifies confidence gap between top two scenarios as LOW/MODERATE/HIGH; LOW confidence limits surfacing to 1 item and softens narrative tone, HIGH allows full suppression. (2) **Scenario History & Pattern Analysis** — ScenarioHistory model tracks daily scenarios; PatternAnalyzer detects multi-day repetitions (MOOD_CRITICAL ≥3/5d, DRIFT_CRITICAL ≥4/7d, HEALTH_CRITICAL ≥3/5d) and adds gentle escalation hints to narrative. (3) **Adaptive Weight Tuning** — WeightAdjustment model tracks per-user weight offsets; every 50 decisions, adjusts by ±0.02 based on user compliance, clamped ±0.10 from baseline. (4) **Capacity Composite Model** — weighted composite of sleep_deficit, mood_decline, emotional_load, schedule_overload, open_loop_count normalised 0-1; classifies HIGH_CAPACITY/NORMAL/LOW/CRITICAL; LOW reduces max surfaced to 2, CRITICAL to 1. Updated pipeline: 10 steps with all new stages wrapped in try/except. 4 new observability panels on staff dashboard (confidence distribution, scenario frequency, capacity trend, weight deltas). 71 tests pass (was 42).
  - Files: `apps/core/ai_arbitration/models.py`, `scenario_classifier.py`, `capacity_engine.py` (new), `pattern_analyzer.py` (new), `weight_tuner.py` (new), `arbitration_engine.py`, `intervention_engine.py`, `narrative_engine.py`, `admin.py`, `tests.py`, `apps/core/models.py`, `apps/core/ai_observability/views.py`, `templates/intelligence/observability_dashboard.html`, `apps/core/migrations/0076_ual_v2_stability_adaptation.py`, `docs/UAL_V2_IMPLEMENTATION_REPORT.md`

- **Feature: Universal Arbitration Layer (UAL) — Executive Judgment Architecture** — Creates a central reasoning layer that sits between multi-engine signal generation and user-facing intervention. UAL collects signals from 13 sources (SAE, PIE, PRIE, PGE, Drift Engine, Relationship Engine, Sleep, Medication, Calendar, Schedule, Mood, Life Events, Open Loops), normalises them into 14 signal dimensions (0-1), classifies into one of 6 scenario types (TIME_CRITICAL, HEALTH_CRITICAL, DRIFT_CRITICAL, MOOD_CRITICAL, RELATIONSHIP_CRITICAL, STABLE_EXECUTION) using weighted scoring, fuses cross-domain composites (7 patterns: LOW_CAPACITY_DAY, PHYSICAL_RISK, RELATIONAL_OPPORTUNITY, EMOTIONAL_OVERLOAD, RECOVERY_NEEDED, ALIGNMENT_CRISIS, DEADLINE_CONVERGENCE), selects one of 6 intervention styles (DIRECTIVE, PROTECTIVE, ACCOUNTABILITY, SUPPORTIVE, STRATEGIC, EXECUTION), and builds a unified executive narrative injected into the system prompt. Max 3 items surfaced, competing signals suppressed. ArbitrationDecisionLog model for refinement. No AI calls — pure deterministic Python. 42 tests pass.
  - Files: `apps/core/ai_arbitration/__init__.py`, `models.py`, `signal_collector.py`, `scenario_classifier.py`, `signal_fuser.py`, `intervention_engine.py`, `narrative_engine.py`, `arbitration_engine.py`, `admin.py`, `tests.py`, `apps/core/models.py`, `apps/ai/personal_assistant.py`, `apps/core/migrations/0075_ual_arbitration_decision_log.py`, `docs/FOLLOWUP_UAL.md`, `docs/UNIVERSAL_ARBITRATION_IMPLEMENTATION_REPORT.md`

- **Feature: Executive Operator — Behavioral Intelligence Upgrade** — Transforms the Chief of Staff from reactive assistant into proactive executive operator. Adds: (1) **Morning Executive Briefing** — structured first-of-day greeting with sleep data, approaching life events, medication/health gate, day overview narrative, and journal pattern follow-up. (2) **Session Gap Detection** — computes time since last interaction, translates to human language ("It's been 3 days"), summarizes what changed/was missed during absence. (3) **Rolling Conversation Memory** — generates AI summary of older messages beyond the 15-message window, stored in `conversation.context_summary`, injected into every response for continuity. (4) **Life Event Surfacing** — queries SignificantEvent and LifeEvent models for approaching events (14-day window), surfaced in CoS context and morning briefing. (5) **Journal Review Intelligence** — mood trend analysis and repeated health keyword detection across recent entries. (6) **Extended Learning Categories** — 3 new extraction patterns (health_concern, life_event_mention, commitment_made) for richer cross-conversation recall. All wrapped in try/except for graceful degradation. 28 new tests, 365 AI tests pass.
  - Files: `apps/ai/executive_briefing.py` (new), `apps/ai/tests/test_executive_briefing.py` (new), `apps/ai/personal_assistant.py`, `apps/core/ai_learning/models.py`, `apps/core/ai_learning/learning_extractor.py`, `apps/core/ai_orchestrator/cos_context.py`, `apps/core/migrations/0074_add_executive_memory_fields.py`, `docs/EXECUTIVE_OPERATOR_IMPLEMENTATION_REPORT.md`

- **Bug Fix: Smart Suggestions showing raw ISO dates instead of formatted dates** — The Time Command Center's Smart Suggestions panel displayed datetimes as truncated ISO strings (e.g., "2026-02-21T06:0...") instead of user-friendly format. Fixed by: (1) Adding `formatDateTime()` JS helper that renders dates as `MM/DD/YYYY h:MM AM/PM`, (2) Formatting suggestion detail elements on page load via JS, (3) Updating `formatTime()` to use uppercase AM/PM with space for consistency, (4) Formatting due dates in suggestion rationale text as `MM/DD/YYYY` in the backend service. Files: `templates/calendar_engine/dashboard.html`, `apps/calendar_engine/services/suggestions.py`.

- **Bug Fix: Dashboard "Today's Guidance" showing duplicate and contradictory items** — The guidance panel was showing multiple weight trend insights from different days (e.g., "-7.0 lb" and "-2.4 lb") and contradictory journal guidance ("4-day journaling streak" alongside "You haven't journaled recently"). Root cause: `get_active_guidance()` had no deduplication by guidance type — each PIE insight generated a unique dedupe key via `insight.id`, so multiple insights of the same type accumulated over the 7-day expiry window. Fix adds two display-time filters: (1) **Per-type deduplication** — only the newest item per `(guidance_type, module)` pair is shown, so only the latest weight trend appears. (2) **Supersession logic** — defines which guidance types supersede others within the same module (e.g., `positive_reinforcement` supersedes `journal_inactivity`), so a journaling streak hides the stale "haven't journaled" item. Older items remain in the database for the full inbox/history but no longer crowd the dashboard tile.
  - Files: `apps/core/ai_guidance/guidance_engine.py`, `apps/dashboard/tests/test_guidance_panel.py`, `apps/core/ai_guidance/tests.py`

- **Enhancement: Full workout data visibility for CoS AI** — CoS previously only saw workout count (e.g., "3 workouts this week") with no detail. Now includes calories burned, duration, distance, average heart rate, and recent workout names/types in all three AI layers: (1) `cos_context.py` health_signals — expanded from just count to include total calories, total minutes, avg HR, distance, and last 3 workout names. (2) `data_service.py` `get_workout_data()` — now aggregates and returns calories_burned, distance_miles, avg_heart_rate per session plus workout_type and source. (3) `context_builder.py` `_format_workout_data()` — formats all new fields into natural language including per-workout detail lines. Also expanded health summary workout section to include full aggregates and recent workout breakdown. Added 4 new search types to `search_service.py`: mobility, heart_rate_events, audio_exposure, dietary_nutrients — previously unsearchable by CoS. Updated workout search to include calories, HR, distance, duration metadata. All 1,246 tests pass.
  - Files: `assistant/data_service.py`, `assistant/context_builder.py`, `apps/core/ai_orchestrator/cos_context.py`, `apps/ai/search_service.py`
...
- **Enhancement: HealthKit workout display — show activity data instead of exercises/sets** — The workout detail and list templates were designed for manual resistance training (exercises, sets, reps, volume). HealthKit-synced workouts like Pickleball have different data: duration, calories burned, distance, avg HR, start/end time — none of which was displayed. Now detects HealthKit workouts (via `source == "apple_health"`) and shows activity-appropriate cards: duration, calories, distance, avg heart rate, start/end times with clock icons. List view shows "45 min • 320 cal • 142 bpm" instead of "0 exercises". Also added Apple Health badge to identify synced workouts. Enhanced iOS app to extract average heart rate from `HKWorkout.statistics(for:)` (iOS 16+) and send it to backend. Backend handler now saves `avg_heart_rate` on WorkoutSession.
  - Files: `templates/health/fitness/workout_detail.html`, `templates/health/fitness/workout_list.html`, `ios/.../HealthMetric.swift`, `ios/.../HealthKitManager.swift`, `apps/mobile/views.py`

- **Bug: CoS workout count always 0 — wrong state engine path** — `cos_context.py` was reading `health.workout_count_7d` but workouts live in the `fitness` module as `workouts_7d`. This meant CoS always saw 0 workouts in health signals, even when the user had synced workouts from Apple Watch. Fixed by changing the path to `fitness.workouts_7d` and adding a direct `WorkoutSession` DB query fallback (like steps/sleep already have).
  - Files: `apps/core/ai_orchestrator/cos_context.py`

- **Fix: CoS unable to see health data for generic queries** — Even after wiring all health models into the AI pipeline, CoS still said "I don't see any HealthKit data" because: (1) Generic phrases like "can you see my health data?" or "any new activity?" didn't match any specific intent keywords — the detector required specific data type keywords like "weight" or "steps". (2) The `cos_context` system prompt only showed health signals when they were alerting (e.g., sleep < 6.5h), not always. (3) The `is_asking_for_analysis` detector in personal_assistant.py didn't include health/HealthKit phrases. Fixed by: (1) Added `health_summary` data type to intent detector with 25+ keywords covering generic health queries ("health data", "healthkit", "apple health", "my vitals", "synced data", etc.). (2) Added `get_health_summary_data()` to PersonalDataService — queries ALL health models and returns a consolidated overview with counts and averages. (3) Added `_format_health_summary_data()` to context builder — produces a clear summary showing exactly what health data CoS can see. (4) Updated `format_cos_system_injection()` to ALWAYS include health signals in the system prompt, not just when alerting. (5) Added health-related phrases to `is_asking_for_analysis` in personal_assistant.py so state data is included when users ask about health. All 1,121 tests pass.
  - Files: `assistant/intent_detector.py`, `assistant/data_service.py`, `assistant/context_builder.py`, `assistant/views.py`, `apps/core/ai_orchestrator/cos_context.py`, `apps/ai/personal_assistant.py`

- **Fix: Heart rate sync creating false 8-hour sleep entries** — When heart rate data synced from Apple Health without a corresponding sleep entry, the `process_heart_rate_metric()` handler created a dummy `SleepEntry` with hardcoded `total_duration_minutes=480` (8 hours), bedtime 10:00 PM, wake time 6:00 AM. This produced uniform false 8.0h sleep readings tagged "Apple Health" for every day. Fixed by removing the dummy sleep entry creation — HR data now only attaches to existing sleep entries, and is skipped when no sleep entry exists. Added data migration `0048_remove_dummy_sleep_entries` to delete all existing dummy entries (identified by exactly 480 minutes, no stage data, no quality rating, apple_health source).
  - Files: `apps/mobile/views.py`, `apps/health/migrations/0048_remove_dummy_sleep_entries.py`, `apps/core/fixtures/release_notes.json`, `apps/core/management/commands/load_initial_data.py`

- **Major: Wire all health data into CoS AI visibility** — CoS was blind to most HealthKit data even though it was syncing correctly. The personal data query system only supported 4 health models (weight, fasting, workouts, medicine) and the search service had 6 types. Expanded the entire AI data pipeline across 7 files to give CoS full visibility into ALL health data. Changes: (1) `assistant/data_service.py` — added 6 new `get_*_data()` methods for steps, sleep, mobility, HR events, audio exposure, dietary nutrients with caching and aggregation. (2) `assistant/intent_detector.py` — added keyword detection for steps, sleep, mobility, heart rate events, audio exposure, dietary nutrients. (3) `assistant/views.py` — added 6 new supported types + type mappings (cardio→steps, fitness→steps). (4) `assistant/context_builder.py` — added 6 new `_format_*_data()` methods for natural language context injection. (5) `apps/ai/personal_assistant.py` `_get_health_state()` — expanded from 4 to 11 health models (added steps, heart rate, sleep, blood pressure, glucose, blood oxygen, HR events). (6) `apps/ai/search_service.py` — added 7 new `_search_health_*()` methods for steps, heart rate, blood pressure, glucose, blood oxygen, water, and expanded the "search all" list from 6 to 12 types. (7) `apps/core/ai_orchestrator/cos_context.py` — expanded health signals with direct model queries for HR avg, BP latest, glucose avg, SpO2 avg, steps avg, sleep avg, and HR event count as fallbacks when state engine values are missing. All 1,998 tests pass.
  - Files: `assistant/data_service.py`, `assistant/intent_detector.py`, `assistant/views.py`, `assistant/context_builder.py`, `apps/ai/personal_assistant.py`, `apps/ai/search_service.py`, `apps/core/ai_orchestrator/cos_context.py`, `assistant/tests/test_views.py`
...
- **Major: Chief of Staff Consolidation — Retire standalone Assistant page** — The `/assistant/` page duplicated the CoS chat panel (available on every page) and had unique content that belonged elsewhere. Consolidated into a single, coherent UX: (1) Created new "Today's Priorities" dashboard tile (`cos_priorities`) with interactive checkboxes, dismiss, and refresh — loads via existing `/assistant/api/priorities/` endpoint. (2) Replaced `AssistantDashboardView` with a `RedirectView` to dashboard home — `/assistant/` now 302s to `/`. (3) Rebranded all 8 navigation references from "Assistant" to "Chief of Staff" — nav links, left rail, top bar alignment badge, quick actions tile, more page, preferences, dev notice modal all now point to `ai:cos_settings`. (4) Deleted `assistant_dashboard.html` (1,776 lines of duplicate chat/priority/CSS/JS code). (5) Updated help topic PK 17 and added release note PK 70. All `/assistant/api/*` endpoints unchanged — CoS panel depends on them.
  - Files: `apps/dashboard/services/config_service.py`, `templates/dashboard/tiles/cos_priorities.html` (new), `templates/dashboard/home.html`, `apps/ai/views.py`, `templates/ai/assistant_dashboard.html` (deleted), `templates/components/navigation.html`, `templates/components/desktop_left_rail.html`, `templates/components/desktop_top_bar.html`, `templates/dashboard/tiles/quick_actions.html`, `templates/core/more.html`, `templates/users/preferences.html`, `templates/components/development_notice_modal.html`, `apps/ai/tests/test_personal_assistant.py`, `apps/core/fixtures/release_notes.json`, `apps/help/fixtures/help_topics.json`, `apps/core/management/commands/load_initial_data.py`

- **Docs: Add 10 missing app sections to features doc** — System audit found `docs/wlj_claude_features.md` was missing sections for 10 apps. Added comprehensive documentation for: Users & Authentication (233 tests), Core Infrastructure (160 tests), Dashboard (108 tests), Journal (107 tests), Life & Organization (226 tests), Admin Console (278 tests), Help System (103 tests), Mobile API & iOS Support (51 tests), Security Assessment (184 tests), and Calendar Engine Technical Reference (31 tests). Each section includes feature overview, key models, views/endpoints, key files, test counts (verified via scoped test runs), and integration points. ToC updated with items 41–50.
  - Files: `docs/wlj_claude_features.md`

- **Major: CoS intelligence upgrade — GPT-4o model + prompt architecture overhaul** — Three changes to make CoS dramatically more natural and perceptive: (1) Model upgrade: Default changed from `gpt-4o-mini` to `gpt-4o` — significantly better at nuance, empathy, reading between the lines, and giving contextually appropriate responses. (2) Prompt architecture consolidation: The system prompt was 6-7 independently prepended layers (calibration, recalibration, alignment, governance, learned profile, operational context, base prompt) that competed for LLM attention. Restructured into a clear priority hierarchy — personality/relationship instructions FIRST, user knowledge SECOND, operational context LAST. Removed duplicate learned profile injection (was in both cos_context AND separate layer). Removed duplicate governance profile (was in both cos_context AND governance instructions). (3) Operational context slimmed: Renamed from "CHIEF OF STAFF OPERATIONAL CONTEXT" to "SITUATIONAL AWARENESS" and cut raw metrics the LLM can't usefully interpret (drift scores, tier weights, override frequencies, weekly pressure details, capacity percentages). Kept only actionable data: schedule, calendar, medication, active insights, predictions, relationship signals. Also increased max_tokens (350→450, 500→600) and raised temperature (0.5→0.65) for more natural conversational tone. Softened rigid user_prompt rules ("Do NOT end with follow-up question") to let GPT-4o's natural conversational ability come through.
  - Files: `config/settings.py`, `apps/ai/personal_assistant.py`, `apps/core/ai_orchestrator/cos_context.py`, `apps/core/ai_orchestrator/briefing_formatter.py`, `apps/core/blueprint/tests.py`
...
- **Fix: Enable AI intelligence for all users by default** — `ai_enabled` defaulted to `False`, meaning all scheduled intelligence engines (PGE guidance, WIRE weekly reports, DBE daily briefings, etc.) silently skipped every user. Changed default to `True` and added data migration to flip all existing users. Intelligence is the core product — it should be on by default, and users can opt out in Preferences if they want.
  - Files: `apps/users/models.py`, `apps/users/migrations/0066_enable_ai_for_all_users.py`

- **Enhancement: Intelligence engines fire on all data entry, not just UAIO actions** — Two major issues prevented the intelligence pipeline (SAE → PIE → PRIE) from producing insights, predictions, and guidance: (1) Regular Django form views (weight, journal, habits, goals, faith, workouts, nutrition, sleep, blood pressure, fasting) bypassed the intelligence chain entirely — only AI-orchestrated actions triggered engines. Created `intelligence_hook.py` utility and wired it into 12 key create views across health, journal, purpose, and faith apps. (2) Insight and prediction rules had overly conservative minimum data thresholds that prevented output for new users. Lowered: weight trend 3→2 entries, net change 5→1 lb; missing weight 14→7 days; body comp gap 30→14 days, change 2%→1%; habit broken streak 7→3 entries, gap 3→2 days; habit consistency 10→5 completions; journal streak 5→2 days, drop-off 10→5 days; scripture drop-off 7→3 days; PRIE weight 3→2, habits 3→1, strength 4→2 sessions.
  - Files: `apps/core/ai_orchestrator/intelligence_hook.py` (new), `apps/health/views.py`, `apps/journal/views.py`, `apps/purpose/views.py`, `apps/faith/views.py`, `apps/core/ai_insights/rules_health.py`, `apps/core/ai_insights/rules_body_composition.py`, `apps/core/ai_insights/rules_habits.py`, `apps/core/ai_insights/rules_journal.py`, `apps/core/ai_insights/rules_scripture.py`, `apps/core/ai_predictions/prediction_rules_health.py`, `apps/core/ai_predictions/prediction_rules_habits.py`, `apps/core/ai_predictions/prediction_rules_transformation.py`, `apps/core/ai_insights/tests.py`, `apps/core/ai_predictions/tests.py`

- **Enhancement: Drop all insight thresholds to minimum + first-entry rules** — Removed remaining conservative thresholds so engines act on ANY activity. Weight trend net_change: 1→0 (any change fires). Missing weight gap: 7→3 days. Missing body comp gap: 14→7 days. Body fat change: 1.0%→0.1%. Habit broken streak: 3→1 entry, gap 2→1 day. Habit consistency: 5→1 completion. Journal streak: 2→1 day. Journal drop-off: 5→3 days. Scripture reading: 3→1 day reading, gap 3→1 day. Created 7 new "first entry" insight rules (`rules_first_entry.py`) that fire immediately on a user's very first data point in each domain: first weight, first journal, first habit log, first scripture reading, first body comp, first workout, first goal — providing instant positive reinforcement instead of silence.
  - Files: `apps/core/ai_insights/rules_health.py`, `apps/core/ai_insights/rules_body_composition.py`, `apps/core/ai_insights/rules_habits.py`, `apps/core/ai_insights/rules_journal.py`, `apps/core/ai_insights/rules_scripture.py`, `apps/core/ai_insights/rules_first_entry.py` (new), `apps/core/ai_orchestrator/intelligence_hook.py`, `apps/core/ai_insights/tests.py`

- **Enhancement: Intelligent Cross-Device Quick Links with Mobile App Deep Linking** — Extended the existing Quick Links feature (Preferences > Quick Links) with mobile app deep linking. Users can now add an optional mobile app URL (e.g., `chase://`, `bofa://`, `mychart://`) to any quick link. On mobile devices, clicking the link attempts the app deep link first with a 1.5-second fallback to the web URL. Added new fields: `mobile_app_url`, `icon`, `category`, `open_in_new_tab`, `usage_count`. All link clicks now route through a redirect view (`/user/open-link/<id>/`) that detects mobile via User-Agent, increments usage, and serves device-appropriate behavior. Added edit modal for existing links, Django admin registration with deep link fieldset, `get_most_used_links()` intelligence hook for future AI integration, and 40 comprehensive tests.
  - Files: `apps/users/models.py`, `apps/users/views.py`, `apps/users/urls.py`, `apps/users/admin.py`, `apps/core/context_processors.py`, `templates/users/preferences.html`, `templates/users/quick_link_redirect.html`, `templates/components/desktop_top_bar.html`, `templates/components/navigation.html`, `apps/users/tests/test_quick_links.py`, `apps/users/migrations/0065_add_deep_link_fields_to_externallink.py`, `apps/core/fixtures/release_notes.json`, `apps/core/management/commands/load_initial_data.py`
...
- **Fix: Intent registration completeness audit + validation test** — Comprehensive audit found systemic registration gaps across the AI engine system: 14 intents had zero examples in the system prompt (fitness, purpose, faith actions poorly recognized), 7 action handlers didn't use HTIE time enrichment (food, fasting, workouts, habits, gratitude couldn't be backdated), 5 modules missing from PA feature links (medical, scan, capture, finance, brain training), transformation/settings intents missing from engine categories and INTENT_HANDLERS. Fixed ALL gaps and created `test_intent_registration.py` — 10 automated tests that cross-reference every tool definition against handler maps, engine categories, dispatch cases, handler methods, and TIME_AWARE_INTENTS. Tests FAIL if any registration is missing, preventing this class of bug from ever shipping again.
  - Files: `apps/ai/tests/test_intent_registration.py` (NEW), `apps/ai/intent_service.py`, `apps/ai/action_handlers.py`, `apps/ai/personal_assistant.py`, `apps/ai/intents/__init__.py`, `apps/core/ai_orchestrator/intent_engine.py`
  - Why: Systemic design flaw — no enforcement that new intents are registered in all 5+ required locations. Every new feature was a manual multi-file registration that got forgotten.

- **Fix: CoS date hallucination and broken intent routing** — The intent recognition system had NO knowledge of today's date. When users said "tomorrow", OpenAI hallucinated a random date (e.g., Oct 04 instead of Feb 20). Fixed by: (1) injecting today's date + weekday into the intent system prompt, (2) adding explicit calendar event examples with relative date resolution, (3) updating the `create_event` tool to accept "today"/"tomorrow" strings, (4) fixing `handle_create_event` to check orchestrator resolved time first with proper fallback chain, (5) adding `LIFE_INTENTS` to `TIME_AWARE_INTENTS` so the time pipeline enriches calendar events. Also added negative examples to prevent the AI from triggering create_event when users share routine info ("I wake up at 5am every day").
...
**New Destinations:**
- **Profile** (`/user/profile/`) - View and edit profile information; includes delete account link
- **Delete Account** (`/user/delete-account/`) - Permanently delete account and all associated data
- **Export Your Data** (`/user/data-export/`) - Download complete personal data copy (JSON or CSV)
