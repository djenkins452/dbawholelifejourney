...
2. **Operations Phase I observability hardening — OPS-6/8/9/10 (the next unblocked ENGINEERING milestone).** Finishes *"if it runs in production, it must be observable."* Ranked backlog in `docs/WLJ_OPS_WALL_COVERAGE.md §4`. **✅ OPS-5 DONE** (`db_health`) and **✅ OPS-7 DONE** (`task_health`: pool-wide stuck/failures/retries/revocations). Remaining: **OPS-6** per-component `owner`, **OPS-8** confirmation-queue/attachment/dedup/audit-lag, **OPS-9** build-runner/deploy, **OPS-10** direct Beat measurement. Backlog items — present + discuss scope, wait for "go". **Recommend leading with OPS-8** (confirmation-queue/audit-lag — closes real correctness-adjacent gaps) or **OPS-6** (`owner` — cheap cross-cutting win).
