web: python manage.py fix_stale_migrations && python manage.py migrate --noinput && python manage.py add_drill_sergeant && python manage.py load_initial_data -v 2 && python manage.py sync_workout_to_templates -v 0 && python manage.py recalculate_task_priorities -v 0 && python manage.py load_project_from_json docs/ux_improvements_tasks.json -v 0 && python manage.py reset_reading_plan_progress && python manage.py collectstatic --noinput && gunicorn config.wsgi --preload --log-file - --timeout 300
