...
| **1 (Shared hover card + relationship-derived recognition)** | ✅ shipped | Shared Person hover card (`static/js/wlj-person-hover.js`, global) — hover any recognized chip → canonical name + relationship + recognized-as surfaces + Open Person; data via `people:card` + the `hooks.person_summary` provider. **Relationship-derived recognition** (`apps/relationships/relationship_recognition.py`): first-person role phrases ("my wife", "my daughter", "my father") resolve to the canonical Person as **deterministic read-only projections** of `relationship_type` — NOT stored `RecognitionPhrase`. Registered via `hooks.resolve_role` + new `hooks.person_roles`/`all_role_phrases`; unique-role-only (ambiguous → no resolve); first-person only. Shown under **Automatically recognized** (read-only); recognized in Journal prose (`PersonMention.source_type=relationship_role`) and the hover card with ZERO Journal changes. Shared `canonical_bridge.ensure_canonical` reused by role resolution. Browser-verified my wife→spouse / my daughter / my father. |
| **0c self-anchor / 0d consumer repoint / 0e legacy / 0f retire** | ⏳ pending | Identity rows now exist canonically, but relationships/legacy/ai_relationships identity tables are **not** yet retired and consumers (SAE, dashboards, mobile import) still read their own stores. Role-phrase resolution now works via the relationships **hook** (no full graph repoint needed yet); a native canonical relationship graph still lands at 0d. The full canonical People browse UI (list + nav) also lands with 0d; `/people/<pk>/` is the first canonical Person page. **Deferred:** gender-aware spouse phrasing (only "my wife" for a female spouse) needs a pronoun/gender field. |
