...
def build_signals(user, band: str) -> dict:
    ...
    if w is not None:
        ...
        s["weight"] = {
            ...
            "current": float(w), "unit": health.get("weight_unit", "lb"),
...
def select_primary_lever(signals, band):
    """The single highest-leverage adjustment. NOT always protein. Returns a
    short phrase or None (no meaningful lever)."""
    candidates = []  # (weakness_rank, phrase)
    # workouts (7d — time-independent)
    wk = signals.get("workouts")
    if wk:
        if wk.get("count", 0) == 0:
            candidates.append((0, "getting a workout or two back on the board"))
        elif wk.get("count", 0) == 1:
            candidates.append((2, "nudging workout frequency up slightly"))
    # sleep (7d — time-independent)
    sl = signals.get("sleep")
    if sl:
        below = sl.get("avg") is not None and sl["avg"] < 7
        declining = sl.get("trend") == "declining"
        low_consistency = isinstance(sl.get("consistency"), (int, float)) and sl["consistency"] < 50
        if declining or low_consistency:
            candidates.append((1, "improving sleep consistency"))
        elif below:
            candidates.append((3, "getting sleep a little closer to 7 hours"))
    # nutrition — only when we can fairly judge it (not morning)
    n = signals.get("nutrition")
    if n and band != "morning" and n.get("protein_pct") is not None and n["protein_pct"] < 80:
        candidates.append((2, "making protein more consistent, especially on workout days"))
    if not candidates:
        return None
...
def _compose_change_anything(signals, band):
    if not _has_min_data(signals):
        return None
    lever = select_primary_lever(signals, band)
    if not lever:
        return ("I wouldn't change anything major right now — your signals are "
                "holding together well. It's more about staying consistent than "
                "making a correction.")
    return (f"I wouldn't make major changes right now. If I picked one thing, "
            f"I'd focus on {lever} — that's the highest-leverage adjustment I "
            f"see. Everything else looks more like refinement than a real "
            f"correction.")
...
_COMPOSERS = {
    ...
    "change_anything": lambda s, b: _compose_change_anything(s, b),
    "pace_check": lambda s, b: _compose_pace_check(s),
    "overtraining": lambda s, b: _compose_overtraining(s),
}


def build_health_analyze(user, msg_lower: str) -> str | None:
