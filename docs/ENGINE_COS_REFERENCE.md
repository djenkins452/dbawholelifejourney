...
**Last updated:** 2026-09-03 (**CoS Cognitive Simplification Stages 0–1** — prompt telemetry (`telemetry.py`), the constitution invariant/guidance split (`constitution_map.py`), and the corrected `_prompt_sections` / CURRENT SITUATION assembly. Ledger: `docs/WLJ_COS_COGNITIVE_SIMPLIFICATION.md`.) Previously 2026-08-20 (**Model Interface runtime documented — this doc previously described ONLY the legacy conversational path.** The certified CoS that actually serves conversations is `apps/ai/model_interface/`, selected by `CoSGateway.resolve_runtime()` on `use_model_interface` (precedence: model-interface → ChatGPT CoS → legacy Beth). Added three things: (1) **CoS Runtimes** — the gateway's three coexisting runtimes and which one owns a turn, plus the second write flag `use_model_interface_writes` (fail-safe read-only); (2) **Model Interface Runtime** — the worker request flow, the **Executive Context Envelope** built by `ModelInterfaceService.build_standing_context` (`ai_relationship`, `deterministic_understanding`, `current_context`, `conversation_state`, `personal_truth`, `missions`, `execution_state`, `current_action`, `pending_confirmations` [writes only], `interview` [active Getting to Know You session only], `artifact_history` [prior-turn uploads — deliberately NOT under `current_context`, which means NOW]) with each field's producer and freshness, the 11-layer `_system_prompt` assembly, the tool surface in `apps/ai/model_interface/constitution.py` (16 truth tools + `navigate_to_workspace` always; the curated named write set from `ALLOWED_WRITE_INTENTS` + `resolve_pending_action` / `complete_execution_item` / `record_interview_knowledge` / `next_review_item` / `schedule_follow_up` only when writes are enabled; **no generic `request_action`**), Phase-2 bounded Executive Synthesis, and the audit/cost-accounting seam; (3) the worker path `run_model_interface_generation` (`apps/ai/model_interface/tasks.py`) — registered via `apps/ai/tasks.py`, **not** in `CELERY_TASK_ROUTES`, so it runs on the default queue in the separate **`wlj-worker`** service, NOT web — `/_health/` reports web only, so verify the worker's commit before validating a CoS change. The former **CoS Context Pipeline** section (request flow, 19 parallel builders, v4–v8 prompt layers, `format_cos_system_injection()`, `cos_ctx:*` caching) is now explicitly marked **LEGACY runtime** throughout — it has no counterpart in the model interface and must not be conflated with it. Previous: 2026-07-16 (Executive Briefing day-truth — the canonical execution authority `build_execution_state()` now exposes **`execution_phase`** (via `compute_execution_phase()` in `apps/core/execution/execution_state.py`): a facts-only day-phase (`before_first_commitment / underway / ahead / behind / midday / afternoon / winding_down / day_complete` + `first_commitment`, `minutes_until_first_commitment`, counts) reusing `timing.py` (`earliest_future_commitment` / `completed_ahead_of_schedule`) + canonical `daypart`. Exposed through `decision_authority.execution_facts()` so every surface consumes ONE day-phase fact. The CoS Executive Briefing (`apps/core/cos_briefing/executive_summary.py :: _derive_headline` → `_headline_for_phase`) now GROUNDS the headline in `execution_phase` and no longer infers today's state from the clock/weekly trend. There is exactly ONE headline path: `_derive_headline(exec_state, focus_now)` → `_headline_for_phase` (which returns a single neutral constant `_NEUTRAL_HEADLINE` for the unknown/degraded case). The old `_HEADLINE_MATRIX`/`_time_band` AND the interim `_FALLBACK_HEADLINE` trend-keyed table are fully REMOVED (not bypassed) — no lookup table can reconnect. `_derive_overall_state` is now the WEEKLY trend only (badge scoped "This Week"). Fixes the 4:56 AM "Slow start" fabrication. Previous: 2026-07-11 (Command Center — new **ExecutiveSynthesis** section (`ops_executive.py :: build_executive_summary`, runs LAST in the SAME cycle): a DETERMINISTIC reduction over the assembled telemetry that answers the 5 operator questions — overall status, customer impact, explainable 'why not 100' score, single prioritized action, enriched incident cards (cause/confidence/affected/recovery/root-cause chain via the engine dependency registry), operational narrative, and per-KPI trend direction from a cache history. No new monitors, no AI, no request-path compute. Previous: OPS-2/3/4 — three new Ops Wall telemetry sections + engines: **StorageMon** (`storage`: Postgres size+growth, Redis memory/eviction, disk/volume), **ChatQueueMon** (`chat_queue`: depth/wait/throughput/stuck/starvation via Celery lifecycle signals), **UpstreamHealth** (`upstream_health`: OpenAI availability/latency/consecutive-failures/degradation via a passive `AIService._log_usage` recorder — distinguishes WLJ-healthy from OpenAI-degraded). All background-collected in `build_ops_stream_payload()`; request path reads cache only. Previous: 2026-07-08 (THE CONDUCTOR — Step 2b: promote META to authoritative. Step 2a shadow proved (production) the accountability turn "you let me slide on…" classifies `meta`→repair at HIGH confidence while the live router answered goals (`COS_CLASSIFY_MATCH agree=False`) = a Layer-2 orchestration failure. 2b: in `route_message`, when the Classifier returns `speech_act=meta` + `high` confidence the Conductor OWNS selection and dispatches the repair handler AHEAD of the keyword lanes (after screen/page, before the lane loop); repair yielding nothing FALLS THROUGH unchanged (Return-Contract DECLINED). Emits `COS_CONDUCT` + `COS_CLASSIFY_MATCH`. SCOPE = exactly one speech act; all else stays shadow; medium-meta NOT promoted; the only behavior that changes is the guidance-critique case `plan()` was missing (prior_turn_ref/meta_correction already routed to repair). Fixed family vocab: classifier `expected_owner` for meta = "meta" (matches `owner_family(conversation_repair)=meta`) → correctly-owned repair logs `agree=True`. No reasoning/truth/capability/UX change beyond turn ownership; whether repair fully satisfies accountability is a LAYER-3 question answered by repair's return, not orchestration. Tests: `apps/ai/tests/test_conductor_2b.py`. Step 2c (handler return states + no-repeat) + further promotions NOT started — await production validation. Previous — THE CONDUCTOR — Step 2a: shadow Classifier (record-only). Step 2 of the approved roadmap lifts selection into ONE Classifier so capabilities stop self-selecting; ships as a strangler. STEP 2a (this change): new `apps/ai/chatgpt_cos/classifier.py :: classify(message, has_prior, page_context)` → `Classification{speech_act, expected_owner, confidence, signal}` over the approved precedence ladder (1 screen · 2 meta/repair · 3 continuation · 4 correction · 5 reasoning_mode · 6 retrieval · 7 orientation · 8 general · 9 fallback); shallow/deterministic/DOMAIN-AGNOSTIC with its OWN form-level cues, imports NO capability (closed core), never raises. `route_message` runs it FIRST and LOGS `COS_CLASSIFY` (speech_act/expected/confidence); the current router then answers UNCHANGED; each winner logs `COS_CLASSIFY_MATCH` (expected vs `owner_family(lane)` + agree). ZERO behavior change — the orchestration-investigation instrument: every turn records which capability SHOULD own it, so mis-ownership (e.g. the accountability turn "you let me slide on …" classifies meta→repair while the live router answers goals) surfaces as data. Purity enforced for BOTH conductor.py + classifier.py by `test_conductor_contract.py`. Step 2b (make Classifier authoritative for unambiguous speech acts) NOT started — awaiting shadow agreement data. Tests: `apps/ai/tests/test_classifier.py`. Previous — THE CONDUCTOR — Step 1: unified turn-commit lifecycle (foundation, record-only). Architecture stabilization (approved design + governing contract): production showed the ten Phase-2 capabilities behaving like several assistants taking turns because the router is one ordered chain where selection is positional/keyword-broad AND conversation state is fragmented across three stores and not advanced every turn (stale state leaks forward). Approved fix = The Conductor: ONE component owns each turn; state advances every turn; capabilities ADVERTISE eligibility (appropriate_when/requires/returns/precedence) and the Conductor decides — they no longer self-select. Contract: Conductor owns orchestration ONLY (turn ownership, lifecycle, classification/arbitration, dispatch, shared-state custody, commit, deixis grounding, precedence, narration-invocation, observability) and NEVER intelligence (diagnosis/coaching/planning/retrieval/summarization/judgment → capabilities) or truth (→ WLJ). Guardrails G1–G6 (no domain knowledge, no content generation, no truth-reads-for-meaning, shallow classification, bounded/finite, no invented fallback reasoning). Rollout is one foundational step at a time, production-tested between steps. STEP 1 (this change, record-only, no behavior change): new `apps/ai/chatgpt_cos/conductor.py` :: `commit_turn(conversation, winner, result, user)` runs at the `route_message` choke point after EVERY winning lane (page-reference early return + main loop) and advances one unified `conductor_state` in conversation.metadata (turn, last_act, active_subject_key, last_seen, capped history); emits COS_TURN_COMMIT. No classifier/dispatch/reorder yet. Enforced by `apps/ai/tests/test_conductor_contract.py` (fails CI if conductor.py imports intelligence/domain/truth or composes user text). Step 2 (lift selection into the Classifier) NOT started — awaiting production observation. Tests: `apps/ai/tests/test_conductor.py`. Previous — DAY CONTINUITY — continue the day, don't wake up fresh. Beth replayed "You got about 6 hours of sleep" on "Good morning" at 7/11/3/5:30 — every conversation behaved like the first of the day. Capability (Phase 2, no new engine/learning/migration): new `apps/ai/chatgpt_cos/day_continuity.py` — a LIGHT, EPHEMERAL, per-user, per-LOCAL-DAY ledger in cache (expires local midnight) of what has been established today (`oriented`, `topics`, `last_seen`, `fingerprint`). MATERIAL CHANGE is the primary determinant (not the clock, not a new chat): `compute_fingerprint(sig)` snapshots priority_action/health_critical/today+overdue/accomplishments/foundation/biggest_risk/mission focus (read off the already-computed ExecutiveSignals — request-path safe); `material_changes(prev,cur)` → human deltas; `assess(user)` → orient_full (first) | reorient_delta (changed → concise UPDATED orientation) | continue (unchanged → light "welcome back", any time). Hooks: `_morning_checkin` (greeting) and `compose_orientation` (self-report opener) consult continuity + return `compose_continuation` when returning, and `mark_established` after delivering; the opening check-in→brief passes `skip_continuity=True` so the FIRST orientation is never collapsed. Explicit "brief me" unaffected (never routes through the gated openers → intentional repetition preserved). Daypart/stance (`apps/core/truth/daypart.py`) still drives posture. Tests: `apps/ai/tests/test_day_continuity.py`. Previous — CONVERSATIONAL INTENT EVOLUTION — Status → Diagnosis. A conversation's MISSION can hold while the KIND of reasoning evolves (Status → Diagnosis → Planning → Decision); users never announce the transition. Production failure: "How's my France goal?" (status) → "hard time breaking 289, not falling off like the beginning" got another status summary that drifted to prayer/other goals. Proven producer: `_self_report_lane` reads any subjective statement as a mood → `compose_orientation` whole-life read. Capability (Phase 2, no new engine/learning/migration; ladder architecture, ONLY Status→Diagnosis wired live): new `apps/ai/chatgpt_cos/reasoning_mode.py :: classify_mode` — thin, deterministic, DOMAIN-AGNOSTIC (struggle/plateau/slowdown/unexpected/confusion/lost-motivation cues, never domain words) → STATUS/DIAGNOSIS/PLANNING/DECISION/None; DIAGNOSIS consumed, PLANNING/DECISION SCAFFOLDED (phase-tagged, not routed). New `reasoning/diagnosis.py :: answer_diagnostic` reuses the reasoning lane retrieval (`synthesize_plan`→`retrieve_truth`→`build_working_memory`, borrowing `goal_concerns`/`health_concerns` truth scope) but runs a dedicated DIAGNOSTIC prompt (reason about what CHANGED over the trajectory + one investigative step) — shared REASONING_PROFILES/run_reasoning untouched, no new planner intent. New `lanes._diagnostic_lane` placed BEFORE self_report/goals_checkin/cos_briefing; `_resolve_diagnostic_subject` grounds the subject (named goal → health/weight context → goal framing → prior-turn active topic) and DECLINES when no shift or no groundable subject. Mission persistence unaffected. Tests: `apps/ai/tests/test_intent_evolution.py`. Previous — CONVERSATIONAL MISSION PERSISTENCE + EXECUTIVE THINKING PARTNERSHIP — Beth reset her reasoning after almost every message (chatbot behavior). Once a mission was established ("let's make today lighter") she failed to interpret later messages inside it: "anything I can move that aren't supplements?" → literal supplement search; "mostly it's work stuff you don't know about" → restarted the overwhelm script instead of recognizing the problem moved OUTSIDE WLJ. Capability (Phase 2, no new engine/learning/migration): one rule per turn — interpret the message inside the standing mission until completed/reframed/replaced. `mission`/`mission_scope` ride on conversation_state; `conversation_planner.mission_delta`/`thinking_partner_delta` classify CONTINUE / REFRAME_EXTERNAL / REPLACE (meta-feedback + greeting + explicit WLJ pivot → REPLACE, route normally). New front-of-registry `lanes._mission_lane` claims ONLY active-mission continuation (so a stray word can't hijack it) and declines otherwise — establishment, repair, and normal routing untouched. External-work reframe = Executive Thinking Partner: `_reframe_external_partner` (acknowledge pivot → stop optimizing WLJ → one structuring question, deterministic, no WLJ data) then `_thinking_partner_continue`/`_thinking_partner_answer` (SAME no-personal-data sandbox as the general lane, thinking-partner posture) — capture of work items offered LAST via `_emerged_work_items`, never before the thinking. why_explainer yields to an active thinking_partner thread. Tests: `apps/ai/tests/test_mission_persistence.py`. Previous — REFRESH INTENT + PLAN-AWARE RECOVERY GUARD — production trust fix. In a problem-solving flow, "I updated my stuff, look again" mis-fired: correction-repair (recalled prayer/Bible + replayed an old brief) plus the already-fixed stale recovery line. Root causes: (1) "look again" is a `_CRITIQUE_CUES` entry → `is_critique` → conversation_planner REPAIR; "refresh"/"recheck"/"I updated my tasks" matched nothing. (2) plan-aware `OvertrainingRiskRule` only reframes NEW insights — OLD persisted `Insight.message` rows are re-read verbatim by `cos_intelligence.active_intelligence` → `biggest_risk` → brief (Path A, through route_message) and `cos_briefing/executive_summary._collect_needs_attention`/`_collect_biggest_risk` (Path B, not). (3) repair pulled unrelated context into a workload conversation. Fix (no new engine): `conversation_planner.is_refresh_request` + a REFRESH branch in `plan()` placed BEFORE repair, gated on `state == "problem_solving"` → routes to new `lanes._problem_solving_refresh` (best-effort `ensure_fresh(["tasks"])`, re-reads today's rhythm/execution truth via `_agenda_worth_surfacing` + `_pa_classify`, SHORT "needs you vs can move" read, NO briefing/prayer/France/protein/sleep). `naturalize.recovery_reframe(text, user)` last-mile guard reframes the stale overtraining wording to plan-aware coaching ONLY when `read_training_plan(user).has_recovery_day`; wired at BOTH `route_message` choke points (Path A) AND `executive_summary._collect_needs_attention` (Path B). "look again" outside a problem-solving flow still repairs. Tests: `apps/ai/tests/test_refresh_and_recovery.py`. Previous — QUESTION-SCOPE DISCIPLINE — a named goal returns ONLY that goal. "How am I doing overall towards my 2027 France goal?" briefed the whole portfolio: `reasoning/plan.py::_infer_named_goal_intent` lacked "on track"/"pace" words → defaulted to `goals_progress` (PORTFOLIO curator enumerates every goal). Fix: (1) intent — per-goal progress/trajectory phrasing ("how am I doing / how's it going / progress toward / where do I stand on X") → `goal_on_track`, and the named-goal DEFAULT changed from `goals_progress` → `goal_on_track` (only singular/deictic goals reach this classifier; plural "how are my goals?" still → goals_progress). (2) focal scoping — `preroute_named_goal` now returns `(intent, focal_goal)` (the matched title), threaded via `engine.py` → `synthesize_plan(focal_goal=)` → `stages.build_working_memory` → `stages._scope_to_focal_goal` trims goals working memory to ONLY the named goal (drops the `active_goals` portfolio list) for `_GOAL_SPECIFIC_INTENTS`, and records `other_active_goal_count`. `goal_on_track` profile/fallback talk about ONE goal and MAY offer, in one sentence, to review the rest. Parent/child (mission advanced by health) intact. Tests: `apps/ai/tests/test_goal_question_scope.py`. Previous — EXECUTIVE CONVERSATIONAL DISCIPLINE — orient, then stop talking. The need classifier chose the conversation, but the RESPONSE was still a full briefing: a volunteered self-report ran `_self_report_lane` → `compose_executive_brief` and dumped foundation/France/sleep/protein/due/overdue/etc. New `executive_brief.compose_orientation(user, …)` — SHORT: acknowledge → orient (ahead/steady/lighter) → the SINGLE next thing (next meaningful scheduled item via `_agenda_worth_surfacing`, else the one `priority_action`) → "What do you need from me {part-of-day}?" → STOP (health-critical still leads; night defers to close-out). The two OPENER paths (`_self_report_lane`, `_post_checkin_brief`) now orient; the FULL read stays on the explicit-request paths (`_cos_briefing_lane` "how am I doing today?"/"what do I need to know", and the self-aware repair). `classify_need`: NEGATIVE energy not brushed off ("I'm exhausted") → problem-solving OFFER; "tired but okay" → light orientation; apostrophes stripped so "i'm" matches the lexicons. `_personal_concern_open` tightened to minimal listening ("Okay — tell me what's going on with Haley."). Tests: `apps/ai/tests/test_conversation_posture.py`. Previous — CONVERSATIONAL NEED / POSTURE — choose the conversation, not the script. Beth didn't classify what KIND of conversation an opener was: `classify_subjective_energy` is polarity-only and funnels ANY feeling → `_self_report_lane` → `compose_executive_brief`, so a problem ("I'm behind"), a good day, and a relational worry ("worried about Haley") all collapsed to the same full briefing. New `conversation_planner.classify_need(message)` → `problem_solving` (behind/overwhelmed) | `personal_concern` (worry about a PERSON/life, NOT an executive domain — health/goal/work worries stay on their normal path) | None (unchanged routing). `plan()` consults it right after repair so a clear need OVERRIDES the greeting/brief default. Two posture handlers in `lanes.py`: `_problem_solving_offer` (acknowledge + OFFER to move/simplify/drop, grounded in the real due-today count — not a briefing) and `_personal_concern_open` (`concern_object` extracts the person; listening posture, no protein/workout/mission talk). Introduce executive priorities only when they serve the moment. Tests: `apps/ai/tests/test_conversation_posture.py`. Previous — PLAN-AWARE RECOVERY + finer operational filtering. (1) RECOVERY WITHIN THE PLAN: `OvertrainingRiskRule` (`apps/core/ai_insights/rules_cross_domain.py`) fired "consider a rest day" from `sleep<6.5 AND workouts_7d>=5` with no plan awareness (Insight → `active_intelligence` risks → `interpret().biggest_risk` → brief). New deterministic reader `apps/health/services/training_plan.py :: read_training_plan(user)` (active WorkoutPlan + WorkoutSchedule rotation → has_plan / has_recovery_day / days_per_week / alternates / today_type). The rule now REFRAMES when the workout count matches a structured plan WITH a built-in recovery day: "Protect tonight's sleep to keep your training on plan … the move is better sleep, not another rest day." Only an unstructured plan or volume EXCEEDING it keeps the raw overtraining framing. (2) OPERATIONAL FILTERING: `executive_brief._agenda_worth_surfacing` now also drops OPERATING-RHYTHM tasks ("log nutrition", "weigh in", "track macros") by nature even when stored as a `task`, so cadence never competes with a real commitment. Tests: `apps/core/ai_insights/tests_plan_aware_recovery.py`. Previous — EXECUTIVE FILTERING — say what matters, not everything known. (1) INTERNAL ARTIFACT LEAK KILLED: "…Consolidated from 3 readings into one concern. range 49–55%, average 52%" reached the brief via `cos_briefing/consolidation.py` message → `executive_summary._collect_biggest_risk` fallback → `interpret().biggest_risk` → `executive_brief._priority_story` (verbatim). Fix at SOURCE: `consolidation.py` customer-facing `message` is now natural language only ("{subject} has been below target across the last {span}"); raw range/average stays in `title` for ops; "Consolidated from N readings" removed. `_priority_story` narrates risk naturally ("One thing worth staying on top of today — …", no "derail the day"). `naturalize.py` adds safety-net rules stripping any residual "Consolidated from N readings" / "range X–Y%, average Z%" clause. (2) FILTERING: `executive_brief._agenda_narrative` runs each rhythm item through the priority-tier classifier (`executive_interpretation._pa_classify`) and DROPS routine items (supplement doses, "log X"); `_rhythm_split` refactored to preserve item dicts. (3) JUDGMENT ENDING: leftover-item line is now "One thing still open from earlier is X — worth closing that out today" (never "it's your call"). (4) CONCRETE ACTIONS: `_next_move_story` + `_concrete_today_action` lead with a food ("Start with eggs this morning"), not the nutrient. Tests: `apps/ai/tests/test_executive_filtering.py`. Previous — EXECUTIVE VOICE + GOAL HIERARCHY — natural language, one mission thesis. (1) GOAL HIERARCHY: the primary mission (France 2027) was narrated as a competing priority ("the real leverage is moving France forward") though its milestones are wired to `objective_metric="weight_lb"` — the SAME metric daily health execution moves. `build_goal_state` now exposes `mission["advanced_by"]="health"` (milestone health-metric or health domain, read-only, no migration); `interpret()` adds `_mission_info()` + `ExecutiveSignals.mission` and, when `advanced_by=="health"`, sets `highest_leverage=""` so the mission is NOT emitted as a competing leverage/priority candidate; `executive_brief._mission_thesis` leads the read as the day's through-line ("the everyday work you're putting in — nutrition, training, weight, sleep — is exactly what moves {mission} forward … it IS the mission"). (2) NATURAL VOICE: reworded the reconciliation/thesis/momentum/backlog beats (no "lived experience", "energy/recovery-management day", "backlog", "recovery latitude"); new `apps/ai/chatgpt_cos/naturalize.py :: naturalize()` runs at the `route_message` choke point (sibling of `response_coherence.harmonize`) as a safety net translating any leaked internal vocabulary from deterministic beats OR LLM narration into human words. Tests: `apps/ai/tests/test_voice_and_hierarchy.py`. Previous — EXECUTIVE SYNTHESIS — listen first, incorporate accomplishments, one whole-picture read. A volunteered morning self-report (feeling + accomplishments, NO greeting/question) used to be claimed by `personal_reasoning`→`health_focus_today` (single-domain "prioritize sleep"); `compose_executive_brief` was only reachable via a pending check-in or briefing question. New `self_report` lane (LANE_REGISTRY, before accomplishment/retrieval/reasoning) fires on a genuine subjective report (`classify_subjective_energy`), records subjective + any accomplishment into the one picture, then composes the whole-picture brief; declines questions. `accomplishment.detect()` now robust to natural foundation phrasing ("finishing my prayer", "great start") + new `is_foundation_label()`. `interpret()` SEPARATES foundation (spiritual) from effort (physical): only physical effort earns "recovery latitude"; foundation → new `ExecutiveSignals.foundation`, never flips the day into recovery. `compose_executive_brief` gains `_foundation_story` (leads), a positive_over_debt reconciliation that HOLDS energy AND recovery ("not an energy-management day… still a recovery-management one — protect an earlier bedtime"), and `_next_move_story` tying protein-early to the ACTUAL planned workout (day_truth) with concrete options. `_post_checkin_brief` records foundation so greeting + volunteered paths share the same synthesis beat. Tests: `apps/ai/tests/test_executive_synthesis.py`. Previous — MORNING CHECK-IN TRUST — deterministic day-truth grounding + user-correction repair. New `apps/ai/chatgpt_cos/day_truth.py` (read-only projection over existing providers): `todays_planned_workout(user)` reads the ACTUAL scheduled workout from `get_remaining_rhythm_items` (WorkoutSchedule→CalendarEvent), `protein_options(user)` from DietaryProfile. `reasoning/stages.py` health curator injects both into working memory; `_rank_health_concerns`/`_concrete_today_action`/`_movement_action` defer every movement recommendation to the schedule (never a goal-inferred modality — fixes "strength on a cardio day"); `_HEALTH_GUIDANCE` forbids inferring a workout type and requires concrete protein options. New `apps/ai/chatgpt_cos/correction.py`: `is_factual_correction` (knowledge-challenge / fact-negation / bare "instead of" w/o a first-person decision frame) + `respond` (trust-repair: own the miss → restate corrected truth from the schedule → name the source → corrected plan + protein options, no apology loop). New `correction` lane in LANE_REGISTRY runs BEFORE `decision_support`; `decision_support.detect_decision` gains a decline guard so a correction is never mis-read as change-of-mind ("what are you moving to?"). `accomplishment.detect()` now recognizes completed FOUNDATION work (prayer/Bible reading); `_post_checkin_brief` acknowledges it. Tests: `apps/ai/tests/test_morning_correction_trust.py`. Previous — EXECUTIVE STANCE — daypart reasoning & conversation awareness. New canonical `apps/core/truth/daypart.py`: `phase_of_day`/`executive_stance`/`resolve` map the user's LOCAL hour → phase (morning/midday/evening/night; night wraps midnight) → stance (plan/execute/wind_down/close_out) + a governing POSTURE directive (what to say AND what NOT to say at that hour). ONE source replaces three ad-hoc daypart computations and adds the missing night/bedtime phase. `interpret()` resolves the stance ONCE → `ExecutiveSignals.stance`, shared by every consumer of the one picture. `executive_brief.compose_executive_brief` GATES on `sig.stance`: at NIGHT (close_out) it composes a reflective close-out (`_close_out_story`: look back → let the rest wait → rest; health-critical still leads) instead of a morning execution thesis — fixes the production incoherence where a bedtime "how am I doing?" welded a morning plan to a wind-down tail. `reasoning/stages.run_reasoning` injects the stance POSTURE into the LLM reasoner prompt (every intent respects the hour). `conversation_planner` generalizes the critique cues → principled `refers_to_prior_turn`/`is_meta_conversational` (second-person reference to Beth's message, or a correction of her understanding) → routes "look at the message you gave me"/"that's not what I meant" to the self-aware REPAIR path (priority 6, before decision_support/reconciliation), never a domain lane. Tests: `apps/core/tests/test_daypart_stance.py`, `apps/ai/tests/test_daypart_conversation_awareness.py`. Previous — 2026-07-06: PAGE-AWARE CONTEXTUAL CONVERSATION / CURRENT CONTEXT CONTRACT — the Chief of Staff understands the page you're on and the object in focus, and answers ANY natural question about it. New `apps/core/current_context.py`: `resolve_current_context(user, ref)` (ContentType-based, user-scoped), `NarratableMixin` (mixed into `UserOwnedModel` → `get_context_summary`/`context_ref`/`is_owned_by`; `CONTEXT_FIELDS` hook), `CurrentContextMixin` (view → `<meta name="wlj-context">`), turn-scoped `set_current_focus`/`get_current_focus`. Pipeline: `route_message` runs a `page_reference` lane FIRST (deixis/summary → one grounded LLM answer + page-aware degradation); `resolve_page_focus` resolves the focus by (1) declared `focus_ref` (auto-emitted by base.html for any DetailView of a `UserOwnedModel`, or explicitly by overview pages via `CurrentContextMixin`) → (2) deterministic URL `resolve_focused_object` (any detail/landing URL; server truth) → (3) client content. OWNERSHIP (proven): the reasoning-lane **PLANNER** owns object selection — `reasoning/engine.py :: answer_reasoning_question` reads `get_current_focus()`, `reasoning/stages.py :: run_planner(user, message, focus)` receives the focus IDENTITY ONLY (kind+title, never content) BEFORE planning, so a goal in focus routes "am I making progress?" to a goals intent (not health); retrieval/working-memory/reasoner are unchanged deterministic executors and the reasoner sets `skip_current_context=True` (grounds via working memory, NO prompt-prepend). Shared choke point `services.py :: _ground_current_context` now grounds ONLY the planner-less tool-loop fallback (sandbox general lane + planner/phrasing opt out via `skip_current_context`). Faith resolves the PRODUCTION Journey system (`apps.faith.journey.JourneyDay` via `get_active_journey`/`get_current_day`/`get_day_in_arc`), narrated from scripture refs + verse blocks + context_before + key_insight + reflection; legacy `UserReadingPlan` preserved. Also: `get_openai_client()` WARNS on missing `OPENAI_API_KEY` (never-silent); multi-line `{# #}` template comments forbidden (they leak — regression test enforces). Tests: `apps/core/tests/test_current_context.py`, `apps/ai/tests/test_page_reference.py`. Design: `docs/WLJ_CURRENT_CONTEXT_CONTRACT.md`. Previous — 2026-07-02: SIGNIFICANT EVENT PIPELINE v1 — the Chief-of-Staff reflex: mission-significant events are now recognized and acted on IN THE MOMENT (event-driven), not on the CoS Event Engine's 3-hour schedule. New `apps/ai/significant_events.py`: `SIGNIFICANT_EVENT_TYPES` (purpose.milestone.completed / purpose.goal.completed), `classify_significance()` (mission events → priority 2), and `react_to_significant_event()` — refresh dependent truth (SAE+CoS caches, real-time PIE 'goals' pass) → derive mission progress (e.g. 2/12) → `goal_pace()` next milestone (re-plan) → persist a MAJOR_WIN GuidanceItem with a deterministic what/why/next acknowledgment → run the CoS Event Engine immediately → notify via DNE `deliver_single` (existing channels+policies, wrapped in a savepoint so a dedupe IntegrityError can't poison the caller). Wired onto the existing bus: `objective_weight_milestones.evaluate_weight_milestones()` now emits `purpose.milestone.completed` on the FALSE→TRUE achievement; new subscribers `on_milestone_completed_react`/`on_goal_completed_react` (apps/core/events/subscribers.py) ENQUEUE `apps.ai.tasks.react_to_significant_event_task` (request path stays fast; 3-hour engine remains the backstop). The win is STICKY: `cos_event_engine._WIN_PREFIX` (`cos_event:win:`) is exempt from re-detection auto-resolve — a milestone you reached is true forever — and surfaces in Beth's standing read via `recent_cos_events`. Origin: France 2027 Family 18K Mission weight milestone (283.1 vs 284.9, achieved 2 days late) went unrecognized until the scheduler. NO new model, NO migration. Regression `apps/ai/tests/test_significant_event_pipeline.py` (7); design `docs/SIGNIFICANT_EVENT_PIPELINE.md`; phased follow-on = threshold-significant events + emit-nothing domains (medical/labs first). Previous: P31 EXECUTIVE CONVERSATION PLANNING (Phase 1) — a thin DETERMINISTIC conversation strategy/state layer in front of the lane pipeline; Beth plans the CONVERSATION, not just the answer. New `apps/ai/chatgpt_cos/conversation_planner.py`: `plan(user, conversation, message)` + a conversational-act classifier (greeting/critique/question) + first-class conversation STATE persisted in `AssistantConversation.metadata` (the proven `pending_clarification` pattern — NO migration, NO LLM director). New `conversation_planner` lane (registry position 2, after `clarification_reply`) + `_morning_checkin`/`_post_checkin_brief`/`_repair_response` in lanes.py. Behavior: a GREETING opens with a light CHECK-IN first (greet by name, 1–2 overnight facts e.g. sleep, ask how he's feeling, HOLD the agenda — product decision: morning always checks in first); a CHECK-IN response hands off to the time-aware deterministic BRIEFING (`build_daily_agenda`, adaptive lighter lead on a negative feeling); a CRITIQUE of Beth's prior answer ("does that sound right?") triggers REPAIR (acknowledge + re-ground in the actual time-aware state, never an unrelated fact, offer to correct). All deterministic (OpenAI-disabled safe). The Acceptance harness resets planner state per question (`run_one`) so the isolated-intent bank never leaks state; multi-turn dialogue is covered by `apps/ai/tests/test_p31_conversation_planning.py`. Restore point: tag `beth-stable-v3`. Design: `docs/BETH_CONVERSATION_PLANNING_DESIGN.md`. Previous: P29 FINAL DEEP + MORNING PATH — (DC#1 intent precedence) after goal identity resolves, goal intent precedence now beats generic routes: `_infer_named_goal_intent` reordered/extended so "behind"→goal_on_track (not goal_concerns), "biggest THREAT"→goal_failure_modes (not biggest_goal_risk, checked before it), and "work on/next move/highest leverage"→goals_focus_today; the `next_rhythm` lane yields on `_GOAL_GROUNDED_MARKERS` (this/that/the/our/my mission/goal, milestone) so "what comes next in this mission" reaches Goals; `foundational_facts` declines PROGRESSION questions ("what's after Goal Weight 284.9?") so a milestone-name containing "weight" is not answered as a current-weight fact; `_GOAL_FRAMING` gains day-action cues so "what should I do today for France?" resolves via the France token. (DC#2 morning path) The live chat path == the Acceptance path (both `ChatGPTCoSService.generate` → `route_message`); "Good morning" failed only because no deterministic lane covered it. `_cos_briefing_lane` now claims greetings/morning entries ("good morning", "morning", "start my day", "how is my day looking", time-constrained "if I only have 30 minutes…") via `_is_greeting` + signals → deterministic `build_daily_agenda`, never the tool loop. (DC#3 coverage) Added the morning CoS scenario to the Deep bank (`_checkin_questions`: Good morning / how is my day looking / what could derail me today / 30-minutes). (Prompt-gen) `FAILURE_CLASSIFICATION` gains `production-path divergence` + `acceptance coverage gap` for when a real user report contradicts a GREEN run. New regression `apps/ai/tests/test_p29_morning_and_precedence.py`. No migration; health byte-identical. Previous: P27 DEEP CONVERGENCE — goal IDENTITY before intent wording. (DC#1) The goal pre-router (`apps/ai/chatgpt_cos/reasoning/plan.py::named_goal_intent`/`preroute_named_goal`) is now a first-class goal-IDENTITY resolver: beyond deictic + FULL-title matching it resolves a goal by a DISTINCTIVE TITLE TOKEN (`_distinctive_title_tokens` — alphabetic ≥4-char title words minus `_TITLE_STOPWORDS`/domain-collision words; "France" from "France 2027 Family 18K Mission") gated by goal FRAMING (`_has_goal_framing` — progress/status/forward/threat/behind/milestone/…), so "how is France going?", "my France goal", "the mission status", "biggest threat to the mission" all resolve to GOALS while a general mention ("the capital of France") is NOT stolen. Added "the mission"/"our mission" referents; deliberately did NOT add "my/the goal" (collides with "my goal weight" = health and shadows richer milestone phrasing). (DC#2) `_HEALTH_INTENT_SIGNALS` biggest_health_risk gains "biggest/main/top health concern" so "what's my biggest health concern?" reaches the deterministic biggest_health_risk fallback instead of the tool loop. (Acceptance Center) New DETERMINISTIC CAPABILITY GAP classification in `apps/ai/chatgpt_cos/acceptance_service.py`: an outage message in a domain WLJ OWNS the truth for (goals/health) is a capability gap (deterministic path didn't reach existing truth) — layer `deterministic_truth`, NOT infrastructure — whereas a general-suite outage stays infrastructure. Review prompts now force a 5-way `primary classification` (infrastructure | deterministic capability gap | semantic routing | prompt quality | acceptance contract). New regression `apps/ai/tests/test_p27_convergence.py`. No migration; health byte-identical. Previous: P26 DEEP-SUITE HARDENING — three systemic defect classes. DC#1 deterministic degradation: new `apps/ai/chatgpt_cos/lanes.py::_cos_briefing_lane` (registered between `next_rhythm` and `personal_reasoning`) answers holistic CoS requests — "what needs my attention", "help me plan the rest of the day", "wrap up my day", "what should I know today" — from the deterministic `build_daily_agenda` (rhythm/exec summary), ALWAYS non-empty, NO OpenAI; plus widened `plan.py::_HEALTH_INTENT_SIGNALS` overall_progress (health summary / my diabetes / a1c / my health status) so health questions reach a deterministic fallback when the planner is down. DC#2 goal semantic breadth: `plan.py::_foundational_goal_intent` now maps failure-analysis paraphrases ("watch out for", "could derail", "what would stop") → goal_failure_modes and move-the-needle paraphrases ("move this forward", "highest leverage action") → goals_focus_today, health-context-gated. DC#3 personal/external boundary: new `foundational_facts.py::external_general_signal` — an EXTERNAL/definitional question ("what is a healthy weight generally?", "what is normal blood pressure?") with NO personal grounding suppresses personal retrieval (classify_foundational_fact → None) AND `lanes.py::_looks_general` claims it for the general lane, so it never returns "your current weight is…". New regression `apps/ai/tests/test_p26_deep_hardening.py` (OpenAI-disabled CoS suite + goal paraphrase suite + boundary suite, validating ACTUAL rendered responses). Health byte-identical (signals additive); no migration. Previous: 2026-06-26 Goals reasoning — NAMED-GOAL PRE-ROUTER + RICH GOAL CONTEXT. (1) New deterministic goal pre-router `apps/ai/chatgpt_cos/reasoning/plan.py::named_goal_intent()` (pure) + `preroute_named_goal(user, message)`, wired into `reasoning/engine.py::answer_reasoning_question` BEFORE `run_planner`: a question that names an active goal/mission — or uses "my mission"/"this goal"/"that goal" — is forced to the GOALS domain so the health planner can no longer STEAL it (root cause #1). Reads canonical titles READ-ONLY via `get_domain_state(user,'purpose')` (snapshot, no request-path recompute); goal-context-gated + title length-gated (≥4) + bare-domain-collision-word guard so it never captures unrelated (e.g. health) questions. Health routing stays byte-identical (the `deterministic_intent` resilience path is untouched; pre-router only fires when goal context exists). (2) `apps/core/ai_state/state_builder.py::build_goal_state` now attaches a `context` block to each active-goal entry AND the mission, built from EXISTING canonical `LifeGoal`/`GoalMilestone` fields ONLY (NO migration): `active_milestone`(+detail), `next_milestones`, `recently_completed_milestone`, `current_phase`, `why_it_matters`, `success_definition` (root cause #2 — goals reasoning previously ignored this). `_evidence_from_snapshot` recommendation is grounded in active-milestone → momentum → risk-drivers, with a generic-planning GUARD (a goal WITH milestones never receives "plan the goal"/"outline next steps"/"take one step"/"make progress"; Fix #3). (3) `reasoning/stages.py::goals_working_memory` curator (`_coach_evidence`) surfaces the rich context to the reasoner + a `_GOAL_GUIDANCE` instruction to ground the next step in the active milestone — executive-clean (no IDs/enums/scores/field names/source paths). +14 deterministic tests in `apps/ai/tests/test_goals_reasoning.py` (66 pass); no migration. NOTE: 2 PRE-EXISTING failures in `apps/ai/tests/test_health_retrieval.py` (`_handle_sleep_query`/`_handle_last_workout_query` now return a friendly no-data message vs None) are unrelated — confirmed identical on baseline via `git stash`. Previous: 2026-06-24 (ChatGPT CoS transition Phase 7 — CHAT PIPELINE CHANGE (flag-gated): the evidence-tool loop now runs in the SERVER-OWNED background path so it survives navigation. `apps/ai/personal_assistant.py::_generate_response_stream` got a flag-gated branch — when `WLJ_COS_EVIDENCE_TOOLS_ENABLED`, it runs `_call_api_with_tools` inside the generator that `run_chat_generation` (Celery) owns + relays via `chat_stream_bus`/resume, emitting the final answer as one chunk; flag OFF keeps the token-streaming path byte-for-byte unchanged. tasks.py/bus/resume/multi-tab machinery REUSED untouched. Plus a feature-flagged distinct ChatGPT CoS header (`feature_flags.WLJ_COS_EVIDENCE_TOOLS_ENABLED` via `context_processors.feature_flags`; `assistant_panel.html` `--chatgpt` modifier + "AI" badge + "Powered by ChatGPT" sublabel; `assistant-panel.css` deep-blue→teal gradient + gold accent, CSP-safe). ALSO fixed a pre-existing faith bug: `handle_log_prayer` no longer passes the non-existent `prayer_status` kwarg to `PrayerRequest`. Previous: ChatGPT CoS transition Phase 6 — `execute_action` tool ENABLED (first WRITE tool) via new `apps/ai/cos_services/action_execution.py::execute_action(user, action, params)`, which routes EVERY write through the EXISTING single path: builds `IntentResult(intent_type=action, parameters=params)` → `IntentService.execute_intent()` → UAIO → the 54 existing handlers (fail-closed Learning-Mode gate preserved). NO new write path. Strict Day-1 allowlist (13 create/log/complete intent_types). Confirmation REUSES `apps/core/ai_orchestrator/action_policy.ACTION_POLICY` (category+risk) with the Phase-6 CoS threshold: DESTRUCTIVE or HIGH/CRITICAL → `confirmation_required` (re-call with confirmed=true); routine actions execute. JSON-safe envelope (success/failed/denied/confirmation_required/error); never raises; `COS_ACTION` telemetry. All 5 CoS tools now advertised. NOTE: validation surfaced a PRE-EXISTING bug — `action_handlers.handle_log_prayer` passes a non-existent `prayer_status` kwarg to `PrayerRequest` (TypeError); flagged separately, not a Phase-6 defect. Previous: ChatGPT CoS transition Phase 5 — `search_history` tool ENABLED via new `apps/ai/cos_services/history_search.py::search_history(user, query, domain=None, timeframe=None)`, which EXPOSES the previously-unused `apps/ai/search_service.py::SearchService` (keyword search over journal/health/goals/faith/organize[life]/finance/capture + search_all) and `apps/notes/services.py::search_notes_cos`. No new search engine/embeddings/index. Timeframe parsed deterministically + applied uniformly on each result's `date`. Honest statuses (ready/empty/unsupported_domain/error); `SEARCH_HISTORY` telemetry. Advertised CoS tools now 4 (standing_context, domain_state, decision, search_history); only execute_action remains disabled (P6). Previous: ChatGPT CoS transition Phase 4 — `get_decision` tool ENABLED in `apps/ai/cos_services/tool_registry.py::_h_decision`, reusing the EXACT `CosDecisionView` pipeline (`cos_mode_router.normalize_mode` → `execution_state.build_execution_state` → `selectors.select`) for execution/risk/fix — NO new decision logic, no LLM-generated decisions. New DEV harness `apps/ai/management/commands/validate_cos_tools.py` runs the real tool loop end-to-end (requires OPENAI_API_KEY; fails closed without). Deterministic half validated against real services; real-model link pending live run. Previous: ChatGPT CoS transition Phase 3 — CHAT PIPELINE CHANGE (flag-gated, default OFF): the answering LLM can now call read-only CoS evidence tools mid-answer via a NEW bounded tool loop `apps/ai/services.py :: _call_api_with_tools()` (existing `_call_api`/`_call_api_stream` untouched). New `apps/ai/cos_services/tool_registry.py` (OpenAI tool schemas bound to existing services; ENABLED: get_standing_context, get_domain_state; registered-DISABLED: get_decision/search_history/execute_action for P4/5/6) + `tool_dispatcher.py` (deterministic routing only, JSON-safe envelope, `COS_TOOL` telemetry, never raises). Wired into `personal_assistant.py::_generate_response` behind `WLJ_COS_EVIDENCE_TOOLS_ENABLED` (default False — no production change); vision turns and streaming bypass the loop (streaming evidence-tools = Phase 7). On any tool-loop error it falls back to a plain `_call_api` completion. Single orchestration path preserved — NO parallel orchestrator. Previous: ChatGPT CoS transition Phase 2 — new `apps/ai/cos_services/domain_state.py :: get_domain_state(user, domain, allow_build=False)`: ONE generic, registry-driven ChatGPT read surface for all WLJ domains. Delegates to the canonical SAE `get_module_state()` over `MODULE_BUILDERS` — no domain-specific readers, no re-aggregation (Law 9). `DOMAIN_REGISTRY` is an EXPOSURE map (ChatGPT domain → SAE module; e.g. life→tasks, purpose→goals) layered on top of SAE; the core `_canonical_module` alias map is UNTOUCHED. State-first/read-only on the request path (allow_rebuild=False → `pending` on cold snapshot); honest statuses (ready/pending/no_state_source[e.g. notes]/unsupported_domain/error); JSON-safe; `DOMAIN_STATE` telemetry. Shared JSON helpers extracted to `apps/ai/cos_services/serialization.py` (StandingContextService now imports them). No SAE/model change. Previous: ChatGPT CoS transition Phase 1 — extracted `build_executive_from_context(context)` from `build_executive_context()` in `apps/core/ai_orchestrator/cos_context.py` as a PURE projection over an existing context dict, so the new `apps/ai/cos_services/standing_context.py :: get_standing_context()` can derive the executive summary from the cached `cos_context` WITHOUT a rebuild. `build_executive_context(user)` behavior is unchanged (it now calls the extracted helper). New StandingContextService is the always-loaded ChatGPT CoS context: read-only, cache-first via `apps/ai/readiness_cache.get_cached_cos_context` (returns a `pending` shell on miss, NEVER live-computes on the request path), reuses existing providers only — no new intelligence/engines. WLJ remains the deterministic source of truth; ChatGPT becomes the reasoning layer. Previous: 2026-06-15 — CoS Coherence Guards Phase 2 — count + entity guards now FAIL-CLOSED by default; calorie two-truths fixed at source via NutritionCalorieTrendRule 7-day relabel. See "CoS Coherence Guards" under Truth Layer Architecture. Previous: Phase 1 observe-only count/entity/calorie detectors. Earlier: Nutrition Answer-First Response Contract — follow-up to the routing-precedence fix below: routing and state were now correct, but the RESPONSE SHAPE was wrong — "How am I doing on protein today?" returned the full CoS decision template (a "Priority note: sleep" cross-domain overlay + "Macro compliance 23/100 — well off target" interpretation + an Action coaching line) when the user asked one narrow factual question. Defect class: correct routing + correct nutrition state + wrong response assembly. Root cause in `apps/ai/deterministic_router.py :: _handle_nutrition_query` — it unconditionally rendered `_format_decision_response(situation, interpretation, action, trust, priority_note=_get_high_priority_note(user, exclude_domain='nutrition'))`; the decision/coaching template + an always-on cross-domain priority note is the wrong UX contract for a direct factual status question. Fix (Problem 1 only — answer-first): new `_nutrition_fact_response(nut, msg_lower)` returns grounded totals ONLY (the asked nutrient — protein/calorie detected from the message; generic phrasing reports both — plus a neutral over/under target delta when a target is grounded; no priority note, no macro-score verdict, no Interpretation/Action, no cross-domain content); new `_is_nutrition_coaching_request(msg_lower)` makes fact mode the DEFAULT and reserves the Situation/Interpretation/Action template for explicit guidance asks ("what should I do about my macros?"); `_handle_nutrition_query(user, msg_lower=None)` gains an early return after the confidence guard (factual → fact response, or None → fall through, never to the coaching template). Phase 0a.2 call site passes `msg_lower`; the Phase 1 data-route dispatcher auto-passes it via signature inspection; the validator-regenerate path (`msg_lower=None`) defaults to answer-first. Before→after for "How am I doing on protein today?": a 4-section decision block with a sleep priority note and macro-compliance coaching → "You're at 136g protein today. / Target: 180g. / That's 44g under target." Scope held to Problem 1 — protein target source, NutritionGoals, macro-compliance formula, calorie-burn grounding, exercise attribution, and workout timezone are UNTOUCHED (separate PRs). +6 tests in `apps/ai/tests/test_nutrition_trust.py` (`TestNutritionAnswerFirstContract`); 19/19 nutrition-trust tests pass; pre-existing router failures identical on baseline via `git stash` — zero new failures. No model/schema/migration changes. Previous: Nutrition Status Routing Precedence Fix — regression follow-up to the Beth-Trust fix below: food estimates correctly bypassed coaching, but "How am I doing on protein today?" routed into EXECUTION COACHING ("Go straight into Bike Ride…") instead of a nutrition status answer. Root cause traced through `classify_and_route` in `apps/ai/deterministic_router.py`: the nutrition data route lives in Phase 1 (`_try_deterministic_data_routes`), but the decision/focus routers run EARLIER — Phase 11.1 `_try_decision_query_route` and Phase 4 `_try_focus_query_route`. "How am I doing…" contains `'how am i doing'` ∈ `_FOCUS_QUERY_PHRASES` → `_is_focus_query` → `_is_decision_query` → Phase 11.1 fired and returned coaching BEFORE Phase 1 nutrition ever ran. Secondary defect: `_match_nutrition_query` only knew a hand-curated phrase list, so `'protein today'`/`'nutrition today'`/`'macro compliance'`/`'how am I doing on protein…'` matched NOTHING. Two-part fix: (1) new Phase 0a.2 HARD nutrition-status route inserted in `classify_and_route` BEFORE Phase 11.1 — a nutrition status query is now answered deterministically and is no longer preemptable by decision/focus/execution routing; food-estimate/logging phrasing is excluded inside the matcher so "I had 8 oysters…" still falls through, and a None handler return (no data / confidence-guard refusal) falls through to the normal pipeline unchanged; (2) `_match_nutrition_query` rewritten two-tier — Tier 1 keeps the exact-phrase anchors, Tier 2 is compositional: a nutrient keyword (protein/calorie(s)/macro(s)/carb(s)/carbohydrate/nutrition/fiber) + a status anchor (today/how am i doing/how are my/how much/status/compliance/this week/have i had/where am i/…), with logging verbs and burn/burned/burnt (exercise calories) excluded. Verified routing: "How am I doing on protein today?", "How are my calories today?", "Protein today", "Calories today", "Nutrition today", "Macro compliance", "How much protein have I had?" → all `route_name='nutrition_query'`; "I had 8 oysters…", "I ate 2 eggs", "how many calories did I burn today", "log my protein for today", "what's left today", "other than nutrition, anything left?" → correctly NOT. +4 tests in `apps/ai/tests/test_nutrition_trust.py` (13/13 pass); pre-existing router failures identical on baseline. No model/schema/migration changes. Previous: Nutrition Beth-Trust Fix — "I had 8 oysters, how much protein?" returned a confident 0 cal / 0 protein while the dashboard showed 1418 cal / 136g. Three nutrition-only defects in the deterministic CoS path, fixed scope-controlled: (FIX 1 routing) `_match_nutrition_query()` in `apps/ai/deterministic_router.py` matched the `'how much protein'` substring INSIDE a food-estimate sentence, hijacking the terminal nutrition STATUS route before the log/estimate path could run — new `_is_food_estimate_query()` guard returns False (suppressing the status route) for consumption phrasing ("I had/ate X", "I just had a …") and `<macro> in/of <food>` estimates so they fall through to log_food/estimate; (FIX 2 grounding) `build_nutrition_state()` in `apps/core/ai_state/state_builder.py` computed "today" from the SERVER tz (`get_current_time().date()`) while the dashboard uses USER tz (`get_user_today`) — near midnight they disagree on which day's intake to show; now `today = get_user_today(user)` and `cutoff_7d` derives from it, so Beth and dashboard share one definition of "today"; (FIX 3 freshness+confidence) `_handle_nutrition_query()` read an unguarded SAE snapshot — now calls `ensure_fresh(user, ["nutrition"])` before reading (same pattern as the `dashboard_v3/services/composer.py` reader) AND refuses a confident "0 calories today" when the snapshot is contradictory (`cal==0` but `food_entries_today>0`) or suspicious (zero cal, no today-count, but weekly entries exist) → returns None / falls through rather than asserting a falsehood. Legitimate "nothing logged yet today" (`food_entries_today==0`) still answers truthfully. Hard invariant under test: after logging food, dashboard total == Beth/SAE total. 9 new tests in `apps/ai/tests/test_nutrition_trust.py`. EXPLICITLY DEFERRED (separate validated Phase B PR): promoting freshness into `get_module_state()` for ALL manual-entry modules — unknown blast radius; this PR's `ensure_fresh` is nutrition-scoped and reversible. No model/intent/schema/migration changes. Previous: Skip-Command Router Interception Fix — follow-up to the routine-skip work below: in production "skip shower" still didn't skip, returning `"No — just shower left."`. Root cause: `'skip '` is a `QUALIFIED_STATUS_PREFIXES` member in `apps/ai/deterministic_router.py` (built for "skip workout, am I done?"), and `is_qualified_status_query()` matched the prefix with no requirement of a status-question closer — so the bare imperative "skip shower" matched the terminal Phase 0a.1 qualified-status route in `classify_and_route`, which runs BEFORE intent recognition. `personal_assistant.send_message` set the response and never reached `handle_skip_routine`. NOT a check-in/end-of-day context issue (the check-in prefilter excludes qualified-status queries). Fix: `is_qualified_status_query` now requires a status closer (`?`, a yes/no status question, or a distinctive remaining-fragment) for the imperative-exclusion prefixes that collide with real commands (`_IMPERATIVE_COMMAND_PREFIXES` = skip/skipping/forget/ignore/leave out/take away). Prepositional prefixes (other than/besides) stay permissive. Net: "skip workout, am I done?" still answers the status question; "skip shower" falls through to intent recognition → skip_routine fires. New helpers `_has_status_closer` + `_STATUS_CLOSER_PHRASES`. 2 new router regression tests in `apps/ai/tests/test_skip_routine.py`. No model changes. Previous: Natural-Language Routine Skip + Skip-Honoring Check-ins — closes the "Beth acknowledged a skip but ignored it" trust gap. Two proven defects fixed: (#1) no NL path existed to skip a routine — `skip_task` is Task-only and the `skip_routine()` helper (`apps/life/services/routine_helpers.py`) was button-only (needed a `schedule_id`), so when the user said "I'm not showering today" the LLM improvised an acknowledgment with NO state mutation; (#2) `today_engine` ignored `RoutineLog.log_status='skipped'`, so even a button-skipped routine still surfaced in check-ins. Fix #1 — new `skip_routine` CoS intent registered across all 8 points: tool def `apps/ai/intents/life_intents.py`, handler map `apps/ai/intents/__init__.py`, `LIFE_INTENTS` set `intent_engine.py`, dispatcher elif `intent_service.py::execute_intent`, `ACTION_POLICY` entry `action_policy.py` (MUTATE/MEDIUM/CONFIRM), system-prompt examples + explicit SKIP-vs-DEFER boundary `intent_service.py::_build_intent_system_prompt`, `NON_TIME_INTENTS` `test_intent_registration.py`, and handler `apps/ai/action_handlers.py::handle_skip_routine`. The handler resolves today's matching `RoutineSchedule` (mirrors reschedule's applies_to_day/specific_date logic), returns honest `item_not_found`/`multiple_matches` when it cannot, calls the EXISTING deterministic `skip_routine()` helper, invalidates CoS cache + emits TASK_UPDATED — confirmation is returned ONLY after the mutation persists (capability-honesty: never "I'll skip it" without a real skipped RoutineLog). Fix #2 — `apps/core/today/today_engine.py::_collect_routine_items` now drops items whose `status=='skipped'` so they never appear in overdue/coming_up/later or inflate the day's total; completion truth/streaks read `RoutineLog` directly so adherence is unaffected. Blast radius verified: streaks/health/drift/compliance read RoutineLog (not today_engine buckets); morning_reconciliation already counts skipped as resolved; next-day reset is natural (RoutineLog is per scheduled_date). 8 new regression tests `apps/ai/tests/test_skip_routine.py` (NL skip writes skipped log + succeeds, unknown routine fails honestly, skipped hidden from check-in via NL + button paths, collector drops only skipped, tomorrow resets). No model changes, no migration. Previous: SAE Manual-Entry Freshness Guard — new `apps/core/ai_state/state_freshness.py :: ensure_fresh(user, modules)` closes the "stale mission signal" trust gap where a journal entry or nutrition log made minutes ago still showed `Journal → 0/wk` / `Nutrition → 0%` until the nightly rebuild. Root cause: the SAE snapshot (`UserState.state_data`) is updated by a fire-and-forget Celery `deferred_sae_refresh` whose sync fallback only triggers when `.delay()` *raises* (broker unreachable) — not when the worker is merely down/backed up, so the broker accepts the task and the snapshot silently lags. The guard is a request-path freshness check wired into `dashboard_v3/services/composer.py :: _read_mission_states()` BEFORE the `get_module_state` reads: for manual-entry modules ONLY (`_MANUAL_MODULE_SOURCES` registry — currently `journal`→JournalEntry, `nutrition`→FoodEntry, keyed by module with phased promotion lines for medicine/habits/checkins), it runs a cheap indexed `Model.objects.filter(user=user, updated_at__gt=UserState.last_updated).exists()` existence check and, only when stale, calls `update_user_state(user, module)` to rebuild that ONE signal via its builder, then clears any per-request `user._sae_cache`. Idempotent/self-healing (the rebuild bumps `last_updated` past the raw write so it won't re-fire) and resilient to Celery being down. Critically preserves `raw → signal → narrative`: it rebuilds the SIGNAL via the builder and reads the signal — it never reads raw rows into narrative logic. Heavy modules (health = ~69 queries, fitness) are deliberately EXCLUDED from the guard to honor the no-heavy-compute-on-request-path rule; they stay background-only. 6 new tests in `apps/core/ai_state/tests_state_freshness.py`; 69 MissionCardTests still pass. No model changes, no migration. Previous: Mission Framework Phase 2B + safe 2C foundation — Mission hero card: `_build_mission_card` (dashboard_v3 composer) now also returns `icon`, `title` (display, leading emoji stripped), `is_primary`, `progress`, and `why`. Icon via `_resolve_mission_icon()` — explicit `LifeGoal.mission_icon` metadata → user-typed leading emoji (`_LEADING_EMOJI_RE`, Unicode-codepoint only, NEVER word inference) → None. New `_build_mission_progress()` = deterministic milestone count (completed/total/filled 0–100) for the SVG hero ring; this is a literal count, NOT readiness. `templates/dashboard_v3/sections/mission.html` rebuilt as a premium hero card with a CSP-safe SVG ring (`pathLength`/`stroke-dasharray` presentation attributes, no inline JS/style); CSS in `static/dashboard_v3/css/dashboard_v3.css`. Safe 2C groundwork: `mission_icon` field (migration 0014, additive) + the `_build_mission_progress` primitive; readiness %, key drivers, watch items, and generated summaries remain DEFERRED and uncomputed (every visible claim must be explainable). NO change to selection, the shared selector, or CoS mission narration — dashboard mission == CoS mission still holds. Previous: Mission Framework Phase 2A — Primary Mission selection: mission selection is now EXPLICIT user intent. New `LifeGoal.is_primary_mission` BooleanField; `select_active_mission_goal()` returns `is_primary_mission=True AND status='active'` with NO derived fallback (removed the foundational/deadline/momentum ranking). One active Primary Mission per user, enforced at the app layer (`make_primary_mission()` atomic unset→set) and DB layer (partial `UniqueConstraint`). Toggle via `GoalPrimaryMissionToggleView` + "⭐ Make this my Primary Mission" control on goal_detail; flag is NOT in the create/edit form fields and NOT AI-settable (deliberate). When no Primary Mission is selected the dashboard card hides and the CoS stays silent (existing None-guards). Both dashboard_v3 composer and `build_goal_state()` consume the one selector → no divergence. Previous: Mission Framework Phase 1B — Beth Mission Awareness: new `apps/purpose/mission_selection.py :: select_active_mission_goal(user)` is the single source of truth for picking the headline foundational LifeGoal (active + foundational, ranked has-milestones → future date → long-horizon ≥90d → momentum score → id). Both the dashboard_v3 composer (`_build_mission_card`) and the SAE goal-state builder consume it, guaranteeing dashboard mission == Beth mission. `build_goal_state()` gains an additive `mission` key (title/current_focus/momentum_trend/days_remaining) composed deterministically from the shared selector; read-only — nightly `GoalMomentumSnapshot` only, no request-path compute. `_build_purpose_context()` passes the mission through and attaches a fixed-mapping `coach_line` via `_MISSION_COACH_LINE` (rising/stable/falling; None → omit), CONTEXTUAL tier — no readiness, no %, no completion language, NOT LLM-generated. No new engine, no MissionBriefing, no schema change. Tests: 3 new in TestGoalStateBuilder + 4 in new `apps/core/ai_orchestrator/tests/test_mission_context.py`. Previous: 2026-05-14 Action Center Chronological Timeline: new `build_chronological_timeline()` in `apps/core/decision_engine/action_prioritizer.py` is a thin wrapper over `build_grouped_action_center` that adds chronological sort, recovery annotations, and per-item emphasis metadata. Action Center becomes a daily execution timeline — time controls vertical ordering; urgency/foundationality/expiry/recovery surface as emphasis (color, ring, badge, dimming) via `_compute_emphasis()`. `RECOVERY_BANNER_COPY` is a deterministic table mapping `recovery_state.mode` to banner text + severity; no LLM in the path. `DashboardV2Service.get_execution_context` merges timeline data into `ac` alongside `phase_groups` so both rendering paths get full data. New partials: `_action_timeline_block.html` (one time block, chronological), `_action_recovery_banner.html` (NORMAL = no banner; RECOVERY/STABILIZE/SHUTDOWN show their canonical line). `templates/dashboard_v2/partials/action_center.html` branches on `ac.timeline_version == 'v2_chronological'` AND `feature_flags.WLJ_ACTION_CENTER_CHRONOLOGICAL`; legacy phase_groups path preserved behind the flag. New `feature_flags` context processor exposes `WLJ_ACTION_CENTER_CHRONOLOGICAL` to templates. Within-block item sort: effective_time, then foundational desc, then title. Cross-block sort: effective_time only (strict). 45 new tests; full focused sweep passes. Feature flag defaults True; legacy path removable after 30-day burn-in. X4 (reschedule visibility — original vs rescheduled time) is deferred. Previous: 2026-05-10 Narration Contract: new `apps/core/ai_orchestrator/narration_contract.py` defines four trust tiers — `canonical_item_truth`, `rollup_summary`, `advisory`, `contextual`. Every section appended in `format_cos_system_injection` (cos_context.py:5833+) carries an explicit `[TIER:...]` header; untagged sections default to `contextual` per the preamble. New DECISIONS section injected after locked facts as canonical_item_truth, carrying selector outputs from `build_execution_state` (next_action / biggest_risk / fix_priority / recovery_state / day_narrative). ACTION PRIORITIES now filters to `eligible_actions` (block-gated); far-future items moved to a new FORWARD SCHEDULE section tagged `contextual`. NEXT UP suppressed when target time is beyond `AT_RISK_HORIZON_MINUTES`. Medication narration collapsed to a single source (exec_summaries) with the dual `_fresh_module_state` rollup narration removed from the prompt — fresh state remains in context for entity grounding only. New `apps/ai/narration_contract_validator.py` performs soft post-response validation: detects "done / overdue / at risk / next action / fix first" claims and grades each by traceability to canonical_item_truth content. New `apps/core/ai_orchestrator/contradiction_telemetry.py` emits `ROLLUP_CONTRADICTION_WARNING` when domain rollup says DONE while a child item is still actionable, or when a medication window's all_taken disagrees with fresh per-dose state. New flag-gated `apps/ai/observability/chat_snapshot.py` writes a single JSON artifact per request to `LOG_DIR/chat_snapshots/<date>/<request_id>.json` (controlled by `WLJ_CHAT_SNAPSHOTS_ENABLED`). All three enforcement layers wired into `personal_assistant.send_message` post-response, after locked-facts truth validation. Architecture Laws bumped to 1.2 with Law 16 (Narration Contract). 36 new tests; full focused test sweep passes. No DB migrations. Previous: 2026-05-03 CoS Recovery Contract: deterministic task classification + recoverability + recovery-state machine + block-collapse selection gate. New modules `apps/core/execution/constants.py`, `apps/core/execution/task_classifier.py`, `apps/core/execution/recoverability.py`, `apps/core/execution/recovery_state.py`. `apps/core/execution/today_execution.py` annotates every ExecutionItem with `task_class ∈ {HARD_EXPIRED, WINDOWED, SOFT_EXPIRED, FLEXIBLE}`, `recovery_grace_minutes`, `is_reset_action` — additive, idempotent. WINDOWED hard cutoff = `min(scheduled + grace, next_anchor_block_start)` so morning anchors cannot drift into afternoon recommendations. `apps/core/decision_engine/action_prioritizer.py` adds three pure helpers consumed by `build_execution_state`: `compute_block_collapses` (strategy ∈ `recover_partially | skip | defer`; foundational AND reset levers stay alive in `recover_partially`); `compute_at_risk` (60–90 min standard horizon, 4 h only with dependency chain via `blocked_dependents`, suppressed entirely when overdue exists and no dependency); `apply_recovery_bucket_selection` (NORMAL pass-through; STABILIZE pushes reset first; RECOVERY orders reset → foundational overdue → quick win → rest; SHUTDOWN drops non-foundational overdue chatter). `prioritize_execution_items` filters non-recoverable items and collapse-suppressed source keys before ranking. `apps/core/execution/recovery_state.py :: compute_recovery_state` emits `{mode, day_narrative ∈ {on_track, behind_recoverable, behind_reset_required, day_lost_salvage, evening_closeout}, missed_foundational_count, recoverable_overdue_count, expired_count, recommended_strategy, reset_action_available}`; mode rules: SHUTDOWN at hour ≥ 20 with ≥ 3 unresolved; RECOVERY at hour ≥ 12 with ≥ 2 recoverable overdue; STABILIZE when ≥ 1 foundational missed AND a reset action is available; NORMAL otherwise. `apps/core/execution/execution_state.py` now exposes `eligible_actions` (active-block filtered subset for EXECUTION mode), `expired_items`, `deferred_items`, `collapsed_blocks`, `at_risk_actions`, `recovery_state`. Active-block gate moved upstream from selectors. `apps/core/execution/selectors.py` slimmed to pure picks: `get_next_action` reads `eligible_actions`, `get_biggest_risk` reads `at_risk_actions` with foundational-expired fall-through, `get_fix_priority` prefers reset / collapsed-block lever in RECOVERY/STABILIZE before existing unblock-count logic. No selector does priority computation, re-ranking, DB reads, or LLM calls. `apps/ai/cos_fact_statements.py :: build_recovery_brief` adds a 2–4 line "DAY MODE" hint to the locked-facts block ONLY when mode != NORMAL — keeps prompt lean in the common case. The classifier's `is_reset_action` flag is set ONLY by `activity_type` rules (`hydration`, `hygiene`, `movement`, `pause`, `faith`) or registry pin — title-token matching is forbidden. The classifier registry recognizes `event/service/appointment/meeting/class` as HARD_EXPIRED, `nutrition_anchor/weigh_in/measurement` as WINDOWED, supplements with `priority='optimization'` get 120 min grace, critical medications get 60 min, generic WINDOWED defaults to 90 min. Tests: 56 new tests in `apps/core/execution/tests/`; canonical 2:10 PM scenario asserts Church filtered, Protein Shake filtered, Shower kept via `recover_partially`, Fish Oil NOT at risk, RECOVERY/STABILIZE triggered post-noon, morning block collapsed. All 94 execution-layer tests pass. No DB migrations. Previous: 2026-04-28 Signal Rendering Framework — Phase 1: new `apps/core/signals/signal_renderer.py` is the single canonical interpretation layer that turns a `UnifiedSignal` into deterministic Label/Meaning/Action text via table lookup `SIGNAL_RENDER_MAP[(domain, type, severity)] → template`. NO LLM, NO branching per domain. Renderer must NOT depend on `signal.title` / `signal.message` — `normalize_signal()` strips them so producer-authored prose can never leak into user output. `LABEL_TAXONOMY = {Alert, Trend, Opportunity}` enforced; `Unclear`/`Mixed`/`Needs clarity` rejected. `DOMAIN_PRIORITY` maps health/medical→foundational, faith/meals/sleep→important, life/tasks→supporting. `select_top_signals(signals, max_n=2)` sorts by (priority, severity, confidence, recency); foundational always surfaces. `resolve_conflicts()` drops same-domain non-foundational signals when a foundational is present (e.g. glucose_high suppresses weight_loss_positive). `_TYPE_ALIAS` is a temporary translation layer so PIE/PRIE/PGE producers stay untouched in Phase 1; every alias use is logged. New endpoint `GET /api/signals/` exposes the renderer over HTTP. `apps/health/services/physical_decision.py` vitals branches now route through the renderer (legacy inline copy retained as fallback). `apps/ai/cos_fact_statements.py` summary builders + `apps/ai/beth_checkin_renderer.py` remain untouched — Phase 2/3 will lift them. Previous: 2026-04-26 CoS Strict Mode Isolation — three deterministic decision modes (Execution / Risk / Fix) now produce ONE line each with a strict format contract: `Next: X. Do this now.` / `Biggest risk: X. Fix this next.` / `Fix this first: X.`. No time math, no reason text, no follow-on, no commentary. `is_item_in_active_block` tightened so overdue items are Execution-eligible only if scheduled in the active block OR the immediately preceding canonical block — long-past items (e.g. 5:30 AM at noon) surface in Risk/Fix only. The locked-facts block sent to the LLM (`format_locked_facts_block`) is slimmed to JUST the next action line — no domain summaries, no overdue lists, no future items, no event acknowledgment — the LLM has nothing to blend. The richer `build_locked_facts()` dict remains available to the truth validator and other surfaces. The keyword router (`cos_mode_router.py`) now covers status queries (`how am I doing`, `where am I at`, `status`, `what's going on`, `update me`, `walk me through`, `where do I stand`, etc.) and defaults them to Execution; precedence is Fix > Risk > Execution. `beth_checkin_renderer.py` schedule guidance, escalation reasons, and directives use categorical phrasing only (`behind` / `at risk` / `upcoming` / `recoverable`) — no minute counts, no countdown language. Previous: 2026-04-26 Action Center: Time Block as Primary Execution Unit — every time-bound section in `_action_group.html` now renders one consistent parent control posting to `dashboard_v2:block_complete_toggle` (`actions/block/<block_key>/toggle/`). The new `BlockCompleteToggleAction` view in `apps/dashboard_v2/views.py` reads items from `build_today_execution(user)` (single source of truth), and either completes or undoes every item in the block. Intake optimization preserved: a block that is purely doses from one window delegates to the canonical `IntakeGroupLogAction` so the analytics rollup is a single window-level event. Mixed-domain blocks dispatch per item: `toggle_routine_completion` for routine items, `Task.mark_complete/mark_incomplete` for tasks, `IntakeLog` create/delete for individual doses (mirrors `IntakeLogAction`'s supply + event semantics). The homogeneity branch in `action_prioritizer.py :: build_grouped_action_center` is gone — result groups always use `group_type='time_block'`, with a new `intake_window_key` field exposed only when the block is purely one intake window (consumed by the new endpoint as an optimization hint). New public helper `time_block_key_for(time)` exported so endpoints match the Action Center's 15-minute rounding rule. Fixes the silent-skip bug where a block of 1 routine + 1 standalone task showed a routine checkbox that only completed the routine items. Existing per-domain endpoints (`routine_complete_toggle`, `intake_group_log`, `task_toggle`, `routine_schedule_toggle`, `intake_log`) untouched and reachable for non-Action-Center callers. Previous: 2026-04-26 CoS Decision Modes — Execution / Risk / Fix: new `apps/core/execution/execution_state.py :: build_execution_state(user, now)` composes Today Execution + active_block + prioritizer into a single state dict consumed by three deterministic selectors in `apps/core/execution/selectors.py` (`get_next_action`, `get_biggest_risk`, `get_fix_priority`). Mode resolution is keyword-based via `apps/ai/cos_mode_router.py` (NO LLM); `send_message()` shortcut + new `GET /assistant/api/cos/decision/?mode=...` endpoint expose the modes to chat and to iOS/future surfaces with identical structured payloads `{mode, primary_action, reason, follow_on, message}`. Risk-mode v1 deliberately omits "If ignored:" consequence text — no deterministic source. Fix-mode unblock counts derive from `dependency_gating.is_task_blocked` semantics via `state["blocked_dependents"]` (pre-computed in build_execution_state). `build_locked_next_action(user)` refactored to a thin wrapper over `get_next_action(state)`. Previous: 2026-04-26 CoS Time/Sequence Integrity — new `apps/core/execution/active_block.py` resolves the user's currently active execution block (morning / mid_morning / lunch / afternoon / evening / nightly) with per-user `scheduled_time` min/max as primary signal and `time_windows.WINDOW_HOURS` as static fallback; includes `LEAD_IN_MINUTES = 15` so the next block's earliest items become eligible only as the current block winds down. `build_locked_next_action()` in `apps/ai/cos_fact_statements.py` now restricts "Start with X" eligibility to `{overdue, now}` only — `next` (≤2h away) is referenced only as a follow-on hint, never as the primary recommendation — and applies the active-block gate so future-block items cannot override an unfinished current-block item even when foundational. `action_prioritizer.py` medicine groups now carry their `scheduled_time` as `time_display` (was empty string) so intra-tier sort orders intake correctly. Dashboard and CoS continue to share `prioritize_execution_items()`; with CoS restricted to overdue+now, both surfaces resolve the same primary action without a parallel engine. Regression test: at 07:55, with Measurements at 08:00 and Fish Oil at 09:00, "Start with Measurements" is mandatory. Previous: 2026-04-22 Phase 3 metric-trust — unified signal feed: new `apps/core/ai_signals/unified_feed.py` consolidates signals from PIE / PRIE / PGE / CDCE / cross-domain detector into a single `UnifiedSignal` shape with deterministic priority scoring, dedupe-key-first cluster collapse, source precedence (Guidance > Insight > Prediction > Cross-domain > Correlation > Drift), and bucket assignment into TOP / CRITICAL / POSITIVE lists. CoS context now exposes `top_signals`, `critical_signals`, `positive_signals`, and `signal_summary` keys; the adapter reuses the intelligence payload already loaded in `build_cos_context` (no extra DB queries). 21/21 new `SimpleTestCase` tests. Previous: 2026-04-21 Phase 2 metric-trust cleanup — CoS state-first hardening: added `goals.active_titles`, `goals.upcoming_titles`, `goals.overdue_titles`, and `habits.streaks_per_habit` to SAE state builders. Migrated 5 CoS raw reads (4 `LifeGoal`, 1 `HabitGoal`) to SAE. Replaced `FastingSession.objects.filter(...)` lookup with canonical `fasting.current_fast_active`. Added `log_state_gap()` observability hook; 3 `LabResult` reads now emit explicit `medical.recent_labs` / `medical.lab_test_counts` / `medical.lab_test_trends` state gap warnings instead of hiding the drift. New `apps/core/ai_orchestrator/cos_read_allowlist.py` enumerates every remaining direct ORM read in `cos_context.py` with classification (engine_output / self_state / continuity / reference_data / structured_lookup / gap_pending_state) and rationale. New `CosReadAllowlistTests` fails CI if an unlisted read appears or if counts diverge. Previous: 2026-04-20 Metric Access Layer: new `apps/core/ai_state/metric_access.py` + `metric_registry.py` as the single approved path for AI-facing metric reads. 10 `PersonalDataService.get_*_data()` methods migrated to read canonical SAE state instead of re-aggregating raw models — glucose/weight/sleep/food/steps/water/workout/journal/mood/medication. 12 methods deprecated to stubs returning None. In `cos_context.py`, the `_build_data_state_snapshot` lifetime-count block no longer issues 8 raw `.count()` queries; presence is derived from SAE signals. Medication adherence state now reads `health.medication_status`. `log_direct_orm_read()` telemetry added at 10 remaining direct-ORM read sites for Phase 2 cleanup. Purity-ratcheting test in `tests_metric_access.py` enforces zero aggregations in migrated files and blocks regressions elsewhere. Previous: 2026-04-15 Execution Escalation Engine: 4-level deterministic escalation in beth_checkin_renderer.py, trivial completion rule, duration estimate fixes, nudge state, move_later gating. Earlier: 2026-04-07 Workout-Tomorrow Hardening: workout event adapter is now date-aware — past/today → `WorkoutSession`, future → `WorkoutSchedule`; deterministic empty-state contract in `handle_query_event_history` structurally bypasses the LLM for empty/future queries via `ar.message` direct return; new generic `_is_future_tense_query` gate applied to every per-domain summary matcher in `deterministic_router.py` — generalizes future-tense protection across all domains. Earlier the same day: CDCE domain gating added to `detect_fasting_fitness` + `build_fasting_state`; `workout_consistency_score` now uses canonical schedule-based adherence; data migration 0123 purges stale fasting_fitness correlations for users with fasting disabled.))
...
4. [CoS Runtimes — Which Pipeline Actually Serves a Conversation](#cos-runtimes--which-pipeline-actually-serves-a-conversation)
5. [Model Interface Runtime (CURRENT CoS Pipeline)](#model-interface-runtime-current-cos-pipeline)
6. [CoS Context Pipeline (LEGACY runtime)](#cos-context-pipeline-legacy-runtime)
7. [CoS Proactive Check-In System](#cos-proactive-check-in-system)
8. [Truth Layer Architecture (SAE → PIE → PRIE → PGE)](#truth-layer-architecture)
9. [Known Bugs & Gap Analysis](#known-bugs--gap-analysis)
...
> **Conversational runtime ≠ intelligence pipeline.** The engines above are WLJ's deterministic
> intelligence. The *conversation* is served by whichever runtime the gateway selects — today the
> **model interface** (`apps/ai/model_interface/`). See
> [CoS Runtimes](#cos-runtimes--which-pipeline-actually-serves-a-conversation) before reading the
> [CoS Context Pipeline (LEGACY runtime)](#cos-context-pipeline-legacy-runtime) section.
...
| **SAME** Monitoring | `apps/core/ai_observability/same_engine.py` | `run_same()` | Celery Beat 60s | Every 60s | Engine heartbeats + scheduled-task states (OPS-1) | `OpsAnomaly`, `OpsNarrativeSnapshot`, `wlj:ops:stream_payload` cache |
| **SchedTaskMon** Beat-task monitor (OPS-1) | `apps/core/ai_observability/scheduled_task_monitor.py` | `detect_scheduled_task_missed_runs()` / `get_scheduled_tasks_telemetry()` | Celery `task_postrun` signal + SAME 60s | Per Beat-task run | `CELERY_BEAT_SCHEDULE` cadence + `ScheduledTaskRun` last-run state | `MISSED_RUN` `OpsAnomaly`, `scheduled_tasks` payload section |
| **StorageMon** (OPS-2) | `apps/core/ai_observability/storage_monitor.py` | `get_storage_telemetry()` | SAME 60s (5-min cache) | Every cycle | Postgres `pg_database_size`, Redis `INFO memory`, `shutil.disk_usage`, daily `StorageSnapshot` | `storage` payload section |
| **ChatQueueMon** (OPS-3) | `apps/core/ai_observability/chat_queue_monitor.py` | `get_chat_queue_telemetry()` | Celery publish/prerun/postrun signals + SAME 60s | Per chat task | Redis pending/active zsets + wait/completion lists | `chat_queue` payload section |
| **UpstreamHealth** (OPS-4) | `apps/core/ai_observability/upstream_health.py` | `record_llm_outcome()` / `get_upstream_health_telemetry()` | Passive at `AIService._log_usage` + SAME 60s | Per LLM call | Per-minute OK/err/latency cache buckets + `openai_rate_limited` breaker | `upstream_health` payload section |
| **ExecutiveSynthesis** (Command Center) | `apps/core/ai_observability/ops_executive.py` | `build_executive_summary()` | SAME 60s (runs LAST, reads other sections) | Every cycle | Assembled sections + integrity components + `OpsAnomaly` + engine dependency registry + cache KPI history | `executive` payload section (overall status, customer impact, explainable score, prioritized action, enriched incidents, narrative, trends) |
...
## CoS Runtimes — Which Pipeline Actually Serves a Conversation

> **Read this before the "CoS Context Pipeline" section that follows.** WLJ has THREE coexisting
> conversational runtimes behind ONE gateway. Exactly one serves a given user's turn. The current,
> certified CoS runtime is the **model interface** (`apps/ai/model_interface/`) — documented in the
> next section. The **"CoS Context Pipeline" section below describes the LEGACY runtime only**
> (`apps/ai/personal_assistant.py :: send_message()`), which serves users on neither flag.

**Gateway:** `apps/ai/cos_gateway/gateway.py :: CoSGateway.resolve_runtime(user)` — no surface picks a
runtime; the gateway does, once per request, in this precedence order:

| # | Runtime | Selector (UserPreferences) | Implementation | Status |
|---|---------|----------------------------|----------------|--------|
| 1 | `ModelInterfaceRuntime` | `use_model_interface` | `apps/ai/model_interface/` (`ModelInterfaceService`) | **CURRENT** — the certified CoS runtime |
| 2 | `ChatGPTCoSRuntime` | `evidence_tools_enabled(user)` (`use_chatgpt_cos`) | `apps/ai/chatgpt_cos/` | Prior clean runtime |
| 3 | `LegacyBethRuntime` | default (neither flag) | `apps/ai/personal_assistant.py` | **LEGACY** — what the next section documents |

Runtime classes: `apps/ai/cos_gateway/runtime.py`. Response envelope: `apps/ai/cos_gateway/envelope.py`.
Flag resolution is fail-safe — any error resolves to the previous behavior (False).

**Write authority is a SECOND flag:** `use_model_interface_writes`
(`ModelInterfaceService._writes_enabled()`, fail-safe **False → read-only**). It controls whether
action tools are exposed at all and whether `pending_confirmations` appears in the envelope.

---

## Model Interface Runtime (CURRENT CoS Pipeline)

**Service:** `apps/ai/model_interface/service.py :: ModelInterfaceService`
**Design doc:** `docs/WLJ_MODEL_INTERFACE_DESIGN.md` · **Contract:** `docs/WLJ_LLM_TRUTH_ACTION_CONTRACT.md`

WLJ owns truth; the model owns reasoning. Per turn the service (1) assembles the **Executive Context
Envelope** — structured deterministic DATA, not instructions; (2) exposes the tool surface; (3) drives
the model through the existing tool loop, wrapping every truth read in the canonical truth envelope and
auditing it; (4) optionally runs the bounded Phase-2 synthesis; (5) records the turn in the audit ledger.
It performs **no reasoning of its own** (no classifier, no planner) and issues **no direct writes** — all
writes go through the action interface → `execute_intent` → UAIO.

### Request Flow (worker path — the CoS does NOT run on web)

```
User message
  → CoSGateway.respond(...)                       [apps/ai/cos_gateway/gateway.py]
  → ModelInterfaceRuntime.respond(...)            [apps/ai/cos_gateway/runtime.py]
    ├─ typed-confirmation short-circuit  — an unambiguous text-only "yes"/"cancel" against an OPEN
    │    confirmation resolves DETERMINISTICALLY via `action_interface.resolve_typed_confirmation`
    │    (the model is never the load-bearing path for a confirm/cancel)
    ├─ multimodal arrival — `apps/ai/multimodal.ingest_uploads` stores artifacts BEFORE generation;
    │    active-artifact re-delivery re-perceives an artifact still held in Conversation State
    ├─ duplicate protection — `apps/ai/idempotency` in-flight marker → `duplicate_pending` event
    ├─ DURABLE TURN — user message + PENDING assistant message persisted synchronously (server-owned;
    │    the browser is a viewer, never the owner of a turn's lifetime)
    └─ streaming → run_model_interface_generation.delay(...)   ← Celery
                   [apps/ai/model_interface/tasks.py]
                     → ModelInterfaceService.generate(...)
                     → ALWAYS writes a terminal snapshot to `apps/ai/chat_stream_bus`
                       (done | error) so the SSE relay can never hang
       non-streaming → ModelInterfaceService.generate(...) synchronously in the runtime
```

**`run_model_interface_generation`** — `apps/ai/model_interface/tasks.py`

| Property | Value |
|----------|-------|
| Task name | `apps.ai.model_interface.run_model_interface_generation` |
| Worker registration | imported in `apps/ai/tasks.py` (so the worker autodiscovers it) |
| Queue | **default `celery` queue — NOT in `CELERY_TASK_ROUTES`.** Unlike `run_chat_generation` and `run_chatgpt_cos_generation` (routed to `CHAT_GENERATION_QUEUE`), this task is consumed by the **`wlj-worker`** Railway service |
| Limits | `soft_time_limit=95`, `time_limit=110`, `max_retries=0`, `acks_late=False` |
| Failure contract | `SoftTimeLimitExceeded` / any exception → `_fail()` writes a TERMINAL snapshot **and** marks the persisted `AssistantMessage` `status=failed` |
| Lifecycle | advances the pre-persisted turn `PENDING → RUNNING → COMPLETED/FAILED`; history excludes the current turn's two rows |
| On success | writes answer + `tools_called` to the assistant message; attaches the Rich Confirmation card (`confirmation.bind_conversation`) and the Reveal Target `navigation` directive to the `done` event |
| Always | clears the in-flight idempotency marker in `finally` |

> **DEPLOY CONSEQUENCE (verify before validating any CoS change):** the CoS executes in the separate
> **`wlj-worker`** Celery service. `/_health/` reports the **web** service only — a green web deploy does
> **not** mean the worker is running your code. Confirm the worker's commit before trusting a CoS
> observation.

**`warm_understanding`** — same module. Background warm of the Deterministic Understanding cache,
enqueued non-blocking via `safe_enqueue` on a cold read. `interpret()` is ~850 ms and must never run on
the request path.

### Executive Context Envelope (`build_standing_context`)

**Function:** `ModelInterfaceService.build_standing_context(page_context, conversation, writes_enabled, attachments)`
— `apps/ai/model_interface/service.py:155`

The envelope **only assembles**; it owns none of these interfaces. Each is read at its OWN freshness
(Architecture Law — refresh cadence is an ownership boundary). Every optional block is individually
guarded so the envelope can never hard-fail a turn.

| Key | Producer | Freshness | Notes |
|-----|----------|-----------|-------|
| `ai_relationship` | `apps/ai/cos_services/ai_relationship.py :: get_ai_relationship` | slow (projection) | Chosen assistant name + default relationship style |
| `deterministic_understanding` | `apps/ai/model_interface/understanding.py :: read()` | medium, **cache-first** | Cache `wlj:mi:understanding:{user_id}` (TTL 150s). Cold miss returns `{"status": "pending"}` and enqueues `warm_understanding` — **never computes on the request path**. Exposes ASSESSMENTS only (challenge, risk, workload, priority, patterns, direction), never prescriptions |
| `current_context` | `apps/ai/cos_services/current_context.py :: get_current_context_baseline` | fast | Clock, current screen + `focus`, capabilities, attachments |
| `conversation_state` | `apps/ai/model_interface/conversation_state.py :: read()` | per-turn | "What are we talking about / waiting on" — active subject + active artifacts carried across turns; durable in `AssistantConversation.metadata`. An **owned field, not a retrieval surface**: there is no `get_conversation_state` tool |
| `personal_truth` | `apps/ai/cos_services/personal_truth.py :: build_personal_truth` → `personal_truth_for_context` | cache-first | Durable explicit cross-module facts (targets, conditions, medications, relationships, priorities). **Same composer** as the `get_user_truth` tool — one source |
| `missions` | `apps/purpose/mission_link.py :: get_mission_map()["missions"]` | per-turn | Mission FACTS live here ONCE; `current_action` carries lightweight references into them (no duplicated prose) |
| `execution_state` | `apps/core/execution/decision_authority.py :: execution_facts(user, state=…)` | per-turn | The day's facts, including `execution_phase` |
| `current_action` | `decision_authority.current_action(user, state=…)`, `primary_action` enriched by `mission_link.enrich_action` | per-turn | `{reason, message, primary_action}` — the single deterministic producer of "what to do now" |
| `pending_confirmations` | `apps/ai/model_interface/confirmation.py :: list_open(user)` | per-turn | **Present ONLY when `writes_enabled`.** Lets the model resolve a SPECIFIC confirmation on the user's next "yes" (the id lives in a prior tool result, not the transcript) |
| `interview` | `apps/ai/cos_services/interview.py :: read(user, conversation)` | per-turn | **Present ONLY during an active Getting to Know You session**; absent entirely otherwise — that absence is what keeps deliberate teaching gated to this surface. An inventory of what is known / ruled out, never an agenda |

`build_execution_state(user)` is built **ONCE** per turn and both `execution_facts` and `current_action`
derive from that one state — no second selector, no divergence.

### System Prompt Assembly (`_prompt_sections` / `_system_prompt`)

**Functions:** `ModelInterfaceService._prompt_sections(standing_context)` builds the prompt as
NAMED, ORDERED sections; `_system_prompt` joins their values. Same text, same order — the dict
exists so Stage-0 telemetry can attribute characters to a section without parsing the finished
string.

```
1. constitution         (constitution.py :: CONSTITUTION — identity + guardrails)
2. current_situation    (ONE ordered block — see below)
3. structured_context   (=== STRUCTURED CONTEXT === + json.dumps(standing_context))
4. grounding            (_grounding_lead — after the context, so it is read last)
5. completion_reminder  (RESPONSE_COMPLETION_REMINDER — final, highest-salience position)
```

**CURRENT SITUATION** (`_current_situation`) replaced eight separate high-salience leads
(`f435f5bc`). They existed because the constitution had grown too long to stay salient, so each
restated its rules nearer the user's turn — and the restatements then competed with each other
and with the constitution (142 duplicated instruction mentions across the leads, measured). The
component builders still produce the content; it is assembled once, in a deliberate order,
because ORDER is what a short reply attaches to:

```
1. what we are doing / waiting on   _conversation_state_lead   (unresolved intent outranks all)
2. what is in play right now        _attachment_lead, _focus_lead, _executive_lead, _interview_lead
3. who he is / how to speak to him  _profile_lead, _persona_lead
```

Facts and interaction guidance only — every truth/safety rule stays in the constitution. The
block never restates a policy; it states a situation.

**The leads duplicate NO truth.** Each re-frames facts already in the structured context (single
source) and raises their salience — a fix for the evidence-utilization class where the model
quoted a stored target and then violated it, because the fact sat under an opaque key ~90%
through the prompt.

### Prompt telemetry and the constitution split (Stages 0–1, 2026-09-03)

- **`apps/ai/model_interface/telemetry.py`** — every turn records prompt characters by section,
  tools exposed vs actually called, tool-loop rounds, Phase-2 eligibility/effect, and context
  coverage onto the `response` audit row it already writes. Sizes, counts and WLJ's own tool
  names only — never conversation text or a value from the user's life. Read it with
  `python manage.py cos_prompt_baseline [--turns N] [--email …]`.
- **`apps/ai/model_interface/constitution_map.py`** — classifies all 34 constitution paragraphs
  as INVARIANT (24,246 chars, 15 blocks) or GUIDANCE (44,634 chars, 19 blocks) **without moving
  a character**; `test_constitution_structure_contract.py` proves byte-exact reconstruction, so
  policy cannot be added, deleted or reworded unnoticed. Full ledger:
  `docs/WLJ_COS_COGNITIVE_SIMPLIFICATION.md`.
- **`AIService._call_api_with_tools(..., metrics=dict)`** — optional caller-owned out-parameter
  filled with loop counters (rounds, tool-output size, history trimming). Default `None` leaves
  the loop byte-identical.

### Tool Surface (`apps/ai/model_interface/constitution.py`)

**Entry point:** `all_tools(writes_enabled=True)` — `constitution.py:1870`

```
all_tools = truth_tools() + [navigate_to_workspace]        # always
          + action_tools()                                  # only when writes_enabled
```

Argument enums are generated at call time from the live registries (`_valid_domains()`,
`_valid_health_keys()`, `_valid_truth_*_domains()`, `_valid_ranked_entity_subjects()`, `_named_periods()`)
— **the model can never invent an interface WLJ already owns**.

**Truth tools (16, read-only, always exposed)** — `truth_tools()`:

| Tool | Reads |
|------|-------|
| `get_domain_state` | Current state of one life domain |
| `search_history` | Free-text search across history (optionally domain-scoped) |
| `get_history` | A metric's history for a domain |
| `get_readings` | Individual intra-day readings |
| `get_event_frequency` | How often an event occurs |
| `get_consistency` | Consistency of a metric/behavior |
| `get_change_point` | Where a metric materially changed |
| `get_ranked_entity` | Ranked entities for a subject |
| `get_comparison` | One metric across two periods |
| `get_adherence` | Adherence for a domain |
| `get_entity` | Record-level entities |
| `get_analysis` | Composed analysis of a subject in a domain |
| `get_user_truth` | Personal Truth (same composer as the `personal_truth` envelope field) |
| `get_foundational_health_facts` | Foundational health facts |
| `get_execution_review` | A day's INTENDED execution (a projection; owns nothing) |
| `get_data_health` | Missing-data / sync health |

Every truth read is wrapped in the canonical truth envelope (`apps/core/truth/envelope.py` via
`_wrap_truth`) and audited as `kind="truth"`.

**Reveal tool (always exposed — navigation is not a state mutation):**
`navigate_to_workspace` → `apps/ai/cos_services/reveal.py :: resolve_reveal`. The model names the target
in words; WLJ resolves it to a real URL through the existing `TeachingDestination`/`resolve_route`
authority and owns the "already here" relation.

**Action tools (exposed ONLY when `use_model_interface_writes`)** — `action_tools()`:

1. **The curated named write set** — `_named_action_tools()`, sourced from the existing intent registry
   `apps.ai.intents.ALL_INTENT_TOOLS`, filtered and ordered by **`ALLOWED_WRITE_INTENTS`**
   (`constitution.py:1563`). No copies, no parameter-mapping layer, one source of truth — the model calls
   them by name with the real handler params.
   ...
   ALLOWED_WRITE_INTENTS = (
       ...
       # M4 — real-time journaling
       "create_journal_entry", "add_gratitude",
   )
   ```

2. **`resolve_pending_action`** — the action-agnostic confirmation step. Resolves ONE **specific** bound
   confirmation by `confirmation_id`; the model never guesses an id and never re-issues the original action.
3. **`complete_execution_item`** — THE completion verb for anything WLJ tracks as an execution item.
   Identity-first (`source_type` + `source_id` + a `title` WLJ verifies agrees, else it REFUSES the write);
   retrospective mode reconciles the day it actually happened.
4. **`record_interview_knowledge`** — deliberate teaching, gated to an active Getting to Know You session.
5. **`next_review_item`** — the guided-review driver (Blocker #15).
6. **`schedule_follow_up`** — durable follow-through (`ConversationFollowUp`, fired by PGS).

> **There is no generic `request_action`.** Writes are a curated, named, deterministic set — nothing else
> is reachable, and the read-only stage exposes none of them.

### Phase 2 — Bounded Executive Synthesis

**Module:** `apps/ai/model_interface/synthesis.py` (invoked from `_generate_turn`)

Phase 1 (the tool loop) INVESTIGATES; Phase 2 JUDGES. Eligibility is a pure runtime signal —
`synthesis_eligible(evidence)` requires **≥2 independent substantive truth surfaces** — never a phrase
classifier, so narrow lookups stay single-phase. `run_executive_synthesis` is the **SAME model**, ONE
bounded **no-tool** call that reasons over the gathered EVIDENCE and never sees Phase-1's prose (this is
not judge-the-judge). On empty or error the grounded Phase-1 answer is kept — the turn is never lost. The
result carries `synthesis_used`.

Evidence handed to Phase 2 is filtered: `_SCAFFOLD_KEYS` (instruction/metadata) and `_VERDICT_KEYS`
(`momentum_score`, `momentum_trend`, `recommended_action`, `strategic_summary`, …) are stripped — **WLJ
hands the model FACTS, never a pre-decided verdict.** The facts underneath (milestone %, target dates,
completion counts, last activity) stay, so the model can substantiate its judgment on challenge.

### Audit & Cost Accounting

- Every truth read, action call, and the final response are recorded via
  `apps/ai/cos_services/audit.py :: record_tool_call` (`kind="truth" | "action" | "response"`) into
  `ToolCallLog`, keyed by a **turn id that is unique per turn** (`_new_turn_id`) so an incident can be
  replayed turn by turn.
- `generate()` wraps the whole turn in `llm_traffic_context` (`apps/ai/llm_accounting.py`) before the first
  provider call — a turn can bill several requests (Phase 1 + tool continuations + Phase 2), and all of
  them must attribute to one source: `interactive_chat`, or `getting_to_know_you` when an interview
  session is active, with `traffic_class=production`.
- Deterministic subject anchoring: `_SUBJECT_BEARING_TOOLS` / `_subject_from_truth_result` derive the
  ACTIVE SUBJECT from any successful truth retrieval (references only, never prose) and
  `conversation_state.record_turn` carries it to the next turn.

---

## CoS Context Pipeline (LEGACY runtime)

> **⚠️ LEGACY — this entire section documents the `LegacyBethRuntime` path**
> (`apps/ai/personal_assistant.py :: send_message()` + `apps/core/ai_orchestrator/cos_context.py`), which
> serves only users on **neither** `use_model_interface` nor `use_chatgpt_cos`. It is **NOT** the pipeline
> that serves the certified CoS. Everything below — the request flow, the 19 parallel context builders,
> the v4–v8 system-prompt layers, `format_cos_system_injection()`, and the `cos_ctx:*` caching strategy —
> belongs to that legacy runtime and has **no counterpart in the model interface**, which builds its
> context via `ModelInterfaceService.build_standing_context` (see the section above).
>
> Before applying anything here to a live CoS issue, confirm which runtime owns the user
> (`CoSGateway.resolve_runtime`).
...
### Context Builder (19 Parallel Builders) — LEGACY
...
### System Prompt Assembly (Priority Order) — LEGACY
...
### CoS Context Injection Output — LEGACY
...
### CoS Coherence Guards — Phase 1 (observe-only) (2026-06-14)

`apps/ai/cos_coherence_guards.py` adds three trust detectors that extend the existing post-response truth chain (`cos_truth_validator` + `health_truth` weight guards). Phase 1 is **observe-only**: they DETECT and log but never alter Beth's output and never raise.

- **G1 — Count coherence.** Invariant `routine_total − routine_done == len(pending_names)` (plus impossible-med-count check). Catches "19 of 25 complete" rendered beside 5 pending items. Wired into `cos_fact_statements.build_locked_facts()`.
- **G2 — Operational entity hallucination.** Flags invented medication GROUP labels ("Nightly Medications") — meds have no grouping entity, so a time-word+med-noun phrase absent from the canonical entity set (`build_canonical_entity_set()`: active Intake names + routine/item names) is fabricated. Wired into `cos_truth_validator.validate_locked_facts()`.
- **G3 — Calorie metric coherence.** `get_canonical_nutrition()` is the single "today calories vs target" reader (live FoodEntry ÷ target); detector logs when a "N% under/over target" calorie claim in Beth's reply diverges >2pp from it (the "dashboard 22% vs Beth 26%" two-truths break). Wired into `validate_locked_facts()`.

**Flags:** `WLJ_BETH_COHERENCE_DIAG_ENABLED` (master, default True); `WLJ_BETH_COUNT_GUARD_ENABLED` / `WLJ_BETH_ENTITY_GUARD_ENABLED` (**default True — fail-closed, 2026-06-15**); `WLJ_BETH_NUTRITION_GUARD_ENABLED` (observe-only, default False — the calorie fix is a source relabel, not text correction). Log markers: `[CoS COUNT GUARD]`, `[CoS ENTITY GUARD]`, `[CoS CALORIE GUARD]` (ERROR when fired; `FAIL-CLOSED enforce=True` when correcting).

**Phase 2 enforcement (2026-06-15, live):** count + entity guards correct Beth's reply IN PLACE in `validate_locked_facts` via the weight-delta pattern (`should_reject=False` → applied on both streaming and non-streaming, no regeneration). `enforce_count_coherence` fires only on incoherent canonical counts; `enforce_entity_hallucination` strips fabricated med-group labels. Count coherence also fixed at source in `_build_routine_summary`. Calorie: `NutritionCalorieTrendRule` title now says "trending … (7-day avg)" so the 7-day trend can't masquerade as today's `calorie_compliance_pct`.

**Future:** generalize numeric plausibility (glucose, etc.) + a shared Reconciliation Mode object.
...
### Model Interface Runtime (CURRENT CoS)

| File | Purpose | Lines |
|------|---------|-------|
| `apps/ai/cos_gateway/gateway.py` | `CoSGateway.resolve_runtime()` — the ONE runtime selector | ~125 |
| `apps/ai/cos_gateway/runtime.py` | The three runtimes (`ModelInterfaceRuntime`, `ChatGPTCoSRuntime`, `LegacyBethRuntime`) | ~479 |
| `apps/ai/cos_gateway/envelope.py` | `CoSResponse` envelope + runtime/surface constants | ~70 |
| `apps/ai/model_interface/service.py` | `ModelInterfaceService` — envelope, system prompt, tool dispatch, turn audit | ~1,526 |
| `apps/ai/model_interface/constitution.py` | `CONSTITUTION`, `truth_tools()`, `action_tools()`, `all_tools()`, `ALLOWED_WRITE_INTENTS` | ~1,879 |
| `apps/ai/model_interface/tasks.py` | `run_model_interface_generation` (worker) + `warm_understanding` | ~188 |
| `apps/ai/model_interface/understanding.py` | Deterministic Understanding — cache-first `read()` / background `warm()` | ~144 |
| `apps/ai/model_interface/conversation_state.py` | Conversation State — active subject/artifacts across turns | ~286 |
| `apps/ai/model_interface/synthesis.py` | Phase 2 bounded Executive Synthesis (evidence filtering + eligibility) | ~342 |
| `apps/ai/model_interface/confirmation.py` | Bound confirmations (`create`/`list_open`/`bind_conversation`/`consume`) | ~173 |
| `apps/ai/cos_services/` | The truth/action surfaces the tools call (`domain_*`, `personal_truth`, `execution_review`, `reveal`, `interview`, `audit`, `action_interface`) | — |
| `apps/core/truth/envelope.py` | Canonical truth envelope (freshness / confidence / source) | — |
| `apps/ai/chat_stream_bus.py` | SSE relay snapshots (shared with the legacy/ChatGPT streaming paths) | — |

### Core Pipeline (LEGACY runtime)
