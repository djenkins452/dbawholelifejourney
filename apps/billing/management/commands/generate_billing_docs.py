"""
Generate billing documentation from database configuration.

This command creates markdown documentation that reflects the current
billing configuration stored in the database, ensuring documentation
stays in sync with admin-managed settings.
...
class Command(BaseCommand):
    help = 'Generate billing documentation from database configuration'
    ...
    def handle(self, *args, **options):
        try:
            config_obj = BillingConfiguration.get_config()
            config = config_obj.as_dict()
        except Exception as e:
            self.stderr.write(
                self.style.ERROR(f'Error loading billing configuration: {e}')
            )
            return

        if not config:
            self.stderr.write(
                self.style.ERROR('Billing configuration not found in database')
            )
            return
...
To change pricing or rewards:
1. Go to Django Admin: `/admin/billing/billingconfiguration/`
2. Update the values in the Billing Configuration form
3. Save changes (they take effect immediately)
4. Run `python manage.py generate_billing_docs` to regenerate this documentation

---

*Auto-generated by `python manage.py generate_billing_docs` from database configuration*
"""
