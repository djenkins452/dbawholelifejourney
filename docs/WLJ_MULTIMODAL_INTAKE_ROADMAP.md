...
| **Phase 1.3 — Perception** (only the perception step varies by type) | 🟡 IN PROGRESS | — | M1 PDF → M2 Office → M3 Audio → M4 Video |
| ├ P1.3-M1 **PDF perception** | ✅ DONE | _(this change)_ | Deterministic text extraction (pdfplumber — mechanical decode, not interpretation; Constitution §2). New `apps/ai/perception.py` (`perceive(content_type, raw)` dispatched by type; PDF today) + background task `perceive_artifact` (request-path-safe) + `MultimodalArtifact.perception_status`/`extracted_text`/`page_count` (migration `capture/0008`). `store_and_persist_artifact` queues perception for perceivable types; `attachments_from_ids` surfaces the extracted `text` (bounded 60K) + `page_count` (+ `processing`/`unreadable` signals) so the model reads/summarizes/answers/compares. Constitution ATTACHMENTS prompt updated for documents. Tests: `apps/ai/tests/test_pdf_perception.py` (11, real reportlab PDFs + end-to-end). Multi-turn recall + cross-conversation retrieval = the Artifacts-as-Truth surface (next). |
