...
<!-- RECENT_FIXES_START -->
- **AI Personal Profile (2025-12-28):** Added personal AI profile feature to Preferences. Users can enter details about themselves (age, family, health conditions, interests, goals, etc.) that the AI uses to personalize dashboard insights and coaching messages. Features: (1) New `ai_profile` TextField in UserPreferences model (max 2000 chars), (2) Personal AI Profile section in Preferences page (shown when AI enabled + data consent given), (3) Content moderation via `apps/ai/profile_moderation.py` - blocks prompt injection attempts, harmful content, and sanitizes input, (4) Integration with AIService's system prompts - profile wrapped in bounded context with safety guidelines. Security measures: regex-based prompt injection detection (16 patterns), harmful content filtering, PII warnings (email, SSN patterns), control character removal, prompt delimiter escaping. Industry best practice: profile passed as bounded user-provided context, never treated as system instructions. 24 new tests for profile moderation, context building, service integration, and form handling. Files: `apps/users/models.py` (ai_profile field), `apps/users/migrations/0016_add_ai_profile.py`, `apps/ai/profile_moderation.py` (moderation module), `apps/ai/services.py` (user_profile param), `apps/ai/dashboard_ai.py` (profile integration), `templates/users/preferences.html` (UI section), `apps/ai/tests/test_ai_comprehensive.py` (24 tests).
- **Profile Picture Save Fix (2025-12-28):** Fixed bug where profile picture (avatar) was not being saved for some users (reported by heatherjenkins74@gmail). Two issues identified: (1) The ProfileForm used `FileInput` widget which doesn't preserve existing files when form is resubmitted without a new file - existing avatars were being cleared on any profile update. Fixed by adding logic in `save()` method to restore `self.instance.avatar` when no new file is uploaded. (2) The `clean_avatar()` method's content_type validation could reject valid images from iPhone (HEIC/HEIF) or browsers that send `application/octet-stream`. Added support for `image/heic`, `image/heif`, `image/webp`, and `application/octet-stream` content types. 6 new tests: form preserves existing avatar, clear_avatar checkbox works, rejects files >2MB, accepts valid images, handles HEIC content type, handles missing content type. Files: `apps/users/forms.py` (ProfileForm.clean_avatar, ProfileForm.save), `apps/users/tests/test_users_comprehensive.py` (ProfilePictureTest, ProfileFormTest classes).
- **AI Camera Medicine Purpose Auto-Fetch (2025-12-28):** When scanning a medicine bottle with AI Camera, the OpenAI Vision API now automatically looks up the common medical purpose of the medication (e.g., "blood pressure control", "pain relief", "cholesterol management") and includes it in the action URL. The medicine create form reads this purpose parameter and pre-populates the purpose field, saving users from manually entering it. Users can still override the AI-suggested purpose with their own specific reason. 6 new tests (4 vision URL building, 2 medicine form prefill). Files: `apps/scan/services/vision.py` (updated prompt and _build_actions), `apps/health/tests/test_medicine.py`, `apps/scan/tests/test_vision.py`.
- **AI Camera Medicine Form Prefill Fix (2025-12-28):** Fixed bug where AI Camera scanned medicine data was not populating the medicine add form. When scanning a medicine bottle, the AI correctly identified the medicine but the form fields were empty. Fixes: (1) Added `get_initial()` method to `MedicineCreateView` to read query parameters (name, dose, directions, quantity, purpose), (2) Updated vision service `_build_actions()` to include all extracted fields in the URL (dose, directions, quantity), not just the name. Directions populate the notes field, quantity is parsed to extract the supply count (e.g., "30 tablets" → 30). Supplements also now pass all fields. 11 new tests (6 for medicine form prefill, 5 for vision service URL building). Files: `apps/health/views.py` (MedicineCreateView.get_initial), `apps/scan/services/vision.py` (_build_actions for medicine/supplement).
