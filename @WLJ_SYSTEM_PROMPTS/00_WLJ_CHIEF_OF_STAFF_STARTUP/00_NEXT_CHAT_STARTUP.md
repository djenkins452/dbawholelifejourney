...
1. **Next engineering milestone → OPS-8b (Attachment persistence + dedup + S3 artifact bucket)** — *Operational Hardening.* Multimodal durability + dedup rate + the unmonitored S3 audio bucket. **✅ OPS-8a DONE** (`confirmation_audit` section: confirmation-queue health + audit-stream liveness). Follow the OPS-2/5/7 monitor pattern: telemetry-only, request-path-safe, own section + Ops Wall card. Backlog task — present + discuss scope, wait for "go". `docs/WLJ_OPS_WALL_COVERAGE.md §4.1`.
2. **Then the rest of the post-Phase-I backlog** (`§4.1`): OPS-9 (build-runner/deploy — re-scope small), OPS-11 (retire inert legacy remediation — opportunistic tech-debt). **OPS-6** (`owner`) deferred (near-zero value for a single-owner system); **OPS-10** (direct Beat) closed as accept-inference (subsumed by scheduler_health + OPS-1 + OPS-7).
...
- **WLJ Operations is the primary initiative.** Engineering (Claude, next chat): start **OPS-8b — Attachment persistence + dedup + S3 artifact bucket** (Operational Hardening; OPS-8a is done). Present the scope, discuss, wait for "go". Follow the OPS-2/5/7 monitor pattern (telemetry-only, request-path-safe, own section + card).
