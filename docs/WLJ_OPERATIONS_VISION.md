...
| **DNE engine MISSED_RUN** ("Engine Execution") | Healthy engine flagged: writes an `EngineRun` unconditionally every cycle; 600s cadence + 60s fallback jitter = 660s deadline, but dispatched on the 300s ISE grid (real spacing 600–900s) ⇒ flaps. Monitor false-positive. | **RECONFIGURE** (pending confirm) | Requires prod `EngineRun` rows for DNE (last hour): ~10–13 min spacing + `status=success` ⇒ confirm false-positive ⇒ widen jitter to real cadence; **zero rows ⇒ contradicts investigation, stop & report** (genuine defect). | Blocked on prod read — calibration **not applied**. |

**Note:** all four surfaced as "Recovery Skipped" only because the beat-retry recovery pilot ships dark (observe-only R0) — a presentation artifact (ADR-27), not a per-subsystem signal.
