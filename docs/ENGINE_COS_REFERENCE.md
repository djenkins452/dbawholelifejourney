...
**Flags:** `WLJ_BETH_COHERENCE_DIAG_ENABLED` (master, default True); `WLJ_BETH_COUNT_GUARD_ENABLED` / `WLJ_BETH_ENTITY_GUARD_ENABLED` / `WLJ_BETH_NUTRITION_GUARD_ENABLED` (Phase 2 fail-closed enforcement, default False). Log markers: `[CoS COUNT GUARD]`, `[CoS ENTITY GUARD]`, `[CoS CALORIE GUARD]` (all at ERROR when fired, tagged `OBSERVE-ONLY enforce=<bool>`).

**Phase 2 (not yet built):** flip per-guard flags to fail closed + add Reconciliation Mode (Beth says "something looks inconsistent, let me reconcile" instead of narrating), once telemetry confirms low false-positive rates.
