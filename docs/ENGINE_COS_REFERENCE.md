...
### System Prompt Assembly (`_system_prompt`)

**Function:** `ModelInterfaceService._system_prompt(standing_context)` — `service.py:792`

```
1.  CONSTITUTION                     (constitution.py :: CONSTITUTION — identity + guardrails)
2.  _persona_lead                    (chosen name / relationship style)
3.  _interview_lead                  (only during a Getting to Know You session)
4.  _attachment_lead                 (what the user just uploaded)
5.  _conversation_state_lead         (active subject/artifacts carried in)
6.  _executive_lead                  (executive posture from deterministic_understanding)
7.  _focus_lead                      (ON SCREEN RIGHT NOW — inlines the focused object's content)
8.  _profile_lead                    (durable constraints: targets, conditions, allergies — as HARD CONSTRAINTS)
9.  === STRUCTURED CONTEXT ===  json.dumps(standing_context)
10. _grounding_lead                  (answer/grounding rules — after the context, so read last)
11. RESPONSE_COMPLETION_REMINDER     (final, highest-salience position)
```

**The leads duplicate NO truth.** Each one re-frames facts that are already in the structured context
(single source) and raises their salience — a fix for the evidence-utilization class where the model
quoted a stored target and then violated it, because the fact sat under an opaque key ~90% through a
~60k-char prompt.
